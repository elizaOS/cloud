/**
 * Miniapp Domains Schema
 * 
 * Manages subdomains and custom domains for miniapps.
...
export const miniappDomains = pgTable(
  "miniapp_domains",
  {
  ...
  (table) => ({
    app_id_idx: index("miniapp_domains_app_id_idx").on(table.app_id),
    subdomain_idx: uniqueIndex("miniapp_domains_subdomain_idx").on(table.subdomain),
    custom_domain_idx: uniqueIndex("miniapp_domains_custom_domain_idx").on(
      table.custom_domain
    ),
    vercel_domain_idx: index("miniapp_domains_vercel_domain_idx").on(
      table.vercel_domain_id
    ),
  })
);

export type MiniappDomain = InferSelectModel<typeof miniappDomains>;
export type NewMiniappDomain = InferInsertModel<typeof miniappDomains>;
