...
/**
 * OPTIONS /api/v1/app/billing/checkout
 * CORS preflight handler for app checkout endpoint.
 *
 ...
 * Creates a Stripe checkout session for app users to purchase credits.
 * Supports both credit packs and custom amounts. Returns a checkout URL that redirects back to the app after payment.
 *
 * Request Body:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info("[App Billing] Checkout session created", {
    ...
    logger.error("[App Billing] Error creating checkout session", {
      error,
    });
