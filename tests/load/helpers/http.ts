...
function checkStatus(
  ...
  const predicate =
    typeof expected === "function" ? expected : (s: number) => s === expected;
...
export function httpGet<T>(path: string, options?: HttpOptions): T | null {
  ...
  const res: K6Response = http.get(
    `${baseUrl}${path}`,
    buildParams(options, defaultHeaders),
  );
...
export function httpPost<T>(
  ...
  const res: K6Response = http.post(
    `${baseUrl}${path}`,
    typeof body === "string" ? body : JSON.stringify(body),
    buildParams(options, headers),
  );
  const expected = options?.expectedStatus ?? 200;
...
export function httpPatch<T>(
  ...
  const res: K6Response = http.patch(
    `${baseUrl}${path}`,
    JSON.stringify(body),
    buildParams(options, headers),
  );
...
export function httpPostFile<T>(
  path: string,
  file: { content: string; name: string; mimeType: string },
  options?: HttpOptions,
): T | null {
  const authHeader = (options?.headers ?? headers).Authorization;
  if (!authHeader)
    throw new Error("Authorization header required for file upload");
