 ...
 * Extended App Tests
 *
 * Tests additional app functionality:
 * - Photo generation
 * - Field generation
 * - Image upload
 * - Proxy endpoints
...
test.describe("App Image Upload API", () => {
  test.skip(() => !API_KEY, "TEST_API_KEY environment variable required");

  test("POST /api/upload-images uploads images", async ({ request }) => {
    const response = await request.post(`${APP_URL}/api/upload-images`, {
      headers: {
        Authorization: `Bearer ${API_KEY}`,
      },
      multipart: {
...
test.describe("App Proxy API", () => {
  ...
  test("proxy handles POST requests", async ({ request }) => {
    const response = await request.post(`${APP_URL}/api/proxy/agents`, {
      headers: authHeaders(),
      data: {
    ...
    if (response.status() === 200 || response.status() === 201) {
      ...
      if (data.agent?.id) {
        await request.delete(`${APP_URL}/api/proxy/agents/${data.agent.id}`, {
          headers: authHeaders(),
        });
      }
    } else {
...
test.describe("App Billing API", () => {
  ...
  test("GET /api/v1/app/billing/credit-packs returns packs", async ({ request }) => {
    ...
    if (response.status() === 200) {
      ...
      console.log(`ℹ️ App credit packs returned ${response.status()}`);
    }
  });
  ...
  test("POST /api/v1/app/billing/checkout creates checkout", async ({ request }) => {
    ...
    if (response.status() === 200 || response.status() === 201) {
      const data = await response.json();
      expect(data).toBeDefined();
      console.log("✅ App checkout endpoint works");

      if (data.url || data.checkoutUrl) {
        console.log("   Checkout URL provided");
      }
    } else {
      console.log(`ℹ️ App checkout returned ${response.status()}`);
    }
  });
});
...
test.describe("App Rewards API", () => {
  test.skip(() => !API_KEY, "TEST_API_KEY environment variable required");

  test("GET /api/v1/app/rewards returns rewards info", async ({ request }) => {
...
test.describe("App Pages UI", () => {
  test("app home page loads", async ({ page }) => {
    ...
    console.log("✅ App home page loads");
  });

  test("app chats page loads", async ({ page }) => {
    ...
    console.log("✅ App chats page loads");
  });
  ...
  test("app settings page loads", async ({ page }) => {
    ...
    const content = await page.locator("body").textContent();
    expect(content?.length).toBeGreaterThan(0);
    console.log("✅ App settings page loads");
  });

  test("app connecting page loads", async ({ page }) => {
    await page.goto(`${APP_URL}/connecting`);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);

    const content = await page.locator("body").textContent();
    expect(content?.length).toBeGreaterThan(0);
    console.log("✅ App connecting page loads");
  });

  test("app billing success page loads", async ({ page }) => {
    await page.goto(`${APP_URL}/billing/success`);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);

    const content = await page.locator("body").textContent();
    expect(content?.length).toBeGreaterThan(0);
    console.log("✅ App billing success page loads");
  });
});

test.describe("App Character Creator UI", () => {
...
test.describe("App Agent Detail Page", () => {
  test("agent detail page handles invalid ID", async ({ page }) => {
    await page.goto(`${APP_URL}/agents/invalid-agent-id`);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);
