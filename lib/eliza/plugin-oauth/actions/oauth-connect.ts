...
export const oauthConnectAction: ActionWithParams = {
  ...
  similes: [
    ...
    "CONNECT_ASANA", "LINK_ASANA",
    "ADD_INTEGRATION", "SETUP_CONNECTION", "LINK_GOOGLE", "AUTHENTICATE",
    "LINK_LINEAR", "LINK_SLACK", "LINK_GITHUB", "LINK_NOTION",
  ],
  description:
    "Connect an OAuth platform for the user. Returns an authorization URL. After user completes OAuth in browser, they should say 'done' to verify the connection. Available: google, linear, slack, github, notion, asana",

  parameters: {
    platform: {
      type: "string",
      description: "Platform to connect. Available: google, linear, slack, github, notion, asana",
      required: true,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "connect asana" } },
      { name: "{{name2}}", content: { text: "Connect Asana: https://app.asana.com/...", actions: ["OAUTH_CONNECT"] } },
    ],
  ] as ActionExample[][],
};
