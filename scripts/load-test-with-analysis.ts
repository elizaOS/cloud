...
const VALID_SCENARIOS = ["smoke", "stress", "spike", "soak", "full-platform", "throughput"];
...
async function runLoadTest(): Promise<void> {
  return new Promise((resolve, reject) => {
    const command = SCENARIO === "full-platform" ? "load:local" : `load:${SCENARIO}`;
...
async function analyzeBottlenecks(before: Map<string, SlowQuery>, after: Map<string, SlowQuery>): Promise<void> {
  console.log("\n" + "=".repeat(80));
  console.log("📊 SLOW QUERY ANALYSIS - BOTTLENECKS IDENTIFIED DURING LOAD TEST");
  ...
  worsened.sort((a, b) => b.callsDelta * b.query.avg_duration_ms - a.callsDelta * a.query.avg_duration_ms);
  ...
  if (newQueries.length > 0) {
    ...
    for (const q of newQueries.slice(0, 10)) {
      console.log(`\n  Hash: ${q.query_hash}`);
      console.log(`  Avg: ${q.avg_duration_ms.toFixed(0)}ms | Max: ${q.max_duration_ms}ms | Calls: ${q.call_count}`);
  ...
  if (worsened.length > 0) {
    ...
    for (const { query, callsDelta, avgDelta } of worsened.slice(0, 10)) {
      ...
      console.log(`  Calls: +${callsDelta} | Avg: ${query.avg_duration_ms.toFixed(0)}ms (${avgDelta > 0 ? "+" : ""}${avgDelta.toFixed(0)}ms)`);
      console.log(`  SQL: ${query.sql_text.substring(0, 200)}...`);
    }
  }
  
  // Recommendations
  const highFrequency = [...worsened, ...newQueries.map(q => ({ query: q, callsDelta: q.call_count, avgDelta: 0 }))]
  ...
  if (highFrequency.length > 0 || slowest.length > 0) {
    ...
    for (const q of slowest) {
      console.log(`   Optimize: ${q.query_hash} (${q.avg_duration_ms.toFixed(0)}ms)`);
    }
  }
  console.log();
}
...
async function main() {
  ...
  console.log(`   Found ${afterSnapshot.size} slow query patterns (${afterSnapshot.size - beforeSnapshot.size} new)`);
