...
const executeSchema = z.object({
  ...
  language: z.enum(["python", "javascript", "typescript", "shell", "rust", "go"]).optional(),
...
async function handlePOST(request: NextRequest, context: RouteContext) {
  ...
  if (session.status !== "ready" && session.status !== "executing") {
    return NextResponse.json({ error: `Session not active: ${session.status}` }, { status: 400 });
  }

  if (body.type === "code") {
    if (!body.code || !body.language) {
      return NextResponse.json({ error: "code and language required for type=code" }, { status: 400 });
    }
    const result = await codeAgentService.executeCode({
      sessionId, language: body.language, code: body.code,
      options: { workingDirectory: body.workingDirectory, timeout: body.timeout, env: body.env },
    });
    return NextResponse.json({ result });
  }

  if (!body.command) {
    return NextResponse.json({ error: "command required for type=command" }, { status: 400 });
  }
  const result = await codeAgentService.runCommand({
    sessionId, command: body.command, args: body.args,
    options: { workingDirectory: body.workingDirectory, timeout: body.timeout, env: body.env },
  });
  return NextResponse.json({ result });
}
