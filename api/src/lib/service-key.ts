/**
 * Workers-compatible service-key auth.
 *
 * Mirrors `packages/lib/auth/service-key.ts` but uses WebCrypto instead of
 * `node:crypto` so it runs on Workers without the nodejs_compat shim.
 *
...
/**
 * Constant-time(-ish) comparison via SHA-256 digests of HMAC outputs keyed
 * with a per-call random salt. The HMAC blinds the inputs so direct length
 * inspection of either side reveals nothing.
 */
async function constantTimeEqual(a: string, b: string): Promise<boolean> {
  ...
  // Constant-time XOR compare on equal-length (32-byte) digests.
  let diff = 0;
