...
```bash
curl -X GET "https://cloud.eliza.ai/.well-known/agent-card.json"
```
...
```json
{
  ...
  "image": "https://cloud.eliza.ai/logo.png",
  "version": "1.0.0",
  "capabilities": {
    "streaming": true,
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/discovery" \
  ...
  "agents": [
    {
      ...
      "url": "https://cloud.eliza.ai/agents/abc123",
...
```bash
curl -X POST "https://cloud.eliza.ai/api/a2a" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```javascript
const response = await fetch('https://cloud.eliza.ai/api/a2a', {
  method: 'POST',
  headers: {
...
```bash
curl -X POST "https://cloud.eliza.ai/api/agents/{agentId}/registration.json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
<Callout type="info">
See the [A2A specification](https://github.com/google/a2a) for full protocol details.
</Callout>

## Best Practices

<Cards>
  <Cards.Card title="Clear Skills">
  ...
  <Cards.Card title="Rate Limiting">
    Respect rate limits of other agents
  </Cards.Card>
  <Cards.Card title="Security">
