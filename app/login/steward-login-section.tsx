...
export default function StewardLoginSection() {
  ...
  async function handlePasskey() {
    ...
    setLoading("passkey"); setError(null);
    try {
      const result = await auth.signInWithPasskey(email.trim());
      await handleSuccess(result.token);
    } catch (e: any) { setError(e?.message || "Passkey failed"); setLoading(null); }
  }
  ...
  async function handleEmail() {
    ...
    try {
      ...
      setStep("email-sent");
      setLoading(null);
    } catch (e: any) { setError(e?.message || "Failed to send"); setLoading(null); }
  }
  ...
  if (step === "email-sent") {
    return (
      <div className="space-y-4 text-center py-4">
        <p className="text-white">✉️ Magic link sent to <strong>{email}</strong></p>
        <p className="text-sm text-neutral-400">Check your inbox and click the link to sign in.</p>
        <button
          type="button"
          className="text-sm text-neutral-500 hover:text-white transition-colors"
          onClick={() => { setStep("idle"); setLoading(null); }}
        >
          ← Back to login
        </button>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {providers.passkey !== false && (
          ...
          >
            {loading === "passkey" ? <Spinner /> : "🔑"} Passkey
          </button>
        )}
        {providers.email !== false && (
          ...
          >
            {loading === "email" ? <Spinner /> : "✉️"} Magic Link
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {providers.discord && (
          <button type="button" onClick={() => handleOAuth("discord")} disabled={isLoading}
            className="flex items-center justify-center gap-2 rounded-lg bg-white/5 border border-white/10 text-white py-2.5 px-4 text-sm hover:bg-white/10 disabled:opacity-50 transition-colors">
