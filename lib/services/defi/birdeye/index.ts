...
export class BirdeyeService {
  ...
  static fromEnv(): BirdeyeService {
    ...
    return new BirdeyeService({
      apiKey,
      defaultChain:
        (process.env.BIRDEYE_DEFAULT_CHAIN as BirdeyeChain) ?? "solana",
  ...
  async getTokenPrice(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyePriceData>
    >("/defi/price", { address });
    if (!response.success)
  ...
  async getMultiPrice(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<Record<string, BirdeyePriceData>>
  ...
  async getTokenOverview(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyeTokenOverview>
  ...
  async getOHLCV(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<{ items: BirdeyeOHLCVItem[] }>
    >("/defi/ohlcv", {
      address,
  ...
  async getTokenTransactions(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyeTransactionListData>
    >("/defi/txs/token", {
      address,
  ...
  async getTrendingTokens(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<{ items: BirdeyeTrendingToken[] }>
    >("/defi/token_trending", {
      offset: options.offset ?? 0,
  ...
  async getWalletPortfolio(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyeWalletPortfolio>
  ...
  async getTokenSecurity(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyeTokenSecurity>
  ...
  async getTokenCreationInfo(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<BirdeyeTokenCreationInfo>
  ...
  async searchTokens(
    ...
    const response = await this.getClient(chain).get<
      BirdeyeResponse<{ items: BirdeyeTokenSearchResult[] }>
    >("/defi/v3/search", {
      keyword,
      offset: options.offset ?? 0,
      limit: options.limit ?? 20,
