...
async function cleanupFixtures() {
    if (userA) {
        await dbWrite.delete(userAffiliates).where(eq(userAffiliates.user_id, userA.id)).catch(() => { });
        await dbWrite.delete(affiliateCodes).where(eq(affiliateCodes.user_id, userA.id)).catch(() => { });
        await dbWrite.delete(users).where(eq(users.id, userA.id)).catch(() => { });
    }
    if (userB) {
        await dbWrite.delete(userAffiliates).where(eq(userAffiliates.user_id, userB.id)).catch(() => { });
        await dbWrite.delete(affiliateCodes).where(eq(affiliateCodes.user_id, userB.id)).catch(() => { });
        await dbWrite.delete(users).where(eq(users.id, userB.id)).catch(() => { });
    }
}
