...
<Tabs items={['cURL', 'Claude Code']}>
  <Tabs.Tab>
```bash
curl -X POST "https://cloud.milady.ai/api/v1/messages" \
...
```
  ...
  <Tabs.Tab>
```bash
export ANTHROPIC_BASE_URL="https://cloud.milady.ai/api/v1"
...
| ---------------- | --------------- | -------- | ----------------------------------------------------------- |
| `model`          | string          | yes      | Accepts Anthropic-style model ids such as `claude-sonnet-4` |
