 ...
 *
 * Data access for community moderation tables including token gates,
 * member wallets, moderation events, spam tracking, and blocked patterns.
 */
...
export const moderationEventsRepository = {
  ...
  async getStats(
    ...
    for (const event of events) {
      ...
      if (!event.resolved_at) unresolved++;
    }
...
export const blockedPatternsRepository = {
  ...
  async findByOrganization(
    ...
    // Filter by server (include org-wide patterns)
    if (serverId) {
  ...
  async setEnabled(id: string, enabled: boolean): Promise<OrgBlockedPattern | null> {
    return this.update(id, { enabled });
  },
};
