 ...
 * Milady cloud agents on behalf of token owners.
 *
 * Required env vars:
...
export function validateServiceKey(request: NextRequest): ServiceKeyIdentity | null {
  ...
  const orgId = process.env.WAIFU_SERVICE_ORG_ID;
  const userId = process.env.WAIFU_SERVICE_USER_ID;
 ...
 */
export function requireServiceKey(request: NextRequest): ServiceKeyIdentity {
