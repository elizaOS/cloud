...
async function routeRequest(
  ...
  if (!domain) {
    return htmlResponse(
      generateErrorPage(
        "Not Configured",
        `Domain ${hostname} is not connected to any service.`,
      ),
      404,
    );
  }
  if (!domain.custom_domain_verified) {
    return htmlResponse(
      generateErrorPage(
        ...
        `Domain ${hostname} is pending DNS verification.`,
      ),
      403,
    );
  }
  ...
  if (!result.success)
    return htmlResponse(result.error.html, result.error.status);
  return new NextResponse(result.data.html, {
    status: 200,
    headers: { ...result.data.headers, "X-Custom-Domain": hostname },
  });
}
