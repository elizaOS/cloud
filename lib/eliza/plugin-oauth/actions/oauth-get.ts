...
export const oauthGetAction: ActionWithParams = {
  ...
  parameters: {
    platform: {
      type: "string",
      description: "Platform to check: google, linear, slack, github, notion, jira. If not specified, checks all.",
      required: false,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "is my jira connected?" } },
      { name: "{{name2}}", content: { text: "Jira connected! Logged in as janedoe.", actions: ["OAUTH_GET"] } },
    ],
  ] as ActionExample[][],
};
