...
export async function POST(req: NextRequest) {
  ...
  console.log(
    `[Generate Field] Generated ${fieldName}: ${cleanedValue.slice(0, 50)}...`,
  );
...
function buildPromptForField(
  ...
  if (context.personality)
    contextSummary += `Personality: ${context.personality}\n`;
  ...
  switch (fieldName) {
    ...
    case "backstory":
      ...
      return `Write a brief, natural story (2-3 sentences) about how THE USER met ${context.name || "a person"}${
        context.personality
          ? `. ${context.name || "They"} is described as: ${context.personality}`
          : ""
