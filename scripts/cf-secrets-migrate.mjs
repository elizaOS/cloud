 ...
 *
 * Usage:
 *   node scripts/cf-secrets-migrate.mjs                       # uses .env.production
 *   node scripts/cf-secrets-migrate.mjs ./.env.staging        # explicit file
 *   node scripts/cf-secrets-migrate.mjs --env staging         # passes --env to wrangler
 *   node scripts/cf-secrets-migrate.mjs --env production --dry-run
 *   node scripts/cf-secrets-migrate.mjs --only KEY1,KEY2      # only push these keys
 *
...
/**
 * Spawn wrangler in the api/ working directory, piping the secret value via stdin.
 */
