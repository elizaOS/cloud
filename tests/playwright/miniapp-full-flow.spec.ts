...
test.describe("Miniapp E2E Tests", () => {
  ...
  test.describe("Anonymous Character Creation Flow", () => {
    test("anonymous user can create character", async ({ request }) => {
      test.skip(!miniappAvailable, "Miniapp not available");
  ...
  test.describe("Pass-Through Authentication Flow", () => {
    ...
    test("auth callback page handles session", async ({ page, request }) => {
      ...
      await page.goto(`${MINIAPP_URL}/auth/callback?session=${sessionId}`);
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Referral Code Application", () => {
    ...
    test("referral capture from URL parameter", async ({ page }) => {
      ...
      await page.goto(`${MINIAPP_URL}?ref=TEST-CODE`);
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Character Editing", () => {
    ...
    test("setup: create test character", async ({ request }) => {
      ...
      if (response.status() === 201) {
        ...
        console.log(`✅ Created test character: ${testAgentId}`);
      }
    });

    test("character details can be updated", async ({ request }) => {
      test.skip(!testAgentId, "Test character not created");
  ...
  test.describe("Full Chat Conversation", () => {
    ...
    test("setup: create test agent and chat", async ({ request }) => {
      ...
      if (agentResponse.status() === 201) {
        ...
        if (chatResponse.status() === 201) {
          ...
          testChatId = chat.id;
          console.log(`✅ Created test agent ${testAgentId} and chat ${testChatId}`);
        }
      }
    });

    test("can send text message", async ({ request }) => {
      test.skip(!testAgentId || !testChatId, "Test agent/chat not created");

      const response = await request.post(
        `${CLOUD_URL}/api/v1/miniapp/agents/${testAgentId}/chats/${testChatId}/messages`,
        {
          headers: authHeaders(),
          data: {
    ...
    test("chat history can be retrieved", async ({ request }) => {
      test.skip(!testAgentId, "Test agent not created");
    ...
    test("cleanup: delete test agent", async ({ request }) => {
      if (testAgentId) {
        await request.delete(
          `${CLOUD_URL}/api/v1/miniapp/agents/${testAgentId}`,
          {
  ...
  test.describe("Earn Credits via Sharing", () => {
    ...
    test("referral info includes share URL", async ({ request }) => {
      const response = await request.get(`${CLOUD_URL}/api/v1/miniapp/referral`, {
        headers: authHeaders(),
      });
