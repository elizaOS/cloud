...
import { CreateTriggerSchema, formatTrigger, ErrorResponses, N8N_BASE_URL } from "@/lib/n8n/schemas";
...
export async function GET(request: NextRequest) {
  ...
  if (!workflowId) {
    return NextResponse.json({ success: false, error: "workflowId parameter required" }, { status: 400 });
  }
  ...
  return NextResponse.json({ success: true, triggers: triggers.map((t) => formatTrigger(t)) });
}
...
export async function POST(request: NextRequest) {
  ...
  if (triggerType === "cron" && !config.cronExpression) {
    return NextResponse.json({ success: false, error: "cronExpression required for cron triggers" }, { status: 400 });
  }

  const trigger = await n8nWorkflowsService.createTrigger(workflowId, triggerType, triggerKey, config);

  const webhookUrl = triggerType === "webhook" ? `${N8N_BASE_URL}/api/v1/n8n/webhooks/${trigger.trigger_key}` : undefined;
  const webhookSecret = triggerType === "webhook" ? (trigger.config.webhookSecret as string) : undefined;
