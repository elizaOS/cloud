...
describe.skipIf(!api.hasApiKey())("Agent Publishing", () => {
  let agentId: string;
  ...
  test("GET /api/v1/agents/[id]/monetization returns settings", async () => {
    ...
    const response = await api.get(`/api/v1/agents/${agentId}/monetization`, {
      authenticated: true,
    });
    expect(response.status).toBe(200);
  ...
  test("DELETE /api/v1/agents/[id]/publish unpublishes agent", async () => {
    ...
    const response = await api.del(`/api/v1/agents/${agentId}/publish`, {
      authenticated: true,
    });
    expect(response.status).toBe(200);
...
// ── Error cases ─────────────────────────────────────────────────────
describe.skipIf(!api.hasApiKey())("Agent Publishing — Error Cases", () => {
