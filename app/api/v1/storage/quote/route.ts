...
import {
  ...
  StorageTier,
  StorageTierType,
} from "@/lib/services/storage-provider";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const replicationFactor = parseInt(
    searchParams.get("replicationFactor") || "1",
  );
  ...
  // Parse tier
  const tier = (StorageTier[tierParam as keyof typeof StorageTier] ??
  ...
  return NextResponse.json({
    ...
    humanReadable: {
      ...
      duration:
        tier === StorageTier.PERMANENT ? "Permanent" : `${durationDays} days`,
