...
function loadDeploymentAddresses(_chainId: number): DeploymentAddresses {
  ...
  let config: Record<string, Record<string, Record<string, string>>> = {
    networks: {},
  };
  ...
  return {
    identityRegistry:
      process.env.ERC8004_IDENTITY_REGISTRY || networkConfig.identity || "",
    reputationRegistry:
      process.env.ERC8004_REPUTATION_REGISTRY || networkConfig.reputation || "",
...
async function main() {
  ...
  if (!addresses.cloudReputationProvider) {
    ...
    logger.info(
      `Using existing CloudReputationProvider: ${addresses.cloudReputationProvider}\n`,
    );
  }
  ...
  const metadata: AgentMetadata = {
    name: "Cloud Services",
    description:
      "Decentralized AI inference and storage platform with x402 payments",
