 ...
 * Miniapp Runtime
 * 
 * This route serves miniapp content from Vercel Blob storage.
 * It handles:
...
import type { MiniappRuntimeConfig } from "@/db/schemas/miniapp-bundles";
...
function extractSubdomain(host: string | null): string | null {
  ...
  // Check if it's our miniapp domain
  if (hostname.endsWith(`.${MINIAPP_DOMAIN}`)) {
    return hostname.replace(`.${MINIAPP_DOMAIN}`, "");
  }
...
<script>
  ...
  window.elizaFetch = async function(endpoint, options = {}) {
    ...
    const headers = {
      ...
      'X-Miniapp-App-Id': window.__ELIZA_CLOUD__.appId,
      ...options.headers,
    };
  ...
  window.elizaStorage = {
    async get(collection, id) {
      const res = await elizaFetch('/api/v1/miniapp/storage/' + collection + '/' + id);
      return res.json();
    },
    ...
    async list(collection, query = {}) {
      ...
      const res = await elizaFetch('/api/v1/miniapp/storage/' + collection + '?' + params);
      return res.json();
    },
  };
  
  console.log('[Eliza Cloud] Miniapp runtime initialized', window.__ELIZA_CLOUD__);
</script>
${config.customHead || ""}
`;
...
export async function GET(
  ...
  if (!subdomain) {
    return NextResponse.json(
      { error: "Invalid miniapp URL" },
      { status: 400 }
    );
  }

  // Look up domain (subdomain or custom domain)
  let domain = await miniappDeployService.getDomainBySubdomain(subdomain);
  if (!domain) {
    domain = await miniappDeployService.getDomainByCustomDomain(subdomain);
  }
  ...
  // Get active bundle
  const bundle = await miniappDeployService.getActiveBundle(domain.app_id);
  if (!bundle) {
    return new NextResponse(
      generateNotFoundPage(subdomain),
      {
...
<body>
  <div class="container">
    <h1>Miniapp Not Found</h1>
    <p>The miniapp <span class="subdomain">${subdomain}</span> doesn't exist or hasn't been deployed yet.</p>
    <p><a href="https://elizacloud.ai/fragments">Create your own miniapp →</a></p>
  </div>
</body>
</html>`;
}
