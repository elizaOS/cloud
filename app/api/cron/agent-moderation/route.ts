...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const agents =
      await domainContentModerationService.getPublicAgentsForModeration(500);
    ...
    for (let i = 0; i < agents.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        agents.slice(i, i + BATCH_SIZE).map(async (agent) => {
          try {
            const result =
            ...
            if (result.status === "clean") {
              ...
              const domain = await managedDomainsRepository.findByAgentId(
                agent.id,
              );
              if (domain)
                await suspensionNotificationService.suspendAndNotify(
                  domain.id,
                  result.reasoning || "Agent violation",
                  result.flags,
                );

              logger.warn("[Cron] Agent suspended", {
                id: agent.id,
                name: agent.name,
              });
            } else {
            ...
            stats.errors++;
            logger.error("[Cron] Agent scan failed", {
              id: agent.id,
