...
export async function GET(
  ...
  const { documents, total } = await miniappStorageService.queryDocuments(
    app.id,
    collectionName,
    validation.data
  );
...
export async function POST(
  ...
  if ("documents" in body && Array.isArray(body.documents)) {
    ...
    const documents = await miniappStorageService.insertMany(
      app.id,
      collectionName,
    ...
    logger.info(`[Miniapp Storage API] Inserted ${documents.length} documents`, {
      appId: app.id,
    ...
    const document = await miniappStorageService.insertDocument(
      app.id,
      collectionName,
      docData,
      user.id
    ...
    logger.info(`[Miniapp Storage API] Inserted document`, {
      appId: app.id,
      collection: collectionName,
...
export async function DELETE(
  ...
  const collection = await miniappStorageService.getCollection(app.id, collectionName);
  if (!collection) {
    return NextResponse.json(
  ...
  await miniappStorageService.deleteCollection(app.id, collectionName);

  logger.info(`[Miniapp Storage API] Deleted collection: ${collectionName}`, {
    appId: app.id,
