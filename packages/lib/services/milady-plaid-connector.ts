...
export class MiladyPlaidConnectorError extends Error {
...
function requireConfig(): PlaidConfig {
  ...
  if (!config) {
    throw new MiladyPlaidConnectorError(
      503,
...
async function plaidPost<TResponse>(
  ...
  if (!response.ok) {
    ...
    throw new MiladyPlaidConnectorError(response.status, errorMessage);
  }
...
export async function createPlaidLinkToken(
  ...
  const data = await plaidPost<{
    ...
    client_name: request.clientName ?? "Milady",
