 ...
 *
 * A long-lived daemon that polls for pending `milady_provision` jobs and
 ...
 *   MILADY_SSH_KEY_PATH    - Path to SSH private key for Docker nodes
 *   MILADY_DOCKER_IMAGE    - Docker image to deploy (default: milady/agent:cloud-full-ui)
...
function getSSHKey(): Buffer {
  ...
  if (sshKeyEnv) {
    ...
    log("info", "SSH key loaded from MILADY_SSH_KEY env var");
    return _sshKey;
  }
  ...
  try {
    ...
    const { rows } = await client.query<JobRow>(
      `UPDATE jobs
       ...
       RETURNING id, type, status, data, attempts, max_attempts, webhook_url`,
      [limit],
    );
...
async function provisionAgent(
  ...
  if (sandbox.database_status !== "ready" || !dbUri) {
    ...
    try {
      ...
      await pool.query(
        `UPDATE milady_sandboxes SET database_status = 'provisioning', updated_at = NOW() WHERE id = $1`,
        [agentId],
      );

      const projectName = `milady-${sanitizeProjectName(sandbox.agent_name ?? "agent")}-${agentId.substring(0, 8)}`;
  ...
  const miladyApiToken = crypto.randomUUID();
  ...
  const allEnv: Record<string, string> = {
    ...
    MILADY_API_TOKEN: userEnvVars.MILADY_API_TOKEN || miladyApiToken,
    MILADY_ALLOWED_ORIGINS: `https://${agentId}.${AGENT_BASE_DOMAIN},https://${agentId}.shad0w.xyz`,
    DATABASE_URL: dbUri!,
  };
