...
import { milaidySandboxes, type MilaidySandboxStatus } from "@/db/schemas/milaidy-sandboxes";
...
export class DockerNodeManager {
  ...
  async syncAllocatedCounts(): Promise<Map<string, { before: number; after: number }>> {
    ...
    // Count active sandboxes per node from milaidy_sandboxes
    ...
    for (const node of nodes) {
      const [result] = await dbRead
        ...
        .where(
          and(
            eq(milaidySandboxes.node_id, node.node_id),
