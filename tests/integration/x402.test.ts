...
describe('x402 Facilitator Integration', () => {
  ...
  test('facilitator service caches results', async () => {
    ...
    const facilitator1 = await facilitatorService.getFacilitator('jeju-testnet');
    const time1 = Date.now() - start1;

    const start2 = Date.now();
    const facilitator2 = await facilitatorService.getFacilitator('jeju-testnet');
  ...
  test('facilitator service handles missing facilitator gracefully', async () => {
    const facilitator = await facilitatorService.getFacilitator('nonexistent-network');
  ...
  test('verify payment with invalid header returns false', async () => {
    ...
    const result = await facilitatorService.verify('invalid-header', requirement);
  ...
  test('settle payment with invalid header returns error', async () => {
    ...
    const result = await facilitatorService.settle('invalid-header', requirement);
    expect(result.success).toBe(false);
    expect(result.error).not.toBeNull();
  });

  test('discoverHttpFacilitator prefers Jeju facilitator', async () => {
    const facilitator = await discoverHttpFacilitator('jeju-testnet', { timeoutMs: 2000 });
