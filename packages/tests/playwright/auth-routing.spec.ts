...
test.describe("Authentication Routing", () => {
  test.describe("Login Page - returnTo Parameter", () => {
    test("login page URL should accept returnTo parameter", async ({ page }) => {
      await page.goto(`${BASE_URL}/login?returnTo=/dashboard/settings`);
      expect(page.url()).toContain("returnTo");
    });

    test("login page should render without errors when returnTo is provided", async ({ page }) => {
      await page.goto(`${BASE_URL}/login?returnTo=/dashboard/settings`);
    ...
    test("returnTo parameter should be URL-safe with encoded characters", async ({ page }) => {
    ...
    test("should reject potentially unsafe returnTo values", async ({ page }) => {
  ...
  test.describe("URL Parameter Validation", () => {
    test("returnTo should only accept relative paths", async ({ page }) => {
      ...
      for (const maliciousUrl of maliciousUrls) {
        await page.goto(`${BASE_URL}/login?returnTo=${encodeURIComponent(maliciousUrl)}`);
        await page.waitForLoadState("networkidle");
    ...
    test("returnTo should handle special characters safely", async ({ page }) => {
      const specialPaths = [
      ...
      for (const path of specialPaths) {
        await page.goto(`${BASE_URL}/login?returnTo=${encodeURIComponent(path)}`);
        await page.waitForLoadState("networkidle");
  ...
  test.describe("Page Response Codes", () => {
    ...
    test("login page with returnTo returns 200", async ({ request }) => {
      const response = await request.get(`${BASE_URL}/login?returnTo=/dashboard/settings`);
      expect(response.status()).toBe(200);
    });
...
test.describe("Console Error Monitoring", () => {
  ...
  test("dashboard should not have critical console errors", async ({ page }) => {
    const consoleErrors: string[] = [];
    page.on("console", (msg) => {
      if (msg.type() === "error") {
        consoleErrors.push(msg.text());
