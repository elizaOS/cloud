...
export const getCurrentUser = cache(async (): Promise<UserWithOrganization | null> => {
  try {
    ...
    const stewardCookie = cookieStore.get("steward-token");
    if (!stewardCookie) return null;

    const tokenHash = hashToken(stewardCookie.value);
    const cacheKey = CacheKeys.session.user(tokenHash);
    ...
    if (cachedUser) {
      ...
      if (cachedUser.organization_id) {
        void trackSessionActivity(cachedUser.id, cachedUser.organization_id, stewardCookie.value);
      }
      return cachedUser;
    }

    logger.debug("[AUTH] Verifying steward session cookie");
    const stewardClaims = await verifyStewardTokenCached(stewardCookie.value);
    ...
    if (user.organization_id) {
      void trackSessionActivity(user.id, user.organization_id, stewardCookie.value);
...
export async function requireAuthOrApiKey(request: NextRequest): Promise<AuthResult> {
  ...
  const cookieStore = await cookies();
  const authToken = cookieStore.get("steward-token");
