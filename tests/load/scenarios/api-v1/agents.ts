...
export function listAgents(): Agent[] {
  const body = httpGet<{ agents: Agent[] }>("/api/v1/app/agents", {
...
export function createAgent(): string | null {
  ...
  const body = httpPost<{ agent: { id: string } }>(
    "/api/v1/app/agents",
    generateAgentPayload(),
    {
      expectedStatus: 201,
...
export function getAgent(agentId: string): Agent | null {
  const body = httpGet<{ agent: Agent }>(`/api/v1/app/agents/${agentId}`, {
    tags: { endpoint: "agents" },
  });
...
export function deleteAgent(agentId: string): boolean {
  const deleted = httpDelete(`/api/v1/app/agents/${agentId}`, {
    tags: { endpoint: "agents" },
  });
...
export function agentReadOnly() {
  group("Agent Read", () => {
    ...
    if (agents.length > 0)
      getAgent(agents[Math.floor(Math.random() * agents.length)].id);
  });
  sleep(0.5);
}
