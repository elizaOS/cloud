...
const mockOAuth = {
  ...
  listConnections: mock(async () => [
    { id: "c1", status: "active", email: "user@test.com", scopes: ["gmail.send", "calendar.events"], linkedAt: "2026-01-01T00:00:00Z" },
  ]),
};
...
describe("Google MCP Tools", () => {
  beforeEach(() => {
    ...
    mockOAuth.getValidTokenByPlatform.mockImplementation(async () => ({ accessToken: "test-token" }));
  ...
  describe("Registration", () => {
    ...
    test("registers all expected tools", async () => {
      const { registerGoogleTools } = await import("@/app/api/mcp/tools/google");
      const names: string[] = [];
      registerGoogleTools({ registerTool: (n: string) => { names.push(n); } } as any);

      const expectedTools = [
        "google_status", "gmail_send", "gmail_list", "gmail_read",
        "calendar_list_events", "calendar_create_event", "calendar_update_event",
        "calendar_delete_event", "contacts_list",
      ];
  ...
  describe("gmail_send", () => {
    test("sends email successfully", async () => {
      ...
      const p = parse(await callTool("gmail_send", {
    ...
    test("handles CC and BCC recipients", async () => {
      ...
      await callTool("gmail_send", {
        to: "to@example.com", subject: "Test", body: "Body",
    ...
    test("sends HTML email with correct content type", async () => {
      ...
      await callTool("gmail_send", {
        to: "to@example.com", subject: "HTML", body: "<h1>Hello</h1>", isHtml: true,
      });
    ...
    test("returns messages with mapped fields", async () => {
      globalThis.fetch = mock(async (url: string) => {
        ...
        if (urlStr.includes("/messages/msg-1")) {
          return new Response(JSON.stringify({
            id: "msg-1", threadId: "t-1", snippet: "Hello...",
            labelIds: ["INBOX"], internalDate: "1708416000000",
            payload: { headers: [{ name: "From", value: "sender@test.com" }, { name: "Subject", value: "Hi" }] },
          }), { status: 200, headers: { "Content-Type": "application/json" } });
        }
  ...
  describe("gmail_read", () => {
    test("reads message with full content", async () => {
      mockFetchResponses.set("gmail.googleapis.com/gmail/v1/users/me/messages/msg-123", {
        ...
        body: {
          ...
          labelIds: ["INBOX", "UNREAD"], snippet: "Preview...",
        ...
        body: {
          ...
          payload: {
            ...
            parts: [
              ...
              { mimeType: "application/pdf", filename: "attachment.pdf", body: { attachmentId: "att-1" } },
            ],
          },
        },
      });
    ...
    test("handles missing payload gracefully (null safety)", async () => {
      mockFetchResponses.set("gmail.googleapis.com/gmail/v1/users/me/messages/msg-no-payload", {
        status: 200,
        body: {
          id: "msg-no-payload", threadId: "t-1",
          labelIds: ["INBOX"], snippet: "Preview",
        },
      });
  ...
  describe("calendar_list_events", () => {
    ...
    test("returns nextPageToken from response", async () => {
      mockFetchResponses.set("googleapis.com/calendar/v3/calendars", {
        status: 200,
        body: {
          items: [{ id: "evt-1", summary: "Meeting", start: { dateTime: "2026-02-20T10:00:00Z" }, end: { dateTime: "2026-02-20T11:00:00Z" } }],
    ...
    test("handles all-day events (date without time)", async () => {
      mockFetchResponses.set("googleapis.com/calendar/v3/calendars", {
        status: 200,
        body: {
          items: [{ id: "evt-1", summary: "Holiday", start: { date: "2026-02-20" }, end: { date: "2026-02-21" } }],
        },
      });
  ...
  describe("calendar_create_event", () => {
    test("creates event successfully", async () => {
      mockFetchResponses.set("googleapis.com/calendar/v3/calendars", {
        status: 200,
        body: { id: "new-evt-1", htmlLink: "https://calendar.google.com/event/1", status: "confirmed" },
      });

      const p = parse(await callTool("calendar_create_event", {
    ...
    test("creates event with attendees", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        ...
        return new Response(JSON.stringify({ id: "evt-1", htmlLink: "https://cal.google.com", status: "confirmed" }), {
          status: 200, headers: { "Content-Type": "application/json" },
        });
      }) as typeof fetch;

      await callTool("calendar_create_event", {
        summary: "Sync", start: "2026-02-20T14:00:00Z", end: "2026-02-20T15:00:00Z",
    ...
    test("includes optional fields when provided", async () => {
      ...
      await callTool("calendar_create_event", {
        summary: "Meeting", start: "2026-02-20T14:00:00Z", end: "2026-02-20T15:00:00Z",
        description: "Quarterly review", location: "Room A",
      });
  ...
  describe("calendar_update_event", () => {
    test("updates event by merging with existing data", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        ...
        capturedBody = JSON.parse(init?.body as string);
        return new Response(JSON.stringify({
          id: "evt-1", htmlLink: "https://cal.google.com", updated: "2026-02-20T16:00:00Z",
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;

      const p = parse(await callTool("calendar_update_event", {
  ...
  describe("Error handling", () => {
    test("googleFetch extracts API error message from response", async () => {
      mockFetchResponses.set("gmail.googleapis.com", {
        status: 403,
        body: { error: { message: "Insufficient Permission", code: 403, status: "PERMISSION_DENIED" } },
      });
    ...
    test("handles non-Error thrown objects", async () => {
      mockOAuth.getValidTokenByPlatform.mockImplementation(async () => { throw "string error"; });
      const r = await callTool("gmail_list");
      expect(r.isError).toBe(true);
      expect(parse(r).error).toContain("not connected");
    });
    ...
    test("handles network timeout", async () => {
      globalThis.fetch = mock(async () => { throw new Error("Network request failed"); }) as typeof fetch;
      const r = await callTool("gmail_list");
      expect(r.isError).toBe(true);
  ...
  describe("Concurrent request isolation", () => {
    test("handles concurrent requests with different orgs", async () => {
      ...
      mockOAuth.listConnections.mockImplementation(async ({ organizationId }) => {
        ...
        return [{ id: `conn-${organizationId}`, status: "active", email: `${organizationId}@test.com`, scopes: [] }];
      });
  ...
  describe("URL encoding", () => {
    test("gmail_read encodes messageId in URL", async () => {
      ...
      globalThis.fetch = mock(async (url: string) => {
        capturedUrl = url;
        return new Response(JSON.stringify({
          id: "msg+special/chars", threadId: "t-1",
          payload: { headers: [] },
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;
    ...
    test("gmail_list encodes messageIds in metadata fetch URLs", async () => {
      ...
      globalThis.fetch = mock(async (url: string) => {
        ...
        return new Response(JSON.stringify({
          id: "id+with/slash", threadId: "t-1", snippet: "Test",
          payload: { headers: [] },
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;
  ...
  describe("Boundary conditions", () => {
    test("gmail_list with maxResults=1 fetches exactly one message", async () => {
      ...
      globalThis.fetch = mock(async (url: string) => {
        ...
        return new Response(JSON.stringify({
          id: "msg-1", threadId: "t-1", snippet: "Only one",
    ...
    test("gmail_send plain text email has correct MIME structure", async () => {
      ...
      await callTool("gmail_send", {
        to: "test@example.com", subject: "Plain Test", body: "Hello plain",
      });
    ...
    test("calendar_create_event excludes optional fields when not provided", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        if (init?.method === "POST") capturedBody = JSON.parse(init.body as string);
        return new Response(JSON.stringify({ id: "evt-1", htmlLink: "link", status: "confirmed" }), {
          status: 200, headers: { "Content-Type": "application/json" },
        });
      }) as typeof fetch;

      await callTool("calendar_create_event", {
        summary: "Minimal", start: "2026-02-20T14:00:00Z", end: "2026-02-20T15:00:00Z",
      });
    ...
    test("calendar_update_event converts timed event start/end and clears date field", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        ...
        capturedBody = JSON.parse(init?.body as string);
        return new Response(JSON.stringify({
          id: "evt-allday", htmlLink: "link", updated: "2026-02-20T16:00:00Z",
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;
    ...
    test("mapGmailMessage handles missing internalDate", async () => {
      globalThis.fetch = mock(async (url: string) => {
        ...
        return new Response(JSON.stringify({
          id: "msg-nodate", threadId: "t-1", snippet: "No date",
          payload: { headers: [] },
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;
    ...
    test("mapCalendarEvent handles event with attendees and organizer", async () => {
      mockFetchResponses.set("googleapis.com/calendar/v3/calendars", {
        ...
        body: {
          items: [{
            id: "evt-full", summary: "Full Event",
            start: { dateTime: "2026-02-20T10:00:00Z" },
            end: { dateTime: "2026-02-20T11:00:00Z" },
            location: "Room B",
            status: "confirmed",
  ...
  describe("Thorough edge cases", () => {
    ...
    test("calendar_update_event can clear description by passing empty string", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        ...
        if (requestCount === 1) {
          return new Response(JSON.stringify({
            id: "evt-desc", summary: "Has Description",
        ...
        capturedBody = JSON.parse(init?.body as string);
        return new Response(JSON.stringify({
          id: "evt-desc", htmlLink: "link", updated: "2026-02-20T16:00:00Z",
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }) as typeof fetch;
    ...
    test("gmail_read falls back to text/html when text/plain is absent", async () => {
      mockFetchResponses.set("gmail.googleapis.com/gmail/v1/users/me/messages/msg-html-only", {
        status: 200,
        body: {
          id: "msg-html-only", threadId: "t-1",
          payload: {
            headers: [],
            parts: [
              { mimeType: "text/html", body: { data: Buffer.from("<p>HTML only</p>").toString("base64") } },
            ],
          },
        },
      });
    ...
    test("errMsg returns fallback when error is not an Error instance", async () => {
      mockOAuth.listConnections.mockImplementation(async () => { throw "just a string"; });
      const r = await callTool("google_status");
      expect(r.isError).toBe(true);
    ...
    test("errMsg returns fallback when error is null", async () => {
      mockOAuth.listConnections.mockImplementation(async () => { throw null; });
      const r = await callTool("google_status");
      expect(r.isError).toBe(true);
      expect(parse(r).error).toBe("Failed to check status");
    });
  ...
  describe("LARP: Zod schemas actually reject bad input", () => {
    test("captures and validates schemas from registerTool", async () => {
      ...
      expect(() => calCreateSchema.parse({
        summary: "Valid", start: "2026-02-20T10:00:00Z", end: "2026-02-20T11:00:00Z",
  ...
  describe("LARP: Unverified code paths", () => {
    test("extractBody returns empty string for attachment-only email (no text parts)", async () => {
      mockFetchResponses.set("gmail.googleapis.com/gmail/v1/users/me/messages/msg-attach-only", {
        status: 200,
        body: {
          id: "msg-attach-only", threadId: "t-1",
          payload: {
            mimeType: "multipart/mixed", headers: [{ name: "Subject", value: "Invoice" }],
            parts: [
              { mimeType: "application/pdf", filename: "invoice.pdf", body: { attachmentId: "att-1", size: 12345 } },
              { mimeType: "image/png", filename: "logo.png", body: { attachmentId: "att-2", size: 5000 } },
            ],
          },
        },
      });
  ...
  describe("Multi-tool flows", () => {
    test("list then read: message ID from list works in read", async () => {
      globalThis.fetch = mock(async (url: string) => {
        ...
        return new Response(JSON.stringify({
          id: "flow-msg-1", threadId: "flow-t-1", snippet: "Flow test",
    ...
    test("create then delete event: eventId from create works in delete", async () => {
      ...
      globalThis.fetch = mock(async (_url: string, init?: RequestInit) => {
        ...
        if (init?.method === "POST") {
          return new Response(JSON.stringify({
            id: "flow-evt-1", htmlLink: "https://cal.google.com", status: "confirmed",
          }), { status: 200, headers: { "Content-Type": "application/json" } });
        }
        if (init?.method === "DELETE") {
          return new Response("", { status: 204 });
      ...
      const createResult = parse(await callTool("calendar_create_event", {
