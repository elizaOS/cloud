...
import { MilaidyAgentActions } from "./agent-actions";
...
export default async function MilaidyAgentDetailPage({ params }: PageProps) {
  const user = await requireAuthWithOrg();
  const { id } = await params;

  const agent = await milaidySandboxService.getAgent(id, user.organization_id);
  if (!agent) {
