...
// =============================================================================
// App Builder API (auth required)
// =============================================================================

describe("App Builder API", () => {
  test("GET /api/v1/app-builder requires auth", async () => {
    const response = await api.get("/api/v1/app-builder");
    expect([401, 403]).toContain(response.status);
  });

  test("POST /api/v1/app-builder requires auth", async () => {
    const response = await api.post("/api/v1/app-builder", {});
    expect([401, 403]).toContain(response.status);
  });
});
