...
async function cleanupFixtures() {
    if (affiliateUser) {
        await dbWrite.delete(affiliateCodes).where(eq(affiliateCodes.user_id, affiliateUser.id)).catch(() => { });
        await dbWrite.delete(users).where(eq(users.id, affiliateUser.id)).catch(() => { });
    }
    if (callerUser) {
        await dbWrite.delete(users).where(eq(users.id, callerUser.id)).catch(() => { });
        await dbWrite.delete(organizations).where(eq(organizations.id, callerUser.organizationId)).catch(() => { });
    }
}

describe.skipIf(!process.env.DATABASE_URL)("AI Billing: Affiliate SKUs", () => {
    beforeAll(async () => {
    ...
    test("billUsage applies affiliate markup and credits the affiliate owner", async () => {
        ...
        const finalAffiliateEarnings = await dbRead.select({ balance: redeemableEarnings.available_balance })
            .from(redeemableEarnings).where(eq(redeemableEarnings.user_id, affiliateUser.id)).limit(1);
