...
export function httpGet<T>(path: string, options?: { public?: boolean; expectedStatus?: number; tags?: Record<string, string> }): T | null {
...
export function httpPost<T>(path: string, body: string | Record<string, unknown>, options?: { expectedStatus?: number; tags?: Record<string, string> }): T | null {
...
export function httpPatch<T>(path: string, body: Record<string, unknown>, options?: { tags?: Record<string, string> }): T | null {
...
export function httpDelete(path: string, options?: { tags?: Record<string, string> }): boolean {
  const res = http.del(`${baseUrl}${path}`, null, { headers, tags: options?.tags || {} });
  return checkStatus(res, (s) => s >= 200 && s < 300, `DELETE ${path}`);
}

export function httpPostFile<T>(path: string, file: { content: string; name: string; mimeType: string }, options?: { tags?: Record<string, string> }): T | null {
