...
function jsonRpcError(code: number, message: string, id: string | number | null, data?: Record<string, unknown>) {
...
function validateJsonRpcRequest(body: unknown): { valid: true; method: string; params: Record<string, unknown>; id: string | number | null } | { valid: false } {
  ...
  if (obj.params !== undefined && (typeof obj.params !== "object" || obj.params === null)) return { valid: false };
  if (obj.id !== null && typeof obj.id !== "string" && typeof obj.id !== "number") return { valid: false };
  return { valid: true, method: obj.method, params: (obj.params as Record<string, unknown>) || {}, id: obj.id as string | number | null };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const { isX402Configured, getDefaultNetwork, X402_RECIPIENT_ADDRESS, USDC_ADDRESSES, TOPUP_PRICE, CREDITS_PER_DOLLAR } = await import("@/lib/config/x402");
      if (isX402Configured()) {
        return NextResponse.json(
          jsonRpcError(A2AErrorCodes.AUTHENTICATION_REQUIRED, "Authentication required.", id, {
            x402: { topupEndpoint: "/api/v1/credits/topup", network: getDefaultNetwork(), asset: USDC_ADDRESSES[getDefaultNetwork()], payTo: X402_RECIPIENT_ADDRESS, minimumTopup: TOPUP_PRICE, creditsPerDollar: CREDITS_PER_DOLLAR },
          }),
          { status: 402 },
        );
      }
    ...
    const agentIdentifier = agentChainId && agentTokenId ? `${agentChainId}:${agentTokenId}` : `org:${authResult.user.organization_id}`;
    ...
    const { handleMessageSend, handleTasksGet, handleTasksCancel } = await import("@/lib/api/a2a/handlers");

    // Load secrets
    const a2aSecrets = isSecretsConfigured() ? await loadOrgSecrets(authResult.user.organization_id) : {};
    ...
    try {
      ...
      switch (method) {
        case "message/send":
          result = await handleMessageSend(params as Parameters<typeof handleMessageSend>[0], ctx);
          break;
        case "tasks/get":
          result = await handleTasksGet(params as Parameters<typeof handleTasksGet>[0], ctx);
          break;
        case "tasks/cancel":
          result = await handleTasksCancel(params as Parameters<typeof handleTasksCancel>[0], ctx);
          break;
        default:
          return a2aError(A2AErrorCodes.METHOD_NOT_FOUND, `Method not found: ${method}`, id, 404);
      }

      agentReputationService.recordRequest({ agentIdentifier, isSuccessful: true, method }).catch(() => {});
      ...
      if (msg.includes("Insufficient")) { code = A2AErrorCodes.INSUFFICIENT_CREDITS; status = 402; }
      else if (msg.includes("not found")) { code = A2AErrorCodes.TASK_NOT_FOUND; status = 404; }
      else if (msg.includes("suspended") || msg.includes("banned")) { code = A2AErrorCodes.AGENT_BANNED; status = 403; }

      agentReputationService.recordRequest({ agentIdentifier, isSuccessful: false, method }).catch(() => {});
      return a2aError(code, msg, id, status);
    }
  } catch (outerError) {
    logger.error("[A2A] Unhandled error:", outerError);
    return a2aError(A2AErrorCodes.INTERNAL_ERROR, outerError instanceof Error ? outerError.message : "Internal server error", id, 500);
  }
}
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    methods: [
      { name: "message/send", description: "Send a message to create/continue a task", isStandard: true },
      { name: "tasks/get", description: "Get task status and history", isStandard: true },
      { name: "tasks/cancel", description: "Cancel a running task", isStandard: true },
    ],
