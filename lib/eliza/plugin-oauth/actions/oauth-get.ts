...
export const oauthGetAction: ActionWithParams = {
  ...
  parameters: {
    platform: {
      type: "string",
      description: "Platform to check: google, linear, slack, github, notion, zoom. If not specified, checks all.",
      required: false,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "is my zoom connected?" } },
      { name: "{{name2}}", content: { text: "Zoom connected! Logged in as user@example.com.", actions: ["OAUTH_GET"] } },
    ],
  ] as ActionExample[][],
};
