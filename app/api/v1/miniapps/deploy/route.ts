 ...
 * Miniapp Deploy API
 * 
 * Deploy fragments as serverless miniapps with custom subdomains
 */
...
async function handleDeploy(request: NextRequest) {
  ...
  const validation = DeployMiniappSchema.safeParse(body);
  ...
  logger.info("[Miniapp Deploy API] Starting deployment", {
    userId: user.id,
  ...
  const result = await miniappDeployService.deploy({
    projectId,
    fragment,
    name,
    description,
  ...
  logger.info("[Miniapp Deploy API] Deployment complete", {
    appId: result.appId,
    url: result.url,
  });
