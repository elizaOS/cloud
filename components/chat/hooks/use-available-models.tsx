    ...
    async function fetchModels() {
      try {
        ...
        const filteredModels = (data.data || []).filter((model) =>
          ALLOWED_CHAT_MODELS.includes(model.id as typeof ALLOWED_CHAT_MODELS[number])
        );
        
        // If no models match from API, use fallback curated list
        if (filteredModels.length === 0) {
          console.warn("[useAvailableModels] No allowed models found in API response, using defaults");
          setModels(
