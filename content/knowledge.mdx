...
```bash
# Upload a document
curl -X POST "https://cloud.eliza.ai/api/v1/knowledge/upload-file" \
...
const response = await fetch('https://cloud.eliza.ai/api/v1/knowledge/upload-file', {
  method: 'POST',
  headers: {
...
with open('document.pdf', 'rb') as f:
    response = requests.post(
        'https://cloud.eliza.ai/api/v1/knowledge/upload-file',
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/knowledge/check?documentId=doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://cloud.eliza.ai/api/v1/knowledge/query" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/knowledge?agentId=agent_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/knowledge/doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X DELETE "https://cloud.eliza.ai/api/v1/knowledge/doc_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
const response = await fetch('https://cloud.eliza.ai/api/v1/chat/completions', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
...
```javascript
// 1. Query knowledge
const knowledge = await fetch('https://cloud.eliza.ai/api/v1/knowledge/query', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json',
...
| Operation | Cost |
|-----------|------|
| Document upload | 0.01 credits/MB |
| Embedding | 0.0001 credits/token |
| Query | 0.001 credits/query |
| Storage | 0.01 credits/GB/day |

## Best Practices

<Cards>
  <Cards.Card title="Quality Content">
    Use well-structured, accurate documents
  </Cards.Card>
  <Cards.Card title="Organize">
    Group related documents for better retrieval
  </Cards.Card>
  <Cards.Card title="Update Regularly">
    Keep knowledge current by refreshing documents
  </Cards.Card>
  <Cards.Card title="Test Queries">
    Verify retrieval quality before production
  </Cards.Card>
</Cards>
