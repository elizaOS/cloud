...
if [ -z "$CERTIFICATE_ARN" ]; then
  ...
  echo "   Create an ACM certificate for *.containers.cloud.milady.ai first:"
  echo "   aws acm request-certificate --domain-name '*.containers.cloud.milady.ai' --validation-method DNS --region $REGION"
  exit 1
fi
...
echo "   *.containers.cloud.milady.ai → CNAME → <ALB DNS>"
