...
export const tokenRedemptions = pgTable(
  "token_redemptions",
  {
    ...
    points_amount: numeric("points_amount", { precision: 12, scale: 2 })
      .notNull(),
    
    // USD value (points_amount / 100)
    usd_value: numeric("usd_value", { precision: 12, scale: 4 })
      .notNull(),
    
    // elizaOS token price at time of quote (USD per token)
    eliza_price_usd: numeric("eliza_price_usd", { precision: 18, scale: 8 })
      .notNull(),
    
    // Calculated elizaOS tokens to send
    eliza_amount: numeric("eliza_amount", { precision: 24, scale: 8 })
      .notNull(),
