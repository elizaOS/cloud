/**
 * POST /api/auth/app-session/[sessionId]/complete
 * Complete app authentication for a session
 *
  ...
  try {
    ...
    if (isNewUser && user.organization) {
      ...
      if (currentBalance === defaultBalance) {
        ...
        logger.info("[App Auth] Adjusted initial credits for app user", {
          userId: user.id,
    ...
    const result = await appAuthSessionsService.completeAuthentication(
      sessionId,
      user.id,
    ...
    logger.error("Error completing app authentication:", error);
