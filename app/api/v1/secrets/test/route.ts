...
const PROVIDER_CONFIGS: Partial<Record<SecretProvider, TestConfig>> = {
  ...
  elevenlabs: {
    ...
    extractMetadata: (d) => ({
      subscription: (d.subscription as { tier?: string })?.tier || "",
    }),
  },
  stripe: {
  ...
  telegram: {
    ...
    extractMetadata: (d) => ({
      username: (d.result as { username?: string })?.username || "",
    }),
  },
  github: {
...
async function testProvider(
  ...
  if (provider === "custom") {
    ...
    return {
      valid: res.ok,
      provider,
      message: res.ok ? "Request succeeded" : `HTTP ${res.status}`,
    };
  }
  ...
  const valid = config.isValid
    ? await Promise.resolve(config.isValid(res))
    : res.ok;
  if (valid) {
    ...
    return {
      ...
      provider,
      message: "Valid",
      metadata: config.extractMetadata?.(data),
    };
  }
  ...
  return {
    valid: false,
    provider,
    message: config.extractError?.(data) || `HTTP ${res.status}`,
  };
}
...
async function handler(request: NextRequest): Promise<Response> {
  try {
    ...
    if (provider === "custom" && !customTestUrl) {
      return NextResponse.json(
        { error: "customTestUrl required for custom provider" },
        { status: 400 },
      );
    }
    ...
    logger.error("[Secrets] Test failed", {
