...
export async function getServiceMethodCost(
  ...
  const pricingMap = await cache.getWithSWR<Record<string, string>>(
    cacheKey,
    CACHE_STALE_TIME,
    async () => {
      ...
      const fetchedPricingMap: Record<string, string> = {};
      for (const record of pricingRecords) {
        fetchedPricingMap[record.method] = record.cost;
      }

      return fetchedPricingMap;
    },
    CACHE_TTL,
  );
