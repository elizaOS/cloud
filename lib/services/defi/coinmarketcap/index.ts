...
export class CoinMarketCapService {
  ...
  async getQuotes(
    ...
    logger.info(
      `[CoinMarketCap] Getting quotes for ${ids.length} cryptocurrencies`,
    );
  ...
  async getQuotesBySymbol(
    ...
    logger.info(
      `[CoinMarketCap] Getting quotes for symbols: ${symbols.join(",")}`,
    );

    const response = await this.client.get<{
      data: Record<string, CMCCryptocurrency | CMCCryptocurrency[]>;
    }>("/cryptocurrency/quotes/latest", {
      symbol: symbols.join(","),
      convert,
    });
  ...
  async getCryptocurrencyInfo(
    ids: number[],
  ): Promise<Record<number, CMCCoinInfo>> {
    logger.info(
      `[CoinMarketCap] Getting info for ${ids.length} cryptocurrencies`,
    );
    const response = await this.client.get<{
      data: Record<string, CMCCoinInfo>;
