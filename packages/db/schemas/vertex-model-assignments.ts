...
export const vertexModelAssignments = pgTable(
  ...
  (table) => ({
    tuned_model_idx: index("vertex_model_assignments_tuned_model_idx").on(
      table.tuned_model_id,
    ),
    ...
    organization_idx: index("vertex_model_assignments_organization_idx").on(
      table.organization_id,
    ),
    user_idx: index("vertex_model_assignments_user_idx").on(table.user_id),
    active_idx: index("vertex_model_assignments_active_idx").on(
      table.is_active,
    ),
    global_slot_unique: uniqueIndex(
      "vertex_model_assignments_global_slot_active_idx",
    )
      .on(table.slot)
      .where(sql`${table.scope} = 'global' and ${table.is_active} = true`),
    organization_slot_unique: uniqueIndex(
      "vertex_model_assignments_org_slot_active_idx",
    )
      .on(table.organization_id, table.slot)
      .where(
        sql`${table.scope} = 'organization' and ${table.is_active} = true`,
      ),
    user_slot_unique: uniqueIndex(
      "vertex_model_assignments_user_slot_active_idx",
    )
...
export type VertexModelAssignmentRecord = InferSelectModel<
  typeof vertexModelAssignments
>;
export type NewVertexModelAssignmentRecord = InferInsertModel<
  typeof vertexModelAssignments
>;
