...
describe("Fragment Project Persistence", () => {
  ...
  test(
    "POST /api/v1/fragments/projects - creates project",
    async () => {
  ...
  test(
    "GET /api/v1/fragments/projects/:id - retrieves stored project",
    async () => {
  ...
  test(
    "GET /api/v1/fragments/projects - lists projects",
    async () => {
      if (!apiKeyValid) {
  ...
  test(
    "PUT /api/v1/fragments/projects/:id - updates project",
    async () => {
      if (!apiKeyValid || !createdProjectId) {
        console.log("⚠️ Skipping - no project created");
        return;
  ...
  test(
    "DELETE /api/v1/fragments/projects/:id - deletes project",
    async () => {
      if (!apiKeyValid || !createdProjectId) {
        console.log("⚠️ Skipping - no project created");
        return;
...
describe("Fragment Sandbox & Preview", () => {
  ...
  test(
    "POST /api/fragments/sandbox - creates sandbox with preview URL",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
        console.log("⚠️ Skipping - API key not valid");
        return;
  ...
  test(
    "GET /api/fragments/preview/:containerId - returns HTML content",
    async () => {
  ...
  test(
    "GET /api/fragments/preview/:invalidId - returns 404 for unknown container",
    async () => {
      if (!serverAvailable) {
...
describe("Fragment E2E Flow", () => {
  test(
    "Complete flow: create project → sandbox → preview → delete",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
      ...
      const sandboxResponse = await fetchWithAuth(
...
describe("Fragment Validation", () => {
  test(
    "POST /api/v1/fragments/projects - validates fragment schema",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
        console.log("⚠️ Skipping - server or API key not available");
        return;
  ...
  test(
    "POST /api/v1/fragments/projects - requires name",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
        console.log("⚠️ Skipping - server or API key not available");
        return;
  ...
  test(
    "POST /api/v1/fragments/projects - requires authentication",
    async () => {
      if (!serverAvailable) {
        console.log("⚠️ Skipping - server not available");
        return;
