...
class EndpointDiscoveryService {
  ...
  async discoverA2ASkills(): Promise<EndpointNode[]> {
    ...
    try {
      ...
      logger.error(
        "[EndpointDiscovery] Error discovering main MCP tools:",
        error,
      );
    }
  ...
  async discoverRESTEndpoints(): Promise<EndpointNode[]> {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `[EndpointDiscovery] API directory not found: ${apiDir}, using fallback`,
        );
            ...
            if (entry.isDirectory()) {
              ...
              try {
                ...
                if (methods.length > 0) {
                  ...
                  const description = self.generateEndpointDescription(
                    apiPath,
                    content,
                  );
                ...
                logger.debug(
                  `[EndpointDiscovery] Skipping ${fullPath}:`,
                  error,
                );
              }
            }
      ...
      if (nodes.length === 0) {
        logger.warn(
          "[EndpointDiscovery] No REST endpoints discovered dynamically, using fallback",
        );
        return this.getFallbackRESTEndpoints(baseUrl);
      }

      logger.info(
        `[EndpointDiscovery] Discovered ${nodes.length} REST endpoints dynamically`,
      );
    } catch (error) {
      logger.error(
        "[EndpointDiscovery] Error discovering REST endpoints:",
        error,
      );
  ...
  private getFallbackRESTEndpoints(baseUrl: string): EndpointNode[] {
    ...
    const restEndpoints: Array<{
      ...
      },
      {
        ...
        description: "Discover agents, MCPs, and services",
        category: "discovery",
        auth: "none",
      },
      {
      ...
      {
        path: "/api/v1/n8n/workflows/:id/test",
        method: "POST",
        name: "Test Workflow",
      ...
      {
        path: "/api/v1/n8n/workflows/:id/deploy",
        method: "POST",
      ...
      // DeFi endpoints
      {
        path: "/api/v1/defi/price",
        method: "GET",
        name: "Get Token Price",
        description:
          "Get token price from multiple sources (Birdeye, Jupiter, CoinGecko, CMC)",
        category: "defi",
        auth: "api_key",
      },
      {
        path: "/api/v1/defi/trending",
        method: "GET",
        name: "Get Trending Tokens",
        description: "Get trending tokens from various sources",
        category: "defi",
        auth: "api_key",
      },
      {
        path: "/api/v1/defi/market",
        method: "GET",
      ...
      {
        path: "/api/v1/defi/solana/token",
        method: "GET",
        name: "Solana Token Overview",
        description: "Get detailed Solana token overview from Birdeye",
        category: "defi",
        auth: "api_key",
      },
      {
        path: "/api/v1/defi/solana/wallet",
        method: "GET",
        name: "Solana Wallet Portfolio",
        description: "Get Solana wallet portfolio from Birdeye",
        category: "defi",
        auth: "api_key",
      },
      {
        path: "/api/v1/defi/jupiter/quote",
        method: "GET",
      ...
      {
        path: "/api/v1/defi/helius/transactions",
        method: "GET",
      ...
      {
        path: "/api/v1/defi/health",
        method: "GET",
        name: "DeFi Health",
        description: "Check health status of DeFi services",
        category: "defi",
        auth: "none",
      },
    ];
  ...
  private categorizePath(path: string): string {
    ...
    if (path.includes("/containers") || path.includes("/deploy"))
      return "infrastructure";
    if (path.includes("/knowledge") || path.includes("/query"))
      return "knowledge";
    if (path.includes("/n8n") || path.includes("/workflows"))
      return "workflows";
    if (
    ...
    if (
      ...
      return "discovery";
    if (path.includes("/fragments") || path.includes("/projects"))
      return "fragments";
    ...
    if (
      ...
      return "analytics";
    if (path.includes("/webhooks") || path.includes("/triggers"))
      return "webhooks";
    if (path.includes("/cron")) return "cron";
    if (
      path.includes("/defi") ||
      path.includes("/jupiter") ||
      path.includes("/helius") ||
      path.includes("/swap")
    )
      return "defi";
    return "utilities";
  ...
  private determineAuth(
    ...
    if (
      content.includes("requireAuthOrApiKey") ||
    ...
    if (query) {
      ...
      filtered = filtered.filter(
        (node) =>
          ...
          (node.metadata?.skillId as string)
            ?.toLowerCase()
            .includes(queryLower) ||
          (node.metadata?.toolName as string)
            ?.toLowerCase()
            .includes(queryLower),
      );
    }
  ...
  private categorizeSkill(skillId: string): string {
    ...
    if (skillId.includes("memory") || skillId.includes("conversation"))
      return "memory";
    if (skillId.includes("storage")) return "storage";
    if (skillId.includes("container")) return "infrastructure";
    if (skillId.includes("n8n") || skillId.includes("workflow"))
      return "workflows";
    if (skillId.includes("balance") || skillId.includes("usage"))
      return "billing";
    if (
      skillId.includes("defi") ||
      skillId.includes("token") ||
      skillId.includes("swap") ||
      skillId.includes("jupiter") ||
      skillId.includes("solana") ||
      skillId.includes("helius") ||
      skillId.includes("0x") ||
      skillId.includes("zeroex")
    )
      return "defi";
    return "utilities";
  }
  ...
  private categorizeTool(toolName: string): string {
    if (
      ...
      return "ai";
    if (toolName.includes("memory") || toolName.includes("conversation"))
      return "memory";
    if (toolName.includes("storage")) return "storage";
    if (toolName.includes("container")) return "infrastructure";
    if (toolName.includes("credit") || toolName.includes("balance"))
      return "billing";
    if (
      ...
      toolName.includes("helius") ||
      toolName.includes("0x") ||
      toolName.includes("trending") ||
      toolName.includes("ohlcv")
    )
      return "defi";
