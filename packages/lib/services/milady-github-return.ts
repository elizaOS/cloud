...
export interface LifeOpsGithubReturnDetail {
  ...
  bindingMode?: ManagedMiladyGithubMode | null;
  message?: string | null;
  restarted?: boolean;
}
...
function readMiladyDeepLinkPath(value: string): string | null {
  try {
...
export function resolveMiladyReturnUrl(value: string | null | undefined): string | null {
  ...
  const path = readMiladyDeepLinkPath(trimmed);
  if (!path) {
    return null;
  }
...
export function createLifeOpsGithubReturnResponse(args: {
  ...
  const deepLink = appendLifeOpsGithubDetailToReturnUrl(
    resolveMiladyReturnUrl(args.returnUrl),
    args.detail,
  );
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <div class="actions">
        ${deepLink ? `<a href="${escapeHtml(deepLink)}">Open Milady</a>` : ""}
