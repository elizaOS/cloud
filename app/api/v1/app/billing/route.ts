...
/**
 * OPTIONS /api/v1/app/billing
 * CORS preflight handler for app billing endpoint.
 *
  ...
  try {
    ...
    if (appId) {
      try {
        ...
        logger.error("[App API] Error getting app billing info", {
          appError,
          appId,
        });
    ...
    logger.error("[App API] Error getting billing info", { error });
