...
describe("Webhook Signature", () => {
  describe("Generation", () => {
    test("generates signature with timestamp and HMAC", async () => {
      const { generateWebhookSignature } = await import("@/lib/utils/webhook-signature");
  ...
  describe("Verification", () => {
    test("verifies valid signature", async () => {
      const { generateWebhookSignature, verifyWebhookSignature } = await import("@/lib/utils/webhook-signature");
    ...
    test("rejects tampered payload", async () => {
      ...
      const signature = generateWebhookSignature({ payload: originalPayload, secret });
      const result = verifyWebhookSignature({ payload: tamperedPayload, signature, secret });
    ...
    test("rejects wrong secret", async () => {
      ...
      const signature = generateWebhookSignature({ payload, secret: "correct-secret" });
      const result = verifyWebhookSignature({ payload, signature, secret: "wrong-secret" });
    ...
    test("rejects expired timestamp (default 5 min)", async () => {
      ...
      const signature = generateWebhookSignature({ payload, secret, timestamp: expiredTimestamp });
      const result = verifyWebhookSignature({ payload, signature, secret });
    ...
    test("accepts timestamp within tolerance", async () => {
      ...
      const signature = generateWebhookSignature({ payload, secret, timestamp: recentTimestamp });
    ...
    test("handles missing signature", async () => {
      const { verifyWebhookSignature } = await import("@/lib/utils/webhook-signature");
...
describe("Webhook Utilities", () => {
  test("createSignatureHeaders generates complete headers", async () => {
    const { createSignatureHeaders } = await import("@/lib/utils/webhook-signature");
  ...
  test("getSignatureFromHeaders extracts signature", async () => {
    const { getSignatureFromHeaders, generateWebhookSignature } = await import("@/lib/utils/webhook-signature");
  ...
  test("getSignatureFromHeaders returns null for missing header", async () => {
    const { getSignatureFromHeaders } = await import("@/lib/utils/webhook-signature");
  ...
  test("generateWebhookSecret creates secure random secret", async () => {
    const { generateWebhookSecret } = await import("@/lib/utils/webhook-signature");
...
describe("Webhook Request Handling", () => {
  ...
  test("parses JSON payload correctly", () => {
    const payloads = [
      ...
      { input: '{"nested":{"data":true}}', expected: { nested: { data: true } } },
...
describe("Webhook Security", () => {
  ...
  test("signature prevents replay after tolerance window", async () => {
    ...
    const oldSignature = generateWebhookSignature({ payload, secret, timestamp: oldTimestamp });
...
describe("WebhookTestClient", () => {
  test("constructs correct webhook URL", async () => {
    const { WebhookTestClient } = await import("@/lib/utils/webhook-test-client");
    ...
    expect(client.webhookUrl).toBe("https://api.example.com/api/v1/n8n/webhooks/my-webhook-key");
  });
