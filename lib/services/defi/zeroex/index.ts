...
export type ZeroExChain =
  ...
  | "base" | "avalanche" | "fantom" | "celo" | "linea" | "scroll" | "blast";

const CHAIN_IDS: Record<ZeroExChain, number> = {
  ethereum: 1, polygon: 137, bsc: 56, arbitrum: 42161, optimism: 10,
  base: 8453, avalanche: 43114, fantom: 250, celo: 42220, linea: 59144, scroll: 534352, blast: 81457,
};
...
export interface ZeroExQuoteResponse {
  ...
  orders: Array<{
    ...
    fill: { input: string; output: string; adjustedOutput: string; gas: number };
  }>;
...
export class ZeroExService {
  ...
  static fromEnv(): ZeroExService {
    ...
    if (!apiKey) throw new Error("ZEROEX_API_KEY environment variable is required");
    return new ZeroExService({ apiKey, defaultChain: (process.env.ZEROEX_DEFAULT_CHAIN as ZeroExChain) ?? "ethereum" });
  }
  ...
  private getClient(chain?: ZeroExChain): ZeroExClient {
    ...
    if (!client) {
      client = new ZeroExClient({ apiKey: this.config.apiKey, timeout: this.config.timeout }, targetChain);
  ...
  async getQuote(
    ...
    const routes: SwapRoute[] = response.sources
      ...
      .map((source) => ({ protocol: source.name, inputToken: params.sellToken, outputToken: params.buyToken, portion: parseFloat(source.proportion) }));

    return {
      inputToken: { address: response.sellTokenAddress, symbol: "", name: "", decimals: 18, chainId: effectiveChain as ChainId },
      outputToken: { address: response.buyTokenAddress, symbol: "", name: "", decimals: 18, chainId: effectiveChain as ChainId },
      ...
      fee: response.protocolFee !== "0" ? { amount: response.protocolFee, token: "ETH" } : undefined,
    };
  }

  async getRawQuote(
    params: { sellToken: string; buyToken: string; sellAmount?: string; buyAmount?: string; takerAddress: string; slippagePercentage?: number },
    chain?: ZeroExChain
  ...
  async getPrice(
    params: { sellToken: string; buyToken: string; sellAmount?: string; buyAmount?: string },
    chain?: ZeroExChain
  ...
  buildSwapTransaction(quote: ZeroExQuoteResponse): SwapTransaction {
    return { to: quote.to, data: quote.data, value: quote.value, gasLimit: quote.gas, chainId: quote.chainId };
  }
