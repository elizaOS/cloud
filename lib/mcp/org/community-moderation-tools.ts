...
const VerifyWalletSchema = z.object({
  ...
  signature: z
    .string()
    .describe("Base64-encoded signature of the challenge message"),
});
...
async function handleExecuteModeration(
  ...
  const event = await communityModerationService.events.logEvent({
    ...
    action_taken: params.action as OrgModerationAction,
  ...
  if (params.action === "timeout" && params.durationMinutes) {
    await communityModerationService.spam.applyRateLimit(
      ctx,
      params.durationMinutes,
    );
  }

  logger.info("[Moderation] Manual action", {
    eventId: event.id,
...
async function handleGetModerationHistory(
  ...
  let events = await communityModerationService.events.getRecentEvents(
    params.serverId,
    params.limit,
  );
  if (params.platformUserId)
    events = events.filter((e) => e.platform_user_id === params.platformUserId);
  if (params.platform)
...
async function handleGetUserViolations(
  ...
  const count = await communityModerationService.events.getViolationCount(
    params.serverId,
    params.platformUserId,
    params.platform,
  );
...
async function handleResolveEvent(
  ...
  return {
    success: true,
    message: params.falsePositive ? "Marked false positive" : "Resolved",
  };
}
...
async function handleCreateBlockedPattern(
  ...
  const [pattern] = await db
    ...
    .values({
      ...
      action: params.action as OrgModerationAction,
...
async function handleVerifyWallet(
  ...
  const result = await walletVerificationService.verifyAndLinkWallet(
    params.serverId,
    params.platformUserId,
    params.platform,
...
function handleGetVerificationChallenge(
...
export const communityModerationTools: MCPToolDefinition[] = [
  ...
  },
  {
    ...
    description: "Get recent moderation events",
  ...
  },
  {
    ...
    description: "List blocked patterns",
  ...
  },
  {
    ...
    description: "Delete blocked pattern",
  ...
  },
  {
    ...
    description: "Get settings UI link",
  ...
  },
  {
    name: "verify_wallet",
    description:
      "Verify wallet ownership via signature and link to user. Requires prior challenge from get_verification_challenge.",
  ...
  },
  {
    ...
    description: "Generate wallet verification challenge",
