/**
 * Chat Playground Workflow
 * 
 * Simple, fast conversation mode for playground environments.
 * Uses a single-shot LLM call without planning or action execution.
 * 
 * Flow:
 * 1. Compose basic state with memory providers
 * 2. Single LLM call to generate response
 * 3. Return response immediately
 * 4. Run evaluators in background
 */
...
import { runEvaluatorsWithTimeout } from "../utils/helpers";
import type { ParsedResponse } from "../utils/parsers";

/**
 * Workflow parameters
 */
export interface ChatPlaygroundParams {
 ...
 */
export async function handleChatPlaygroundWorkflow({
  runtime,
  message,
  callback,
}: ChatPlaygroundParams): Promise<void> {
  const responseId = v4();
