...
async function handlePOST(request: NextRequest, context: RouteContext) {
  ...
  if (!session.capabilities.hasGit)
    return NextResponse.json({ error: "Git not enabled" }, { status: 400 });

  const ops = {
    clone: () =>
      codeAgentService.gitClone({
        sessionId,
        ...(body as {
          url: string;
          branch?: string;
    ...
    push: () =>
      codeAgentService.gitPush({
        sessionId,
        ...(body as { remote: string; branch?: string; force: boolean }),
      }),
    pull: () =>
      codeAgentService.gitPull({
        sessionId,
        ...(body as { remote: string; branch?: string }),
      }),
    status: () =>
      Promise.resolve({
        success: true,
