...
async function getSalesforceMcpHandler() {
  ...
  async function resolveInstanceUrl(
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to resolve Salesforce instance URL: ${res.status}`,
      );
    }
  ...
  mcpHandler = createMcpHandler(
    (server) => {
      ...
      server.tool(
        "salesforce_status",
        "Check Salesforce OAuth connection status",
        {},
        async () => {
          try {
      ...
      server.tool(
        ...
        async ({ nextRecordsUrl }) => {
          try {
            ...
            return errorResult(
              e instanceof Error ? e.message : "Failed to fetch next page",
            );
          }
        },
      );
      ...
      server.tool(
        ...
        async () => {
          try {
            ...
            const objects = data.sobjects?.map(
            ...
            return errorResult(
              e instanceof Error ? e.message : "Failed to list objects",
            );
          }
        },
      );
      ...
      server.tool(
        ...
        async ({ objectName }) => {
          try {
            ...
            const fields = data.fields?.map((f: Record<string, unknown>) => ({
              ...
              picklistValues:
                (f.picklistValues as Array<Record<string, unknown>>)?.length > 0
                  ...
                  : undefined,
              referenceTo:
                (f.referenceTo as string[])?.length > 0
                  ? f.referenceTo
                  : undefined,
            }));
            return jsonResult({
            ...
            return errorResult(
              e instanceof Error ? e.message : "Failed to describe object",
            );
          }
        },
      );
      ...
      server.tool(
        ...
        async ({ objectName, limit = 10 }) => {
          try {
            ...
            return errorResult(
              e instanceof Error ? e.message : "Failed to get recent records",
            );
          }
        },
      );
    ...
    { capabilities: { tools: {} } },
    {
      streamableHttpEndpoint: "/api/mcps/salesforce/streamable-http",
      disableSse: true,
      maxDuration: 60,
    },
  );
...
async function handleRequest(req: NextRequest): Promise<Response> {
  try {
    ...
    logger.error("[SalesforceMCP]", error);
