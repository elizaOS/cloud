...
export const oauthGetAction: ActionWithParams = {
  ...
  similes: [
    ...
    "CHECK_SALESFORCE", "VERIFY_SALESFORCE", "CHECK_AIRTABLE", "VERIFY_AIRTABLE",
  ],
  description:
  ...
  parameters: {
    platform: {
      type: "string",
      description: "Platform to check: google, linear, slack, github, notion, salesforce, airtable. If not specified, checks all.",
      required: false,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "am I connected to salesforce?" } },
      { name: "{{name2}}", content: { text: "Salesforce connected! Logged in as user@example.com.", actions: ["OAUTH_GET"] } },
    ],
    [
      { name: "{{name1}}", content: { text: "check my airtable connection" } },
      { name: "{{name2}}", content: { text: "Airtable connected! Logged in as user@example.com.", actions: ["OAUTH_GET"] } },
    ],
  ] as ActionExample[][],
};
