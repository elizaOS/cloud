...
test.describe("Chat Interface", () => {
  ...
  test("send button responds to input", async ({ page }) => {
    ...
    // Find send button
    const sendButton = page.locator('button[type="submit"], button:has(svg)').filter({ hasText: /send/i }).first();
    const submitButton = page.locator('form button[type="submit"]').first();
    
    // Check for any interactive submit element
    const hasSubmit = await sendButton.isVisible().catch(() => false) ||
                       await submitButton.isVisible().catch(() => false);
    
    console.log(`✅ Send mechanism available: ${hasSubmit ? "Yes" : "Form-based"}`);
  });
  ...
  test("send a message and wait for response", async ({ page }) => {
    ...
    const messagesArea = page.locator('[class*="message"], [role="log"], [class*="chat"]');
  ...
  test("character/agent selection is available", async ({ page }) => {
    ...
    const characterSelector = page.locator('select, [role="combobox"], button:has-text("Select"), button:has-text("Character")');
    const selectorCount = await characterSelector.count();
    
    if (selectorCount > 0) {
      console.log(`✅ Character selection available with ${selectorCount} selector(s)`);
    } else {
      // Check for character cards or list
      const characterItems = page.locator('[class*="character"], [class*="agent"]');
  ...
  test("voice/audio controls are present", async ({ page }) => {
    ...
    const audioButton = page.locator('button:has(svg), [aria-label*="voice" i], [aria-label*="audio" i], [aria-label*="mic" i]');
    const hasAudioControls = await audioButton.count() > 0;
    
    console.log(`✅ Audio controls present: ${hasAudioControls ? "Yes" : "No"}`);
  });
});
...
test.describe("Agent Gallery/Marketplace", () => {
  ...
  test("can click on an agent card", async ({ page }) => {
    ...
    const clickableCard = page.locator('a[href*="character"], button:has-text("View"), button:has-text("Chat")').first();
  ...
  test("agent cards have avatars", async ({ page }) => {
    const avatars = page.locator('img[alt*="avatar" i], img[class*="avatar" i], img[class*="character" i], [class*="avatar"]');
...
test.describe("Character Creator (Auth Required)", () => {
  ...
  test("character creator page handles auth", async ({ page }) => {
    ...
    if (url.includes("/login")) {
      console.log("✅ Character creator redirects unauthenticated users to login");
    } else {
      // Check for creator form elements
      const formElements = page.locator('input, textarea, button');
      const elementCount = await formElements.count();
      console.log(`✅ Character creator shows form with ${elementCount} interactive elements`);
    }
  });
  ...
  test("character creator has form fields", async ({ page }) => {
    ...
    if (!url.includes("/login")) {
      ...
      const bioTextarea = page.locator('textarea[name="bio"], textarea[placeholder*="bio" i]');
      
      const hasName = await nameInput.isVisible().catch(() => false);
      const hasBio = await bioTextarea.isVisible().catch(() => false);
      
      console.log(`✅ Character form - Name field: ${hasName}, Bio field: ${hasBio}`);
    } else {
...
test.describe("My Agents (Auth Required)", () => {
  ...
  test("my agents page handles auth", async ({ page }) => {
    ...
    if (url.includes("/login")) {
      ...
      console.log(`✅ My Agents page loaded with ${content?.length} characters of content`);
    }
  });
  ...
  test("create agent button available if authenticated", async ({ page }) => {
    ...
    if (!url.includes("/login")) {
      const createButton = page.locator('button:has-text("Create"), button:has-text("New Agent"), a:has-text("Create")');
...
test.describe("Image Generation", () => {
  ...
  test("image generation page handles auth", async ({ page }) => {
    ...
    if (url.includes("/login")) {
      console.log("✅ Image generation redirects unauthenticated users to login");
    } else {
      const content = await page.locator("body").textContent();
  ...
  test("prompt input is available if authenticated", async ({ page }) => {
    ...
    if (!url.includes("/login")) {
      ...
      const promptInput = page.locator('textarea[placeholder*="prompt" i], input[placeholder*="prompt" i], textarea').first();
  ...
  test("image settings/sliders are present if authenticated", async ({ page }) => {
    const url = page.url();
...
test.describe("Video Generation", () => {
  ...
  test("video generation page handles auth", async ({ page }) => {
    ...
    if (url.includes("/login")) {
      console.log("✅ Video generation redirects unauthenticated users to login");
    } else {
      const content = await page.locator("body").textContent();
  ...
  test("model selection dropdown is present if authenticated", async ({ page }) => {
    const url = page.url();
    
    if (!url.includes("/login")) {
      const modelSelect = page.locator('select, [role="combobox"], button:has-text("Model")');
...
test.describe("Voice Features", () => {
  ...
  test("voice list or creation options are present if authenticated", async ({ page }) => {
    const url = page.url();
    
    if (!url.includes("/login")) {
      const voiceItems = page.locator('[class*="voice"], button:has-text("Create"), button:has-text("Clone")');
...
test.describe("Chat Message Sending", () => {
  test("attempt to send a chat message", async ({ page }) => {
    ...
    if (await chatInput.isVisible({ timeout: 10000 }).catch(() => false)) {
      // Type a message
      const testMessage = "Hello! This is a test message to verify the chat works.";
      ...
      if (inputAfterSend === "") {
        ...
        const sendButton = page.locator('button:has(svg[class*="send" i]), form button[type="submit"]').first();
      ...
      console.log("ℹ️ Chat input not visible - may need character selection first");
    }
  });
});
