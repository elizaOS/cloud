 ...
 *
 * The real package does forbidden top-level I/O (logger init at module load)
 * which Workers reject during validation. Agent runtime lives on the Node
 * sidecar (`services/agent-server`); the Worker only needs the *shape* of
 * common exports so transitive imports don't crash the bundle.
 *
 * - Enums (ContentType, EventType, ChannelType) get string-valued maps.
 * - `logger` is a no-op so any incidental call is harmless.
 * - `UUID` is exported as both a type and a no-op factory (some consumers use
 *   it as a value).
 * - All other named exports fall through a Proxy that returns a no-op function
 *   so unknown imports don't ReferenceError at module init.
 */
...
// Additional names imported across packages/lib — all no-op on Workers.
...
// Class-like stubs (extended/instantiated by some plugins at type level).
...
// --- Type-only re-exports (erased at compile time) ---
...
// --- Default export + namespace fallback ---
const target = {
  logger,
  ContentType,
  EventType,
...
const proxy = new Proxy(target, {
  get(t, prop) {
    ...
    if (typeof prop === "string" && /^[A-Z]/.test(prop)) return prop; // enum-like
    return noop;
  },
});
