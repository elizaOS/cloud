...
export const oauthConnectAction: ActionWithParams = {
  ...
  similes: [
    ...
    "CONNECT_LINKEDIN", "LINK_LINKEDIN",
    "ADD_INTEGRATION", "SETUP_CONNECTION", "LINK_GOOGLE", "AUTHENTICATE",
    "LINK_LINEAR", "LINK_SLACK", "LINK_GITHUB", "LINK_NOTION",
  ],
  description:
    "Connect an OAuth platform for the user. Returns an authorization URL. After user completes OAuth in browser, they should say 'done' to verify the connection. Available: google, linear, slack, github, notion, linkedin",

  parameters: {
    platform: {
      type: "string",
      description: "Platform to connect. Available: google, linear, slack, github, notion, linkedin",
      required: true,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "connect linkedin" } },
      { name: "{{name2}}", content: { text: "Connect LinkedIn: https://www.linkedin.com/...", actions: ["OAUTH_CONNECT"] } },
    ],
  ] as ActionExample[][],
};
