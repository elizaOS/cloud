  ...
  describe("Dead Pod Detection", () => {
    it("should detect pods with stale heartbeats", () => {
      ...
      const deadPods = pods.filter(
        (p) => Date.now() - p.lastHeartbeat > DEAD_POD_THRESHOLD_MS,
      );
  ...
  describe("Failover API Integration", () => {
    ...
    it("should handle failover API failure gracefully", async () => {
      ...
      const response = await fetch(
        "http://test/api/internal/discord/gateway/failover",
        {
          method: "POST",
...
describe("Failover Timing", () => {
  it("should use configurable threshold", () => {
    ...
    const customThreshold = parseInt(
      process.env.DEAD_POD_THRESHOLD_MS ?? String(defaultThreshold),
      10,
    );
  ...
  it("should use configurable check interval", () => {
    ...
    const customInterval = parseInt(
      process.env.FAILOVER_CHECK_INTERVAL_MS ?? String(defaultInterval),
      10,
    );
