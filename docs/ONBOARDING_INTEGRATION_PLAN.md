# Milady ↔ Eliza Cloud: Onboarding Integration Plan
...
**Goal**: Make "Host on Eliza Cloud" the default/recommended runtime option during milady's first-run onboarding, while keeping local mode as a fully-supported fallback.

**Current state**: The milady CLI (`milady start`) runs `runFirstTimeSetup()` in `src/runtime/eliza.ts` which walks the user through: name → personality → AI provider → wallets → GitHub. Cloud integration exists (`src/cloud/`) but is a separate, opt-in path — never surfaced during onboarding.

**Target state**: After the name + personality steps, the user sees a new "How should I run?" prompt where "☁️ Eliza Cloud (recommended)" is the default. Choosing it triggers browser-based auth → agent provisioning → thin client connection — all within the same `milady start` flow.
...
| Surface | Entry | Code |
|---------|-------|------|
| CLI | `milady start` (first run, no agent name in config) | `src/runtime/eliza.ts` → `runFirstTimeSetup()` (line ~3055) |
| CLI | `milady setup` (explicit) | `src/cli/program/register.setup.ts` → `registerSetupCommand()` |
...
Step 8: Persist config
  ...
  └─ Saves to ~/.milady/milady.json
```

### `milady setup` Command
...
### Config Structure (`~/.milady/milady.json`)
...
### Cloud Config Type (`src/config/types.milady.ts`)
...
**Key insight**: The cloud infrastructure in the milady repo is already built. The gap is purely in the **onboarding flow** — cloud is never presented as an option during setup.
...
POST /api/compat/agents/[id]/launch
  ...
  Note: launchManagedMiladyAgent not yet implemented (placeholder)
```
...
### Bridge Client API (milady → cloud agent)

The `ElizaCloudClient` in milady's `src/cloud/bridge-client.ts` calls these internal V1 endpoints:

```
GET    /api/v1/milady/agents           — list agents
POST   /api/v1/milady/agents           — create agent
GET    /api/v1/milady/agents/:id       — get agent
DELETE /api/v1/milady/agents/:id       — delete agent
POST   /api/v1/milady/agents/:id/provision  — provision sandbox
POST   /api/v1/milady/agents/:id/bridge     — JSON-RPC message relay
POST   /api/v1/milady/agents/:id/stream     — SSE streaming message relay
POST   /api/v1/milady/agents/:id/snapshot   — create backup
GET    /api/v1/milady/agents/:id/backups    — list backups
POST   /api/v1/milady/agents/:id/restore    — restore from backup
```

**Note**: The bridge-client uses `/api/v1/milady/agents/` paths, but the compat API is at `/api/compat/agents/`. Either the bridge-client needs to switch to compat routes, or the V1 routes need to be verified as equivalent. **Recommendation**: Use the compat routes for the thin client since they're the official external API.
...
```
User                   milady CLI              Eliza Cloud            Browser
 │                        │                        │                     │
 │  picks "Eliza Cloud"   │                        │                     │
 │───────────────────────>│                        │                     │
 │                        │  POST /api/auth/       │                     │
...
```jsonc
// Added to ~/.milady/milady.json → cloud section
{
  "cloud": {
...
Once connected, the milady CLI/TUI acts as a thin client:
...
4. **Config persistence**: Same `saveMiladyConfig()` path — GUI writes to `~/.milady/milady.json`
...
Scenario 2: No capacity
  ...
  → Can switch to cloud later via `milady cloud connect`
...
milady cloud login    # auth with Eliza Cloud
milady cloud connect  # provision + connect agent

# Switch from cloud to local
milady config set cloud.enabled false
milady start  # starts in local mode
...
### In `milady-ai/milady` (the milady repo)
...
#### 10.5 Modified: `src/config/types.milady.ts` — `CloudConfig`
...
**What**: Add cloud option to the `milady setup` non-interactive path.
...
**What**: New `milady cloud` subcommand group.

**Commands**:
```
milady cloud login   — authenticate with Eliza Cloud (runs cloudLogin())
milady cloud status  — show connection status, agent info, capacity
milady cloud connect — provision a new cloud agent or reconnect to existing
milady cloud logout  — clear stored API key, disable cloud mode
```
...
The `ElizaCloudClient` currently uses `/api/v1/milady/agents/` paths. For the thin client use case via compat API, either:
...
| 10.5 | `src/config/types.milady.ts` | Modify (minor) | P0 — core |
...
**Status**: Route exists but `launchManagedMiladyAgent` is not implemented.
...
The milady `ElizaCloudClient` uses `/api/v1/milady/agents/` paths. Ensure these are either:
...
When milady creates a cloud agent via `POST /api/compat/agents`, the `agentConfig` should include the full character definition (bio, system, style, etc.) so the cloud agent runs with the chosen personality.

**Current state**: `agentConfig` is stored as JSON in `milady_sandboxes.agent_config`. The cloud provisioning worker passes it to the container.

**Verify**: That the character config in `agentConfig` is correctly applied when the container starts. The cloud agent's `milady.json` should include the personality data.
...
```
User              milady CLI          Eliza Cloud         Browser         Docker Node
 │                   │                    │                  │                │
 │  milady start     │                    │                  │                │
 │──────────────────>│                    │                  │                │
 │                   │                    │                  │                │
...
1. **V1 vs Compat routes**: Should `ElizaCloudClient` switch to `/api/compat/agents/` or keep `/api/v1/milady/agents/`? Need to verify both are equivalent or if compat has features V1 lacks.

2. **Character config propagation**: When `agentConfig` is passed to `POST /api/compat/agents`, does the provisioning worker correctly inject it into the container's `milady.json`? Need to trace through `miladySandboxService.createAgent()` → provisioning worker → container startup.

3. **`launchManagedMiladyAgent`**: This is referenced in the launch route but not implemented. Is it needed for onboarding, or can we use create + poll + connect?
...
5. **Desktop app onboarding**: The GUI web UI handles onboarding separately. Who implements the cloud option in the GUI — the milady team or the Eliza Cloud team? Need coordination.
...
9. **Offline detection**: If the user starts `milady start` without internet and cloud is configured, should it fail gracefully or fall back to local?

---

## Appendix: File Reference

### Milady Repo (`milady-ai/milady`)

| Path | Purpose |
|------|---------|
| `src/runtime/eliza.ts` | Main runtime entry, `runFirstTimeSetup()`, `startEliza()` |
| `src/cli/program/register.setup.ts` | `milady setup` command |
| `src/cli/program/register.start.ts` | `milady start` command (calls `startEliza()`) |
| `src/cli/program/register.config.ts` | `milady config` command |
