...
export function queryKnowledge(query: string, limit = 5): unknown[] {
  ...
  const r = callMcpTool<{ results: unknown[] }>("query_knowledge", {
    query,
    limit,
  });
...
export function uploadKnowledge(content: string, title: string): string | null {
  const r = callMcpTool<{ knowledgeId?: string; id?: string }>(
    "upload_knowledge",
    { content, title },
  );
...
export function saveMemory(
  ...
  const r = callMcpTool<{ memoryId?: string; id?: string }>("save_memory", {
    content,
    tags,
  });
...
export function retrieveMemories(query: string, limit = 5): unknown[] {
  return (
    callMcpTool<{ memories: unknown[] }>("retrieve_memories", { query, limit })
      ?.memories ?? []
  );
}
...
export function knowledgeOperationsCycle() {
  group("Knowledge Ops", () => {
    ...
    sleep(0.5);
    if (!config.safeMode)
      uploadKnowledge(generateKnowledgeContent(), "Load Test");
  });
  sleep(1);
}
