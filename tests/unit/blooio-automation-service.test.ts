...
describe("BlooioAutomationService", () => {
  ...
  describe("invalidateStatusCache", () => {
    ...
    it("handles multiple invalidations", () => {
      expect(() => {
        blooioAutomationService.invalidateStatusCache(testOrgId);
        blooioAutomationService.invalidateStatusCache(testOrgId);
        blooioAutomationService.invalidateStatusCache("33333333-3333-3333-3333-333333333333");
      }).not.toThrow();
    });
  });
  ...
  describe("Error Handling", () => {
    it("handles empty organization ID gracefully", async () => {
      const status = await blooioAutomationService.getConnectionStatus("00000000-0000-0000-0000-000000000000");
      expect(status).toHaveProperty("connected");
    });

    it("handles special characters in organization ID", async () => {
      const status = await blooioAutomationService.getConnectionStatus("00000000-0000-0000-0000-000000000001");
      expect(status).toHaveProperty("connected");
    });
  });
  ...
  describe("Credential Methods", () => {
    describe("getApiKey", () => {
      it("returns null when no API key stored", async () => {
        ...
        const apiKey = await blooioAutomationService.getApiKey("55555555-5555-5555-5555-555555555555");
    ...
    describe("getWebhookSecret", () => {
      it("returns null when no webhook secret stored", async () => {
        const secret = await blooioAutomationService.getWebhookSecret("55555555-5555-5555-5555-555555555555");
    ...
    describe("getFromNumber", () => {
      it("returns null when no from number stored", async () => {
        const fromNumber = await blooioAutomationService.getFromNumber("55555555-5555-5555-5555-555555555555");
...
describe("BlooioAutomationService Integration-style Tests", () => {
  ...
  const integrationOrgId = "66666666-6666-6666-6666-666666666666";
  const integrationUserId = "77777777-7777-7777-7777-777777777777";
