/**
 * Milady → PayPal connector (OAuth 2.0 + Reporting API).
 *
...
export class MiladyPaypalConnectorError extends Error {
...
async function paypalTokenRequest(
  ...
  if (!response.ok) {
    ...
    throw new MiladyPaypalConnectorError(response.status, message);
  }
