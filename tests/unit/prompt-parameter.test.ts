...
describe("URL Parameter Building", () => {
  ...
  test("special characters are properly encoded", () => {
    const url = buildPromptUrl("/dashboard/build", "agent with $100 budget & features?");
...
describe("URL Parameter Parsing", () => {
  test("extracts prompt from valid URL", () => {
    const prompt = parsePromptFromUrl("/dashboard/build?prompt=create%20an%20agent");
  ...
  test("handles multiple parameters", () => {
    const prompt = parsePromptFromUrl("/dashboard/build?prompt=test&other=value");
    expect(prompt).toBe("test");
  });

  test("decodes special characters correctly", () => {
    const url = "/dashboard/build?prompt=agent%20with%20%24100%20%26%20features%3F";
...
describe("Integration Simulation", () => {
  test("TopHero -> Build page flow", () => {
    ...
    const destination = activeTab === "agent" ? "/dashboard/build" : "/dashboard/fragments";
  ...
  test("clicking example prompt flow", () => {
    const examplePrompt = "An agent based on my dead father who gives life advice";
...
describe("Session Storage Simulation", () => {
  ...
  test("all tab types can be serialized", () => {
    const tabs: Array<"agent" | "app" | "image" | "video"> = ["agent", "app", "image", "video"];
