...
export async function GET(
  ...
  const document = await miniappStorageService.getDocument(app.id, documentId);
...
export async function PATCH(
  ...
  const document = await miniappStorageService.updateDocument(
    app.id,
    documentId,
    updateData,
    user.id
  ...
  logger.info(`[Miniapp Storage API] Updated document`, {
    appId: app.id,
    collection: collectionName,
    documentId,
    userId: user.id,
...
export async function PUT(
  ...
  const document = await miniappStorageService.replaceDocument(
    app.id,
    documentId,
    newData,
    user.id
  ...
  logger.info(`[Miniapp Storage API] Replaced document`, {
    appId: app.id,
    collection: collectionName,
    documentId,
    userId: user.id,
...
export async function DELETE(
  ...
  if (hardDelete) {
    await miniappStorageService.purgeDocument(app.id, documentId);
    logger.info(`[Miniapp Storage API] Hard deleted document`, {
      appId: app.id,
      collection: collectionName,
      documentId,
      userId: user.id,
    ...
    await miniappStorageService.deleteDocument(app.id, documentId, user.id);
    logger.info(`[Miniapp Storage API] Soft deleted document`, {
      appId: app.id,
      collection: collectionName,
      documentId,
      userId: user.id,
