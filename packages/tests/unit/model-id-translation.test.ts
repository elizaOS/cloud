...
describe("toOpenRouterModelId", () => {
  test("rewrites xai/ to x-ai/", () => {
    ...
    expect(toOpenRouterModelId("xai/grok-4-fast-reasoning")).toBe(
      "x-ai/grok-4-fast-reasoning",
    );
  });

  test("rewrites mistral/ to mistralai/", () => {
    expect(toOpenRouterModelId("mistral/codestral")).toBe(
      "mistralai/codestral",
    );
    expect(toOpenRouterModelId("mistral/magistral-medium")).toBe(
      "mistralai/magistral-medium",
    );
  });

  test("passes through openai/, anthropic/, google/, groq/", () => {
    expect(toOpenRouterModelId("openai/gpt-5.4-mini")).toBe(
      "openai/gpt-5.4-mini",
    );
    expect(toOpenRouterModelId("anthropic/claude-opus-4.7")).toBe(
      "anthropic/claude-opus-4.7",
    );
    expect(toOpenRouterModelId("google/gemini-3-pro-preview")).toBe(
  ...
  test("does not rewrite when prefix already matches OpenRouter format", () => {
    ...
    expect(toOpenRouterModelId("mistralai/codestral")).toBe(
      "mistralai/codestral",
    );
  });

  test("only rewrites the leading prefix, not occurrences inside the path", () => {
    expect(toOpenRouterModelId("openai/xai-named-model")).toBe(
      "openai/xai-named-model",
    );
  });
...
describe("fromOpenRouterModelId", () => {
  ...
  test("rewrites mistralai/ back to mistral/", () => {
    expect(fromOpenRouterModelId("mistralai/codestral")).toBe(
      "mistral/codestral",
    );
  });

  test("passes through other prefixes", () => {
    expect(fromOpenRouterModelId("openai/gpt-5.4-mini")).toBe(
      "openai/gpt-5.4-mini",
    );
    expect(fromOpenRouterModelId("anthropic/claude-opus-4.7")).toBe(
      "anthropic/claude-opus-4.7",
    );
  });
