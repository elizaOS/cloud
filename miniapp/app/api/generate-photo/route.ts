...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!ELIZA_CLOUD_API_KEY) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Image generation not configured. Please contact support.' }),
    ...
    const response = await fetch(`${ELIZA_CLOUD_URL}/api/v1/generate-image`, {
      method: 'POST',
      headers: {
    ...
    console.log(`[Generate Photo] Successfully generated image: ${imageUrl.slice(0, 100)}...`);

    // Return streaming response for consistency with client expectations
    const stream = new ReadableStream({
      start(controller) {
        // Send progress update
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', message: 'Image generated' })}\n\n`));
        // Send the image
