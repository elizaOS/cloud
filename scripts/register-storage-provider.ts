...
async function main() {
  ...
  const privateKey =
    process.env.CLOUD_OPERATOR_PRIVATE_KEY || process.env.PRIVATE_KEY;
  if (!privateKey) {
  ...
  if (existingProvider.registeredAt > 0n) {
    ...
    console.log(
      `  Type: ${Object.keys(ProviderType).find((k) => ProviderType[k as keyof typeof ProviderType] === existingProvider.providerType)}`,
    );
    ...
    console.log(
      `  Capacity: ${CLOUD_CAPACITY_GB} GB total, ${CLOUD_AVAILABLE_GB} GB available`,
    );
    ...
    for (const [tier, price] of Object.entries(CLOUD_PRICING)) {
      ...
      const tierName = Object.keys(StorageTier).find(
        (k) => StorageTier[k as keyof typeof StorageTier] === tierNum,
      );
  ...
  const cloudEndpoint =
    ...
    "https://cloud.jeju.network/api/v1/storage";
