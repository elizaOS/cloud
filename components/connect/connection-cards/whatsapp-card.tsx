...
export function WhatsAppCard({ status, onConnected }: WhatsAppCardProps) {
  ...
  if (status.loading) {
    return (
      <Card aria-busy="true" aria-label="Loading WhatsApp connection status">
        <CardContent className="flex items-center justify-center py-8">
          <Loader2 className="h-6 w-6 animate-spin text-muted-foreground" aria-hidden="true" />
          <span className="sr-only">Loading WhatsApp connection status...</span>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className="opacity-60">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="flex items-center gap-1">
            ...
            Requires Twilio connection first
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    return (
      <Card className="border-green-500/50 bg-green-500/5">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Send & receive WhatsApp messages</CardDescription>
        </CardHeader>
        <CardContent className="pt-0">
          <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
            <WhatsAppIcon className="h-5 w-5 text-[#25D366]" />
            <span className="text-sm font-medium">
              {status.details?.phoneNumber || "WhatsApp enabled"}
            </span>
          </div>
        </CardContent>
      </Card>
