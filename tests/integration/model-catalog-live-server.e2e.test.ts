...
function hasUsableCronConfig(): boolean {
  return !isPlaceholderCredential(process.env.CRON_SECRET);
}
...
describe.skipIf(!shouldRun)("Model catalog live server E2E", () => {
  beforeEach(async () => {
  ...
  test("reuses the existing cache entry across repeated /models and /models/status requests", async () => {
    ...
    expect(statusBody.models.every((model: { available: boolean }) => model.available))
      .toBe(true);
  });
