...
async function simulateCloudFacilitatorVerify(
  ...
  requirements: { scheme: string; network: string; asset: string; amount: string; payTo: string; extra?: Record<string, string> }
  ...
  let payload: {
    ...
    payload: { signature: string; authorization: { from: string; to: string; value: string; validBefore: string; nonce: string } };
  };
  try {
  ...
  try {
    ...
    return { isValid: false, invalidReason: `signature_verification_failed: ${(err as Error).message}`, payer };
  }
...
describe("x402 Plugin ↔ Eliza Cloud Integration", () => {
  ...
  it("should reject a payment header with wrong network", async () => {
    ...
    const sig = await account.signTypedData({
      ...
      message: { owner: account.address, spender: FACILITATOR_ADDRESS, value: 10000n, nonce: 0n, deadline },
    });

    const header = Buffer.from(JSON.stringify({
      x402Version: 2,
      accepted: { scheme: "upto", network: "eip155:1", asset: BASE_SEPOLIA_USDC, amount: "10000", payTo: FACILITATOR_ADDRESS },
      payload: { authorization: { from: account.address, to: FACILITATOR_ADDRESS, value: "10000", validBefore: deadline.toString(), nonce: "0" }, signature: sig },
    })).toString("base64");

    const result = await simulateCloudFacilitatorVerify(header, {
      scheme: "upto", network: "eip155:84532", asset: BASE_SEPOLIA_USDC, amount: "10000", payTo: FACILITATOR_ADDRESS,
    });
  ...
  it("should reject a payment header with expired deadline", async () => {
    ...
    const sig = await account.signTypedData({
      domain: { name: "USD Coin", version: "2", chainId: 84532n, verifyingContract: BASE_SEPOLIA_USDC },
      types: PERMIT_TYPES, primaryType: "Permit",
      message: { owner: account.address, spender: FACILITATOR_ADDRESS, value: 10000n, nonce: 0n, deadline: pastDeadline },
    });

    const header = Buffer.from(JSON.stringify({
      x402Version: 2,
      accepted: { scheme: "upto", network: "eip155:84532", asset: BASE_SEPOLIA_USDC, amount: "10000", payTo: FACILITATOR_ADDRESS },
      payload: { authorization: { from: account.address, to: FACILITATOR_ADDRESS, value: "10000", validBefore: pastDeadline.toString(), nonce: "0" }, signature: sig },
    })).toString("base64");
  ...
  it("should reject a payment header with insufficient amount", async () => {
    ...
    const sig = await account.signTypedData({
      domain: { name: "USD Coin", version: "2", chainId: 84532n, verifyingContract: BASE_SEPOLIA_USDC },
      types: PERMIT_TYPES, primaryType: "Permit",
      message: { owner: account.address, spender: FACILITATOR_ADDRESS, value: 5000n, nonce: 0n, deadline },
    });

    const header = Buffer.from(JSON.stringify({
      x402Version: 2,
      accepted: { scheme: "upto", network: "eip155:84532", asset: BASE_SEPOLIA_USDC, amount: "5000", payTo: FACILITATOR_ADDRESS },
      payload: { authorization: { from: account.address, to: FACILITATOR_ADDRESS, value: "5000", validBefore: deadline.toString(), nonce: "0" }, signature: sig },
    })).toString("base64");
  ...
  it("should reject a tampered signature", async () => {
    ...
    const header = Buffer.from(JSON.stringify({
      x402Version: 2,
      accepted: { scheme: "upto", network: "eip155:84532", asset: BASE_SEPOLIA_USDC, amount: "10000", payTo: FACILITATOR_ADDRESS },
      payload: { authorization: { from: account.address, to: FACILITATOR_ADDRESS, value: "10000", validBefore: deadline.toString(), nonce: "0" }, signature: tampered },
    })).toString("base64");
  ...
  it("should verify payment from a different wallet fails payer check", async () => {
    ...
    const sig = await otherAccount.signTypedData({
      domain: { name: "USD Coin", version: "2", chainId: 84532n, verifyingContract: BASE_SEPOLIA_USDC },
      types: PERMIT_TYPES, primaryType: "Permit",
      message: { owner: otherAccount.address, spender: FACILITATOR_ADDRESS, value: 10000n, nonce: 0n, deadline },
    });
    ...
    const header = Buffer.from(JSON.stringify({
      x402Version: 2,
      accepted: { scheme: "upto", network: "eip155:84532", asset: BASE_SEPOLIA_USDC, amount: "10000", payTo: FACILITATOR_ADDRESS },
      payload: { authorization: { from: EXPECTED_ADDRESS, to: FACILITATOR_ADDRESS, value: "10000", validBefore: deadline.toString(), nonce: "0" }, signature: sig },
    })).toString("base64");
