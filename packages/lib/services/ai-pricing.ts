  ...
  switch (model.pricingParser) {
    ...
    case "veo31": {
      ...
      for (const resolution of ["720p", "1080p"]) {
        entries.push(
          buildFalEntry(model, "second", Number(match[1]), { resolution, audio: false }),
        );
        entries.push(buildFalEntry(model, "second", Number(match[2]), { resolution, audio: true }));
      }
      entries.push(
        buildFalEntry(model, "second", Number(match[3]), { resolution: "4k", audio: false }),
      );
      entries.push(
        buildFalEntry(model, "second", Number(match[4]), { resolution: "4k", audio: true }),
      );
      break;
    }
    case "veo31lite": {
      ...
      entries.push(
        buildFalEntry(model, "second", Number(match[1]), { resolution: "720p", audio: true }),
      );
      entries.push(
        buildFalEntry(model, "second", Number(match[2]), { resolution: "720p", audio: false }),
      );
      entries.push(
        buildFalEntry(model, "second", Number(match[3]), { resolution: "1080p", audio: true }),
      );
      entries.push(
        buildFalEntry(model, "second", Number(match[4]), { resolution: "1080p", audio: false }),
      );
      break;
    }
    case "kling": {
      ...
      entries.push(
        buildFalEntry(model, "second", Number(match[1]), { audio: false, voiceControl: false }),
      );
      entries.push(
        buildFalEntry(model, "second", Number(match[2]), { audio: true, voiceControl: false }),
      );
      if (match[3]) {
        entries.push(
          buildFalEntry(model, "second", Number(match[3]), { audio: true, voiceControl: true }),
        );
      }
      break;
    }
    case "hailuo_standard": {
      ...
      entries.push(buildFalEntry(model, "request", Number(match[1]), { durationSeconds: 6 }));
      entries.push(buildFalEntry(model, "request", Number(match[2]), { durationSeconds: 10 }));
      break;
    }
    case "hailuo_pro": {
    ...
    case "wan": {
      ...
      entries.push(buildFalEntry(model, "second", Number(match[2]), { resolution: "1080p" }));
      break;
    }
    case "pixverse": {
      const match = paragraph.match(
