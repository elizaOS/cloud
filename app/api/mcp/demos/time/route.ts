...
const handler = createPaidMcpHandler(
  (server) => {
    // ========================================================================
    // Tool 1: Get Current Time - $0.0001 per request
    // ========================================================================
    server.paidTool(
      "get_current_time",
            ...
            return {
              content: [{
                ...
                text: JSON.stringify({
                  ...
                  suggestion: "Use IANA format (e.g., 'America/New_York') or common aliases (e.g., 'PST', 'EST', 'JST')",
    ...
    // ========================================================================
    // Tool 2: Convert Timezone - $0.0001 per request
    // ========================================================================
    server.paidTool(
      "convert_timezone",
        ...
        try {
          ...
          if (!isValidTimezone(fromTz)) {
            return {
              content: [{
                type: "text" as const,
                text: JSON.stringify({ error: `Invalid source timezone: '${fromTimezone}'` }, null, 2),
              }],
              isError: true,
            };
          }
          ...
          if (!isValidTimezone(toTz)) {
            return {
              content: [{
                type: "text" as const,
                text: JSON.stringify({ error: `Invalid target timezone: '${toTimezone}'` }, null, 2),
              }],
              isError: true,
            };
          }
    ...
    // ========================================================================
    // Tool 3: Format Date - $0.0001 per request
    // ========================================================================
    server.paidTool(
      "format_date",
    ...
    // ========================================================================
    // Tool 4: Calculate Time Difference - $0.0001 per request
    // ========================================================================
    server.paidTool(
      "calculate_time_diff",
    ...
    // ========================================================================
    // Tool 5: List Timezones - $0.0001 per request
    // ========================================================================
    server.paidTool(
      "list_timezones",
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                ...
                timezones: results,
                aliases: Object.entries(TIMEZONE_ALIASES).slice(0, 10).map(([alias, tz]) => ({
                  alias,
                  timezone: tz,
                })),
              }, null, 2),
...
export async function GET() {
  return NextResponse.json({
    name: "Time & Date MCP",
    version: "2.0.0",
    description: "Real-time date/time utilities with timezone conversion, formatting, and calculations using native JavaScript Intl APIs.",
    transport: ["http", "sse"],
    tools: [
      { 
