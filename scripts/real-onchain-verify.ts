...
async function verifySolana(): Promise<VerifyResult> {
  ...
  result.totalSupply = (Number(mintInfo.supply) / Math.pow(10, mintInfo.decimals)).toLocaleString();
  
  return result;
}

async function checkPrice(source: string, url: string): Promise<{ price?: number; error?: string }> {
  ...
  if (source === "coingecko") {
    const price = (data as Record<string, { usd?: number }>)[ELIZA_TOKENS.base.toLowerCase()]?.usd;
    return price ? { price } : { error: "No price data" };
  }
  
  if (source === "dexscreener") {
    const price = (data as { pairs?: Array<{ priceUsd?: string }> }).pairs?.[0]?.priceUsd;
...
async function main() {
  ...
  console.log("-".repeat(70));
  try {
    const eth = await verifyEVM("Ethereum", mainnet, "https://eth.llamarpc.com", ELIZA_TOKENS.ethereum as Address);
  ...
  console.log("-".repeat(70));
  try {
    const baseResult = await verifyEVM("Base", base, "https://mainnet.base.org", ELIZA_TOKENS.base as Address);
    console.log(`  Block: ${baseResult.blockNumber}`);
    console.log(`  Token: "${baseResult.tokenName}" (${baseResult.tokenSymbol})`);
  ...
  console.log("-".repeat(70));
  try {
    const bnb = await verifyEVM("BNB", bsc, "https://bsc-dataseed.binance.org", ELIZA_TOKENS.bnb as Address);
  ...
  if (evmWallet) {
    ...
    try {
      ...
      console.log(`  elizaOS Balance (Base): ${walletStatus.tokenBalance} tokens`);
  ...
  if (allNetworks && pricesWork) {
    console.log("  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("  🎉 ALL ON-CHAIN VERIFICATIONS PASSED - SYSTEM IS OPERATIONAL");
    console.log("  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  } else {
