...
if (!discordUrl && !slackUrl) {
  console.error(
    "No webhook URLs configured. Set DB_SLOW_QUERY_DISCORD_WEBHOOK or DB_SLOW_QUERY_SLACK_WEBHOOK",
  );
  process.exit(1);
}
      ...
      embeds: [
        {
          ...
          fields: [
            {
              name: "Duration",
              value: `${testPayload.durationMs}ms`,
              inline: true,
            },
  ...
  const res = await fetch(slackUrl, {
    ...
    body: JSON.stringify({
      blocks: [
        {
          type: "header",
          text: { type: "plain_text", text: "🧪 TEST: Slow Query Alert" },
        },
        {
          type: "section",
          fields: [
