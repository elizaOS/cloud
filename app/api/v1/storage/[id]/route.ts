...
export async function DELETE(
  ...
  if (signature && signerAddress) {
    ...
    if (
      ownerFromPath &&
      ownerFromPath.toLowerCase() !== signerAddress.toLowerCase()
    ) {
      return NextResponse.json(
  ...
  const walletInPath = user.wallet_address
    ? pathParts.find(
        (p) => p.toLowerCase() === user.wallet_address?.toLowerCase(),
      )
    : null;
  ...
  return new NextResponse(null, {
    ...
    headers: {
      ...
      "Access-Control-Allow-Headers":
        "Content-Type, Authorization, X-API-Key, X-PAYMENT, X-Signature, X-Signer-Address",
      "Access-Control-Expose-Headers": "X-Payment-Requirement",
    },
  });
}
