...
describe("Wallet Authentication", () => {
  ...
  it("should throw when findOrCreateUserByWalletAddress throws", async () => {
    mockFindOrCreate.mockRejectedValueOnce(
      new Error("User associated with wallet address not found"),
    );

    await expect(verifyWalletSignature(mockRequest)).rejects.toThrow(
      "User associated with wallet address not found",
    );
  });
