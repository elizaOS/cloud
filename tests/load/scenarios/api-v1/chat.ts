...
export function createChatCompletion(messages: ChatMessage[], model = "gpt-4o-mini", maxTokens = 50) {
  ...
  const body = httpPost<{ content?: string }>("/api/v1/chat", { messages, model, maxTokens, temperature: 0.7 }, {
    tags: { endpoint: "chat" }, timeout: "30s",
  });
...
export function a2aChatCompletion(messages: ChatMessage[], model = "gpt-4o-mini", maxTokens = 50) {
  const start = Date.now();
  const body = httpPost<{ result: unknown }>("/api/a2a", { jsonrpc: "2.0", method: "a2a.chatCompletion", params: { messages, model, maxTokens }, id: Date.now() }, {
    tags: { endpoint: "chat" }, timeout: "30s",
  });
  chatCompletionTime.add(Date.now() - start);
  if (!body) return null;
...
export function generateEmbeddings(text: string): number[] | null {
  const body = httpPost<{ embedding?: number[]; embeddings?: number[] }>("/api/v1/embeddings", { text }, { tags: { endpoint: "chat" } });
