...
class PlatformCredentialsService {
  async createLinkSession(params: CreateLinkSessionParams) {
    ...
    const scopes = params.requestedScopes?.length
    ...
    logger.info("[Credentials] Session created", {
  ...
  async getSessionByOAuthState(
    oauthState: string,
  ): Promise<PlatformCredentialSession | null> {
    const [session] = await db
      .select()
      .from(platformCredentialSessions)
      .where(
        and(
          eq(platformCredentialSessions.oauth_state, oauthState),
          eq(platformCredentialSessions.status, "pending"),
        ),
      )
      .limit(1);
    ...
    if (session.expires_at < new Date()) {
      await db
        .update(platformCredentialSessions)
        .set({ status: "expired" })
  ...
  async completeOAuth(
    ...
    logger.info("[Credentials] OAuth completed", {
      platform: session.platform,
  ...
  async listCredentials(
    organizationId: string,
    options?: { platform?: PlatformType; appId?: string; status?: string },
  ) {
  ...
  async revokeCredential(credentialId: string, organizationId: string) {
    ...
    if (credential.access_token_secret_id) {
      await secretsService
        .delete(credential.access_token_secret_id, organizationId, SYSTEM_AUDIT)
        .catch((err) => {
          logger.warn("[Credentials] Failed to delete access token secret", {
            credentialId,
    ...
    if (credential.refresh_token_secret_id) {
      await secretsService
        ...
        .catch((err) => {
          logger.warn("[Credentials] Failed to delete refresh token secret", {
            credentialId,
            error: extractErrorMessage(err),
          });
        });
  ...
  async refreshToken(
    ...
    const result = await this.getCredentialWithTokens(
      credentialId,
      organizationId,
    );
    ...
    logger.info("[Credentials] Token refreshed", {
      credentialId,
      platform: credential.platform,
    });
    return true;
  ...
  async cleanupExpiredSessions() {
    const result = await db
      .delete(platformCredentialSessions)
      .where(
        and(
          lt(platformCredentialSessions.expires_at, new Date()),
          eq(platformCredentialSessions.status, "pending"),
        ),
      );
  ...
  async getCredentialsExpiringWithin(
    hours: number,
  ): Promise<PlatformCredential[]> {
  ...
  async refreshExpiringTokens(
    ...
    logger.info("[Credentials] Proactive refresh complete", {
      refreshed,
      failed,
  ...
  async storeManualCredentials(params: {
    ...
    if (platform === "bluesky") {
      ...
      validated = await this.validateBluesky(
        credentials.handle,
    ...
    logger.info("[Credentials] Manual credentials stored", {
      platform,
      credentialId: credential.id,
    });
    return credential;
  ...
  private async validateBluesky(handle: string, appPassword: string) {
    ...
    const service =
      ...
      !normalizedHandle.endsWith(".bsky.social")
    ...
    const session = await safeJsonParse<{ did: string; accessJwt: string }>(
      response,
    );
    const profileRes = await fetch(
      `${service}/xrpc/app.bsky.actor.getProfile?actor=${session.did}`,
      {
        headers: { Authorization: `Bearer ${session.accessJwt}` },
      },
    );
    const profile = profileRes.ok
      ? await safeJsonParse<{ displayName?: string; avatar?: string }>(
          profileRes,
        )
      : {};
  ...
  async createMastodonLinkSession(
    params: CreateLinkSessionParams & { instanceUrl: string },
  ) {
    const instanceHost = new URL(
      params.instanceUrl.startsWith("http")
        ? params.instanceUrl
        : `https://${params.instanceUrl}`,
    ).origin;
    const cloudUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
    const redirectUri = `${cloudUrl}/api/auth/platform-callback/mastodon`;

    // Get or create app credentials for this instance
    const { clientId, clientSecret } = await this.getOrCreateMastodonApp(
      instanceHost,
      redirectUri,
    );
    ...
    logger.info("[Credentials] Mastodon session created", {
      sessionId: sessionId.slice(0, 8),
      instance: instanceHost,
    });
    return {
      sessionId,
      linkUrl: `${instanceHost}/oauth/authorize?${authParams}`,
      expiresAt,
    };
  }
  ...
  private async getOrCreateMastodonApp(
    instanceHost: string,
    redirectUri: string,
    ...
    if (!response.ok) {
      logger.error("[Mastodon] App registration failed", {
        instance: instanceHost,
    ...
    const app = await safeJsonParse<{
      client_id: string;
      client_secret: string;
    }>(response);
    await secretsService.createSystemSecret(
      this.mastodonSecretName(instanceHost),
      JSON.stringify({
  ...
  async getMastodonAppCredentials(
    instanceHost: string,
  ): Promise<{ clientId: string; clientSecret: string } | null> {
    const existing = await secretsService.getSystemSecret(
      this.mastodonSecretName(instanceHost),
    );
    if (!existing) return null;
    const parsed = parseJson<{ client_id: string; client_secret: string }>(
      existing,
  ...
  async getAvailablePlatforms(organizationId: string): Promise<
    Array<{
      platform: PlatformType;
