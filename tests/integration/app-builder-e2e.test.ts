...
beforeAll(async () => {
  ...
  if (!sandboxConfigured) {
    console.log(
      "⚠️ Vercel Sandbox not configured - sandbox tests will be skipped",
    );
    console.log(
      "   Set VERCEL_TOKEN, VERCEL_TEAM_ID, VERCEL_PROJECT_ID to enable",
    );
  } else {
...
describe("A2A Full App Builder Skills Discovery", () => {
  test("GET /api/a2a lists full app builder skills", async () => {
    ...
    console.log(
      `✅ Found ${skillNames.filter((n: string) => n.includes("app_builder")).length} app builder skills`,
    );
  });
});
...
describe("REST API App Builder Endpoints", () => {
  ...
  test(
    "POST /api/v1/app-builder - creates new session",
    async () => {
  ...
  test(
    "GET /api/v1/app-builder/sessions/:id - retrieves session",
    async () => {
      if (!sessionId) {
  ...
  test(
    "POST /api/v1/app-builder/sessions/:id/prompts - sends prompt",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
      ...
      const response = await apiPost<{
        ...
        prompt:
          "Create a simple button component that says 'Hello World' and logs to console when clicked",
      });
      ...
      console.log(
        `   Files affected: ${response.data.filesAffected.join(", ")}`,
      );
  ...
  test(
    "PATCH /api/v1/app-builder/sessions/:id - extends session",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
  ...
  test(
    ...
    async () => {
      ...
      if (response.ok) {
        ...
        console.log(
          `⚠️ Sandbox returned ${response.status} (may still be starting)`,
        );
      }
    },
    TIMEOUT,
  ...
  test(
    "DELETE /api/v1/app-builder/sessions/:id - stops session",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
...
describe("A2A Full App Builder Skills", () => {
  ...
  test(
    "full_app_builder_start creates session",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
        console.log("⚠️ Skipping - server or API key not available");
        return;
      ...
      const response = await a2aPost(
        skillMessage("full_app_builder_start", undefined, {
      ...
      if (response.status !== 200) {
        console.log(
          `⚠️ A2A returned ${response.status} - sandbox may not be configured`,
        );
        return;
      }
  ...
  test(
    "full_app_builder_prompt generates code",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
      ...
      const response = await a2aPost(
        skillMessage("full_app_builder_prompt", undefined, {
          sessionId,
          prompt:
            "Add a counter component with increment and decrement buttons",
        }),
      );
  ...
  test(
    "full_app_builder_status returns session info",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
      ...
      const response = await a2aPost(
        skillMessage("full_app_builder_status", undefined, {
          sessionId,
        }),
      );
      ...
      console.log(
        `✅ A2A status retrieved, files: ${(data?.generatedFiles as string[]).length}`,
      );
    },
    TIMEOUT,
  );
  ...
  test(
    "full_app_builder_list returns sessions",
    async () => {
      if (!serverAvailable || !apiKeyValid) {
        console.log("⚠️ Skipping - server or API key not available");
        return;
      ...
      const response = await a2aPost(
        skillMessage("full_app_builder_list", undefined, {
      ...
      console.log(
        `✅ A2A listed ${(data?.sessions as unknown[]).length} sessions`,
      );
    },
    TIMEOUT,
  );
  ...
  test(
    "full_app_builder_stop stops session",
    async () => {
      if (!sessionId) {
        console.log("⚠️ Skipping - no session created");
        return;
      ...
      const response = await a2aPost(
        skillMessage("full_app_builder_stop", undefined, {
          sessionId,
        }),
      );
...
describe("Multi-File App Generation E2E", () => {
  ...
  test(
    "Full E2E: Create multi-file app with proper structure",
    async () => {
...
`);
      ...
      if (createResponse.status !== 200) {
        console.log(
          "⚠️ Failed to create session - sandbox may not be configured",
        );
        return;
      }
...
Make sure all files have proper imports and exports.`,
      ...
      console.log(
        `   ✅ Found ${foundPatterns.length}/${expectedPatterns.length} expected file patterns`,
      );
      ...
      console.log(
        `   ✅ Session has ${statusResponse.data.session.messages?.length || 0} messages`,
      );
