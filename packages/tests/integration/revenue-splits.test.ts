...
describe.skipIf(!process.env.DATABASE_URL)(
  "Revenue Splits & Multi-Tier Referrals",
  () => {
    beforeAll(async () => {
      mock.restore();
      ...
      unreferredUser = await createUser(
        "Unreferred",
        `unref-${Date.now()}@test.com`,
      );
    ...
    it("Test 1: First-Touch Immutability", async () => {
      ...
      const signup = await referralSignupsRepository.findByReferredUserId(
        userA.id,
      );
    ...
    it("Test 2: No Referrer (100% Platform)", async () => {
      const { elizaCloudAmount, splits } =
        await referralsService.calculateRevenueSplits(unreferredUser.id, 100);
    ...
    it("Test 3: Multi-Tier Referrals (Nano Banana flow)", async () => {
      ...
      const { elizaCloudAmount, splits } =
        await referralsService.calculateRevenueSplits(buyer.id, 100);
