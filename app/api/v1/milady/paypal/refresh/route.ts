...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await miladyPaypalRouteDeps.refreshPaypalAccessToken(parsed.data);
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to refresh PayPal access token.",
      },
      { status: 500 },
    );
  }
