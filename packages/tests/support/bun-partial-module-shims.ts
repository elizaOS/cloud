...
const parsedCostBuffer = Number.parseFloat(
  process.env.CREDIT_COST_BUFFER ?? "",
);
const COST_BUFFER = Number.isFinite(parsedCostBuffer) ? parsedCostBuffer : 1.5;
const MIN_RESERVATION = 0.000001;
const { UsersRepository: RealUsersRepository } = await import(
...
interface UsersRepositoryModuleStub<
  TUsersRepository extends UsersRepositoryStub,
> {
  UsersRepository: typeof RealUsersRepository;
  usersRepository: TUsersRepository;
}

export function stubUsersRepositoryModule<
  TUsersRepository extends UsersRepositoryStub,
>(overrides: {
  usersRepository: TUsersRepository;
