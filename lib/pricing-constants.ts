...
export const IMAGE_GENERATION_COST = Math.round(BASE_IMAGE_GENERATION_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // $0.01 per image
export const VIDEO_GENERATION_COST = Math.round(BASE_VIDEO_GENERATION_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // $0.05 per video
export const VIDEO_GENERATION_FALLBACK_COST = Math.round(BASE_VIDEO_GENERATION_FALLBACK_COST * PLATFORM_MARKUP_MULTIPLIER * 1000) / 1000; // $0.025 per fallback video
...
export const VOICE_CLONE_INSTANT_COST = Math.round(BASE_VOICE_CLONE_INSTANT_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // ~$0.50 - 1-3 min audio, ~30s processing
export const VOICE_CLONE_PROFESSIONAL_COST = Math.round(BASE_VOICE_CLONE_PROFESSIONAL_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // ~$2.00 - 30+ min audio, 30-60min processing
export const VOICE_SAMPLE_UPLOAD_COST = Math.round(BASE_VOICE_SAMPLE_UPLOAD_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // ~$0.05 - Additional samples to existing voice
export const VOICE_UPDATE_COST = Math.round(BASE_VOICE_UPDATE_COST * PLATFORM_MARKUP_MULTIPLIER * 100) / 100; // ~$0.10 - Update voice metadata/settings
...
export const TTS_COST_PER_1K_CHARS = Math.round(BASE_TTS_COST_PER_1K_CHARS * PLATFORM_MARKUP_MULTIPLIER * 10000) / 10000; // ~$0.029 per 1K chars with markup
export const STT_COST_PER_MINUTE = Math.round(BASE_STT_COST_PER_MINUTE * PLATFORM_MARKUP_MULTIPLIER * 10000) / 10000; // ~$0.004 per minute with markup
