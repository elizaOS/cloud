...
```bash
curl -X POST "https://elizacloud.ai/api/v1/my-agents/characters" \
  ...
  -d '{
    ...
    "style": {
      "all": ["Professional", "Thorough", "Evidence-based"],
      "chat": ["Concise responses", "Cite sources when available"]
    },
...
```bash
curl -X GET "https://elizacloud.ai/api/v1/my-agents/characters" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://elizacloud.ai/api/v1/my-agents/characters/{agentId}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X PUT "https://elizacloud.ai/api/v1/my-agents/characters/{agentId}" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE "https://elizacloud.ai/api/v1/my-agents/characters/{agentId}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<Callout type="info">
Learn more about agent monetization in the [Earnings](/docs/earnings) documentation.
</Callout>

## Best Practices

<Cards>
  <Cards.Card title="Clear Instructions" href="/docs/agents">
    Provide specific style guidelines for consistent behavior
  </Cards.Card>
  <Cards.Card title="Test Thoroughly" href="/docs/agents">
    Use the API Explorer to test your agent before publishing
  </Cards.Card>
  <Cards.Card title="Monitor Usage" href="/docs/agents">
    Track agent analytics to optimize performance
  </Cards.Card>
  <Cards.Card title="Iterate" href="/docs/agents">
