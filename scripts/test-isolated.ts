...
function checkTestOutput(output: string): { passed: boolean; skipped: boolean } {
  ...
  const hasError = output.includes("error:") || output.includes("# Unhandled error");
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      return {
        ...
        output: VERBOSE || !passed ? output : (skipped ? "⚠️  SKIPPED" : undefined),
      };
    } catch (error) {
...
async function main() {
  ...
  console.log(`🔒 ${isolatedFiles.length} files require isolation (use mock.module)\n`);
