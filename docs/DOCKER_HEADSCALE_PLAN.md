...
## Goal
Replace the Vercel Sandbox provider in `milady-sandbox.ts` with milady-cloud's Docker container + Headscale VPN infrastructure. Each agent gets a real VPS-like container on Hetzner nodes with persistent volumes, SSH access, and headscale mesh networking.

## Current State

### Eliza Cloud (this repo)
- `lib/services/milady-sandbox.ts` — orchestrates agent lifecycle via **Vercel Sandbox SDK**
- `db/schemas/milady-sandboxes.ts` — tracks sandbox state, Neon DB, backups
- `app/api/v1/milady/` — REST API for agent CRUD, provision, bridge, snapshot, restore
...
**Files:** `lib/services/sandbox-provider.ts`, `lib/services/milady-sandbox.ts`
...
3. Update `MiladySandboxService` to use `SandboxProvider` interface instead of direct Vercel calls
4. Provider selected via `MILADY_SANDBOX_PROVIDER=vercel|docker` env var
...
2. Port from milady-cloud's `container-orchestrator.ts`:
   - Docker image: configurable via `MILADY_DOCKER_IMAGE` env var
...
3. Add node_id column to `milady_sandboxes` table:
   ```sql
...
```env
# Provider selection
MILADY_SANDBOX_PROVIDER=docker  # vercel | docker

# Docker provider config
MILADY_DOCKER_IMAGE=milady/agent:cloud-full-ui
MILADY_DOCKER_NODES=agent-node-1:37.27.190.196:8,nyx-node:89.167.49.4:8
...
# Wildcard DNS
MILADY_AGENT_DOMAIN=shad0w.xyz
```

## Migration Path
1. Deploy with `MILADY_SANDBOX_PROVIDER=vercel` (no change)
2. Set up Docker nodes, headscale, register nodes via admin API
3. Switch to `MILADY_SANDBOX_PROVIDER=docker`
