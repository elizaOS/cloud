...
export async function deleteTestApp(appId: string): Promise<void> {
  await api.del(
    `/api/v1/apps/${appId}?deleteGitHubRepo=false&deleteVercelProject=false`,
    {
      authenticated: true,
    },
  );
...
export async function getPublicAppInfo(
  appId: string,
): Promise<{ response: Response; body: any }> {
...
export async function deleteTestAgent(agentId: string): Promise<void> {
  ...
  await api
    .del(`/api/v1/agents/${agentId}/publish`, { authenticated: true })
    .catch(() => {});
