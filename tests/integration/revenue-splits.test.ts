...
describe.skipIf(!process.env.DATABASE_URL)("Revenue Splits & Multi-Tier Referrals", () => {
    ...
    it("Test 1: First-Touch Immutability", async () => {
        ...
        const res1 = await referralsService.applyReferralCode(userA.id, userA.organization_id, codeX.code);
        expect(res1.success).toBe(true);

        // Verify it was correctly recorded
        let signup = await referralSignupsRepository.findByReferredUserId(userA.id);
        expect(signup?.referrer_user_id).toBe(userB.id);

        // Attempt to apply Code Y to User A (Should Fail)
        const res2 = await referralsService.applyReferralCode(userA.id, userA.organization_id, codeY.code);
    ...
    it("Test 2: No Referrer (100% Platform)", async () => {
        const { elizaCloudAmount, splits } = await referralsService.calculateRevenueSplits(unreferredUser.id, 100);
    ...
    it("Test 3: Multi-Tier Referrals (Nano Banana flow)", async () => {
        ...
        let creatorCode = await referralsService.getOrCreateCode(userD.id);

        // Update creator code to have the parent
        const { referralCodes } = await import("@/db/schemas/referrals");
        await dbWrite.update(referralCodes)
        ...
        const res = await referralsService.applyReferralCode(buyer.id, buyer.organization_id, creatorCode.code, {
            appOwnerId: appOwner.id
        });
        expect(res.success).toBe(true);

        // Calculate splits on $100
        const { elizaCloudAmount, splits } = await referralsService.calculateRevenueSplits(buyer.id, 100);
