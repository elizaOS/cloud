...
The Milady sandbox provisioning flow now works end-to-end with the Docker provider. The abstraction layer was already in place, but needed wiring to persist docker-specific metadata and integrate optional Headscale VPN.

## Changes Made

### 1. **MiladySandboxService** (`lib/services/milady-sandbox.ts`)
...
- **Bridge** (`/api/v1/milady/agents/[agentId]/bridge`) - Uses `rec.bridge_url`
- **Stream** (`/api/v1/milady/agents/[agentId]/stream`) - Uses `rec.bridge_url` via `bridgeStream()`
- **Snapshot** (`/api/v1/milady/agents/[agentId]/snapshot`) - Uses `rec.bridge_url` to call `/api/snapshot`
- **Restore** (`/api/v1/milady/agents/[agentId]/restore`) - Uses `rec.bridge_url` to call `/api/restore`
- **Provision** (`/api/v1/milady/agents/[agentId]/provision`) - Provider-agnostic via factory
...
### Docker Image
- `MILADY_DOCKER_IMAGE=milady/agent:cloud-full-ui` (default)
...
ALTER TABLE milady_sandboxes ADD COLUMN container_name TEXT;
ALTER TABLE milady_sandboxes ADD COLUMN bridge_port INTEGER;
ALTER TABLE milady_sandboxes ADD COLUMN web_ui_port INTEGER;
ALTER TABLE milady_sandboxes ADD COLUMN headscale_ip TEXT;
ALTER TABLE milady_sandboxes ADD COLUMN docker_image TEXT;
```
...
- [ ] Provision a new agent with `MILADY_SANDBOX_PROVIDER=docker`
- [ ] Verify docker-specific fields are persisted in DB
- [ ] Send bridge requests via `/api/v1/milady/agents/[agentId]/bridge`
- [ ] Stream messages via `/api/v1/milady/agents/[agentId]/stream`
- [ ] Create snapshot via `/api/v1/milady/agents/[agentId]/snapshot`
- [ ] Restore from backup via `/api/v1/milady/agents/[agentId]/restore`
