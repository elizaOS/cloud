...
class UnifiedModerationService {
  ...
  async scan(input: ScanInput): Promise<ScanResult> {
    ...
    if (contentType === "image") {
      ...
      if (shouldSkipImage(size, contentMimeType, contentUrl)) {
        await this.upsertItem(input, contentHash, "clean", [], 1, undefined, undefined);
    ...
    if ("error" in modResult) {
      ...
      if (!isPublic) {
        ...
        await this.upsertItem(input, contentHash, "pending", [], 0, undefined, undefined, attempts, nextScan);
      }
    ...
    // Save result
    const confidence = modResult.flags.reduce((max, f) => Math.max(max, f.confidence), 0);
    await this.upsertItem(input, contentHash, status, modResult.flags, confidence, "omni-moderation-latest", modResult.scores);
  ...
  private async upsertItem(
    ...
    const existing = await db.query.contentModerationItems.findFirst({
      where: and(
        eq(contentModerationItems.sourceTable, input.sourceTable),
        eq(contentModerationItems.sourceId, input.sourceId)
      ),
    });
