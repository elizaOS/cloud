...
describe("Pool Closure Race Condition", () => {
  ...
  describe("Thesis Verification: safeClose calls pool.end()", () => {
     ...
     */
    it("should verify that runtime.close() calls adapter.close() which ends the pool", async () => {
      // Create a runtime
  ...
  describe("Race Condition: OLD Behavior (should fail)", () => {
     ...
     */
    it("should demonstrate OLD eviction behavior causes pool closure", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
  ...
  describe("Race Condition: NEW Behavior (should succeed)", () => {
     ...
     */
    it("should demonstrate NEW eviction behavior preserves pool", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
     ...
     */
    it("should demonstrate parallel queries succeed with NEW eviction", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
  ...
  describe("Race Condition: Multiple Runtimes Sharing Pool", () => {
     ...
     */
    it("should demonstrate shared pool closure affecting other runtimes", async () => {
  ...
  describe("Race Condition: Direct Adapter Close", () => {
     ...
     */
    it("should show direct adapter.close() terminates shared pool", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
  ...
  describe("Race Condition: In-Flight Query During Pool Close", () => {
     ...
     */
    it("should demonstrate in-flight query failure when pool closes", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
      ...
      const queryLoop = async (index: number): Promise<void> => {
        ...
        while (Date.now() - startTime < 500) { // Query for 500ms
          try {
            await adapter!.isReady();
      ...
      if (errors.length > 0) {
        ...
        const hasPoolError = errors.some(err =>
     ...
     */
    it("should demonstrate actual DB query failure when pool closes", async () => {
      const userContext = buildUserContext(testData, {
        agentMode: AgentMode.ASSISTANT,
        webSearchEnabled: false,
      });
...
describe("Production Error Reproduction", () => {
   ...
   */
  it("should reproduce production 'Cannot use a pool after calling end' error", async () => {
    console.log("\n" + "=".repeat(60));
