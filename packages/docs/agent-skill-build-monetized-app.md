...
> **Audience:** the Milady / ElizaOS framework agent — the one running
...
| The `@elizaos/cloud-sdk` package | Already in your runtime (Milady packages it). |
...
/ Codex `SKILL.md` skill for sub-agents that run inside a Milady agent
via the stealth claude-code path.** That skill would give those
sub-agents access to the parent Milady agent's APIs (memories,
characters, runtime state) so they aren't disconnected when Claude
Code spawns inside a Milady-orchestrated coding flow.

Different audience from THIS skill (which targets the Milady agent
itself, not its sub-agents). Worth its own doc + its own implementation.
Likely lives at `docs/claude-subagent-skill-milady-bridge.md` (not yet
written) and the actual `SKILL.md` would land in the Milady agent's
