		...
		<Card className="flex h-full min-h-0 flex-col border-border/60 bg-background/80">
				...
				<div className="relative mt-6 aspect-video w-full overflow-hidden rounded-2xl border border-border/60 bg-muted/40 shadow-inner">
					{video ? (
						<>
							{video.videoUrl ? (
								...
								<div
									...
									style={video.thumbnailUrl ? { backgroundImage: `url(${video.thumbnailUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}
								>
									{!video.thumbnailUrl && (
										<>
											<Play className="h-10 w-10 text-muted-foreground" />
											<p className="mt-3 text-sm text-muted-foreground">Generated video preview</p>
										</>
									)}
								</div>
							)}
							{video.status === "processing" && (
								<div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-background/80 text-center">
									...
									<p className="text-sm font-medium text-foreground">Sending job to Fal…</p>
						...
						<div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-center text-muted-foreground">
							...
							<p className="text-sm font-medium text-foreground">Your video will appear here once generated.</p>
				...
				<div className="mt-6 grid gap-4 rounded-2xl border border-border/50 bg-background/60 p-4 text-sm">
					...
					<div className="grid gap-2 text-xs text-muted-foreground">
						<div className="flex flex-wrap items-center justify-between gap-2">
							...
							<span className="font-medium text-foreground">{video?.modelId ?? 'Not selected'}</span>
						</div>
						{video?.requestId ? (
							<div className="flex flex-wrap items-center justify-between gap-2">
								<span>Request ID</span>
								<span className="font-medium text-foreground break-all">{video.requestId}</span>
							</div>
						) : null}
						<div className="flex flex-wrap items-center justify-between gap-2">
							<span className="flex items-center gap-2"><Clock className="h-3.5 w-3.5" /> Duration</span>
						...
						{processingTimeLabel ? (
							<div className="flex flex-wrap items-center justify-between gap-2">
								...
								<span className="font-medium text-foreground">{processingTimeLabel}</span>
							</div>
						) : null}
						<div className="flex flex-wrap items-center justify-between gap-2">
						...
						<div className="flex flex-wrap items-center justify-between gap-2">
							<span>Seed</span>
							<span className="font-medium text-foreground">{video?.seed ?? 'Auto'}</span>
						</div>
						{video?.referenceUrl ? (
							<div className="flex flex-wrap items-center justify-between gap-2">
								<span>Reference</span>
			...
			<CardFooter className="flex flex-col gap-3 border-t border-border/60 bg-background/70 py-4">
				...
				<div className="flex flex-wrap gap-2">
					<Button className="flex-1 rounded-xl" variant="default" type="button" onClick={handleDownload}>
						<Download className="mr-2 h-4 w-4" />
						Download
					</Button>
					<Button className="flex-1 rounded-xl" variant="outline" type="button" onClick={handleCopyLink}>
						<Link2 className="mr-2 h-4 w-4" />
						Copy link
					</Button>
				</div>
				{copyFeedback && (
					<p className="text-center text-xs text-muted-foreground">{copyFeedback}</p>
				)}
			</CardFooter>
		</Card>
	);
