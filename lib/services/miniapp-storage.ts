...
class MiniappStorageService {
  async createCollection(params: CreateCollectionParams): Promise<MiniappCollection> {
    ...
    logger.info(`[MiniappStorage] Created collection: ${name}`, {
      appId,
  ...
  async getCollection(
    appId: string,
    name: string
  ): Promise<MiniappCollection | null> {
    const [collection] = await db
      .select()
      .from(miniappCollections)
      .where(
        and(
          eq(miniappCollections.app_id, appId),
          eq(miniappCollections.name, name)
        )
      )
      .limit(1);
  ...
  async listCollections(appId: string): Promise<MiniappCollection[]> {
    return db
      .select()
      .from(miniappCollections)
      .where(eq(miniappCollections.app_id, appId))
      .orderBy(asc(miniappCollections.name));
  }
  ...
  async updateCollectionSchema(
    ...
    schema: CollectionSchema,
    indexes?: CollectionIndex[]
  ): Promise<MiniappCollection> {
    const collection = await this.getCollection(appId, name);
    if (!collection) {
      throw new Error(`Collection '${name}' not found`);
    }

    const updateData: Partial<MiniappCollection> = {
      schema,
    ...
    const [updated] = await db
      .update(miniappCollections)
      .set(updateData)
      .where(eq(miniappCollections.id, collection.id))
      .returning();

    logger.info(`[MiniappStorage] Updated collection schema: ${name}`, {
      appId,
      collectionId: collection.id,
  ...
  async deleteCollection(appId: string, name: string): Promise<void> {
    ...
    await db
      .delete(miniappCollections)
      .where(eq(miniappCollections.id, collection.id));

    logger.info(`[MiniappStorage] Deleted collection: ${name}`, {
      appId,
      collectionId: collection.id,
    });
  }
  ...
  async insertDocument(
    ...
    await db
      .update(miniappCollections)
      .set({
        document_count: sql`${miniappCollections.document_count} + 1`,
        updated_at: new Date(),
      })
      .where(eq(miniappCollections.id, collection.id));
  ...
  async getDocument(
    appId: string,
    documentId: string
  ): Promise<MiniappDocument | null> {
    const [document] = await db
      .select()
      .from(miniappDocuments)
      .where(
        and(
          eq(miniappDocuments.app_id, appId),
          eq(miniappDocuments.id, documentId),
          isNull(miniappDocuments.deleted_at)
        )
      )
      .limit(1);
  ...
  async queryDocuments(
    ...
    collectionName: string,
    params: QueryParams = {}
  ): Promise<{ documents: MiniappDocument[]; total: number }> {
    const collection = await this.getCollection(appId, collectionName);
    if (!collection) {
      throw new Error(`Collection '${collectionName}' not found`);
    }
    ...
    const conditions = [
      eq(miniappDocuments.app_id, appId),
      eq(miniappDocuments.collection_id, collection.id),
    ];

    if (!includeDeleted) {
      conditions.push(isNull(miniappDocuments.deleted_at));
    }
    ...
    for (const [field, value] of Object.entries(filter)) {
      ...
      if (indexSlot) {
        if (indexSlot.startsWith("idx_str_")) {
          const col = miniappDocuments[indexSlot as keyof typeof miniappDocuments] as typeof miniappDocuments.idx_str_1;
          conditions.push(eq(col, String(value)));
        } else if (indexSlot.startsWith("idx_num_")) {
          const col = miniappDocuments[indexSlot as keyof typeof miniappDocuments] as typeof miniappDocuments.idx_num_1;
          conditions.push(eq(col, String(value)));
        } else if (indexSlot.startsWith("idx_bool_")) {
          const col = miniappDocuments[indexSlot as keyof typeof miniappDocuments] as typeof miniappDocuments.idx_bool_1;
          conditions.push(eq(col, Boolean(value)));
        }
      } else {
        conditions.push(
          sql`${miniappDocuments.data}->>${field} = ${String(value)}`
        );
      }
    }
    ...
    if (sort) {
      ...
      if (indexSlot) {
        const col = miniappDocuments[indexSlot as keyof typeof miniappDocuments];
        query = query.orderBy(sort.order === "desc" ? desc(col) : asc(col)) as typeof query;
      } else if (sort.field === "created_at") {
        query = query.orderBy(
          sort.order === "desc"
            ? desc(miniappDocuments.created_at)
            : asc(miniappDocuments.created_at)
        ) as typeof query;
      } else if (sort.field === "updated_at") {
        query = query.orderBy(
          sort.order === "desc"
            ? desc(miniappDocuments.updated_at)
            : asc(miniappDocuments.updated_at)
        ) as typeof query;
      } else {
        query = query.orderBy(
          sort.order === "desc"
            ? desc(sql`${miniappDocuments.data}->>${sort.field}`)
            : asc(sql`${miniappDocuments.data}->>${sort.field}`)
        ) as typeof query;
      }
    } else {
      query = query.orderBy(desc(miniappDocuments.created_at)) as typeof query;
    }
  ...
  async updateDocument(
    ...
    const [updated] = await db
      ...
      .where(eq(miniappDocuments.id, documentId))
      .returning();
  ...
  async deleteDocument(
    ...
    await db
      .update(miniappCollections)
      .set({
        document_count: sql`GREATEST(${miniappCollections.document_count} - 1, 0)`,
        updated_at: new Date(),
      })
      .where(eq(miniappCollections.id, existing.collection_id));
  ...
  async insertMany(
    ...
    await db
      .update(miniappCollections)
      .set({
        document_count: sql`${miniappCollections.document_count} + ${documents.length}`,
        updated_at: new Date(),
      })
      .where(eq(miniappCollections.id, collection.id));
  ...
  private async getCollectionById(id: string): Promise<MiniappCollection | null> {
    const [collection] = await db
      .select()
      .from(miniappCollections)
  ...
  private async logChange(
    ...
    await db.insert(miniappDocumentChanges).values({
...
export const miniappStorageService = new MiniappStorageService();
