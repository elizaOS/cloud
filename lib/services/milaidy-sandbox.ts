  ...
  async provision(agentId: string, orgId: string): Promise<ProvisionResult> {
    ...
    if (rec.database_status !== "ready" || !dbUri) {
      ...
      if (!db.success) {
        ...
        return { success: false, sandboxRecord: await miladySandboxesRepository.findById(rec.id), error: db.error };
      }
    ...
    if (!sb.success) {
      ...
      return { success: false, sandboxRecord: await miladySandboxesRepository.findById(rec.id), error: sb.error };
    }
