...
interface UsageResponse {
  ...
  usage: { totalRequests: number; successfulRequests: number; failedRequests: number; totalTokens: number; inputTokens: number; outputTokens: number };
  period: { start: string; end: string };
  lastUpdated: string;
}

async function fetchUsageData(organizationId: string, days: number): Promise<UsageResponse | null> {
  ...
  const [org, transactionStats, usageStats, lastTransaction] = await Promise.all([
    ...
    }).from(creditTransactions).where(eq(creditTransactions.organization_id, organizationId)),
    db.select({
    ...
    }).from(usageRecords).where(and(eq(usageRecords.organization_id, organizationId), gte(usageRecords.created_at, periodStart))),
    db.select({ created_at: creditTransactions.created_at }).from(creditTransactions).where(eq(creditTransactions.organization_id, organizationId)).orderBy(desc(creditTransactions.created_at)).limit(1),
  ]);
  ...
  return {
    credits: { remaining, used: totalDebits, initial: initialCredits > 0 ? initialCredits : 5.0 },
    usage: {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await cache.getWithSWR<UsageResponse>(cacheKey, 30, () => fetchUsageData(organizationId, days));
