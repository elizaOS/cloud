...
describe("provisioning-jobs org-id cross-check", () => {
  test("executeMiladyProvision throws on org-id mismatch", async () => {
    ...
    // Since executeMiladyProvision is private, we test the logic inline.
    const mismatched = {
      data: { organizationId: "org-A" },
      organization_id: "org-B",
    };

    // Replicate the assertion logic from executeMiladyProvision
...
describe("waifu-bridge slugFromUserId determinism", () => {
  ...
  test("same userId always produces the same slug", () => {
    const slug1 = slugFromUserId(
      "waifu:0xABCDEF1234567890abcdef1234567890abcdef12",
    );
    const slug2 = slugFromUserId(
      "waifu:0xABCDEF1234567890abcdef1234567890abcdef12",
    );
