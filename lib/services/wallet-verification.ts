...
import {
  ...
  tokenGatesRepository,
} from "@/db/repositories/community-moderation";
...
class WalletVerificationService {
  ...
  async verifyAndLinkWallet(
  ...
  ): Promise<VerificationResult & { wallet?: OrgMemberWallet }> {
    ...
    logger.info("[WalletVerification] Wallet verified", {
      serverId,
      platformUserId,
  ...
  private async checkEvmBalance(
    ...
    const provider = new ethers.JsonRpcProvider(
      RPC_ENDPOINTS[chain] ?? RPC_ENDPOINTS.ethereum,
    );

    if (
      tokenType === "token" ||
      tokenType === "nft" ||
      tokenType === "nft_collection"
    ) {
  ...
  async syncRoles(
    ...
    for (const wallet of wallets) {
      ...
      await memberWalletsRepository.updateAssignedRoles(
        wallet.id,
        assignedRoles,
      );
    }
...
export const walletVerificationService =
  WalletVerificationService.getInstance();
