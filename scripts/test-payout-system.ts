...
import { ARBITRAGE_PROTECTION, SUPPLY_SHOCK_PROTECTION } from "../lib/config/redemption-security";
import { twapPriceOracle, TWAP_CONFIG, SYSTEM_LIMITS } from "../lib/services/twap-price-oracle";
...
async function testNetworkConfiguration(config: TestConfig): Promise<TestResult[]> {
  ...
  results.push({
    ...
    passed: testnetMode,
    message: testnetMode ? "Running in testnet mode" : "Running in mainnet mode (set PAYOUT_TESTNET=true for testing)",
  });
  ...
  results.push({
    ...
    message: networkConfig ? `Found config for ${networkConfig.name}` : `No config for ${config.network}`,
  });
  
  if (networkConfig) {
    success(`Network: ${networkConfig.name} (chainId: ${networkConfig.chainId})`);
  ...
  const hasToken = networkConfig.tokenAddress !== "0x0000000000000000000000000000000000000000";
  results.push({
    name: "Token Address Configured",
    passed: hasToken,
    message: hasToken ? `Token: ${networkConfig.tokenAddress}` : "Token address not set (deploy test token first)",
  });
  
  if (hasToken) {
    success(`Token address: ${networkConfig.tokenAddress.slice(0, 10)}...`);
  } else {
    error("Token address not configured - set ELIZA_TOKEN_BASE_SEPOLIA in .env");
  }
...
async function testWalletConfiguration(config: TestConfig): Promise<TestResult[]> {
  const results: TestResult[] = [];
  ...
  if (networkConfig.chain) {
    ...
    try {
      ...
      if (hasGas) {
        success(`Native balance: ${formattedNative} ${networkConfig.nativeCurrency}`);
      } else {
        error(`No ${networkConfig.nativeCurrency} for gas - fund the wallet`);
      }
      
      // Check token balance
      if (!config.skipBalance && networkConfig.tokenAddress !== "0x0000000000000000000000000000000000000000") {
        try {
          ...
          const formattedToken = formatUnits(tokenBalance, networkConfig.tokenDecimals);
          ...
          if (hasTokens) {
            success(`Token balance: ${formattedToken} ${networkConfig.tokenSymbol}`);
          } else {
            warn(`No ${networkConfig.tokenSymbol} tokens - transfer some for testing`);
          }
        } catch (e) {
          results.push({
...
async function testPriceOracle(config: TestConfig): Promise<TestResult[]> {
  ...
  const priceNetwork = config.network.includes("sepolia") || config.network.includes("testnet") || config.network.includes("devnet")
  ...
  success(`Limits - Hourly: $${SYSTEM_LIMITS.MAX_HOURLY_REDEMPTION_USD}, Daily: $${SYSTEM_LIMITS.MAX_DAILY_REDEMPTION_USD}`);
  ...
  try {
    ...
    if (twap) {
      ...
      success(`TWAP: $${twap.twapPrice.toFixed(6)} (${twap.sampleCount} samples)`);
...
async function testSecurityValidations(config: TestConfig): Promise<TestResult[]> {
  const results: TestResult[] = [];
  ...
  success(`Large redemption (>$${SUPPLY_SHOCK_PROTECTION.LARGE_REDEMPTION_THRESHOLD_USD}): ${SUPPLY_SHOCK_PROTECTION.LARGE_REDEMPTION_DELAY_MS / 60000} min delay`);
  ...
  success(`Velocity limit: ${SYSTEM_LIMITS.VELOCITY_LIMIT_COUNT} redemptions / ${SYSTEM_LIMITS.VELOCITY_LIMIT_WINDOW_MS / 60000} min`);
  ...
  success(`Quote validity: ${ARBITRAGE_PROTECTION.QUOTE_VALIDITY_MS / 1000} seconds`);
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════╗");
  console.log("║          elizaOS Payout System Test Suite                     ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
