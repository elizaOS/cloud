...
describe.skipIf(skipLiveModelSuite)(
  "CloudBootstrapMessageService - Web Search Multi-Step",
  () => {
    beforeAll(async () => {
      console.log("\n" + "=".repeat(70));
      ...
      if (!connected) {
        throw new Error(
          "Cannot connect to database. Ensure server is running.",
        );
      }
      ...
      testData = await createTestDataSet(connectionString, {
        ...
        characterSettings: webSearchCharacter.settings as Record<
          string,
          unknown
        >,
      });
