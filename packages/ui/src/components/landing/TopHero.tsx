...
const TopHero = () => {
  const [activeOS, _setActiveOS] = useState<"unix" | "windows">("unix");
  const [copied, setCopied] = useState<string | null>(null);
  const [deployingDots, setDeployingDots] = useState(0);
  const [isDeploying, setIsDeploying] = useState(true);
  const [cursorLine, setCursorLine] = useState<"deploy" | "deploying" | "url">("deploy");
    ...
    <section className="w-full flex items-center shrink-0 py-24 lg:py-36 relative overflow-hidden">
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="mx-auto max-w-5xl text-center">
          ...
          >
            <span className={status === "create" ? "bg-brand-orange" : ""}>Create</span> and{" "}
            <span className={status === "deploy" ? "bg-brand-orange" : ""}>deploy</span> AI agents
            in one command. Open source. Zero lock-in.
          </motion.p>

          {/* Terminal Display */}
          <div className="relative mx-auto max-w-3xl mb-2 md:mb-4">
            {status === "create" && <CornerBrackets size="md" color="#FF5800" />}
            {/* Create command */}
            <div className="flex items-center justify-between gap-2 bg-[#161616BF] border border-white/15 rounded px-3 py-2.5">
              <div className="flex items-center gap-2 min-w-0 overflow-x-auto">
                <span style={{ color: status === "create" ? "#FF5800" : "white" }}>▸</span>
          ...
          <div className="relative mx-auto max-w-3xl mb-10 md:mb-16">
            {status === "deploy" && <CornerBrackets size="md" color="#FF5800" />}
            <div className="bg-[#161616BF] border border-white/15 overflow-hidden text-start">
              ...
              <div className="bg-[#161616BF] border-b border-white/10 pl-4 md:pl-6 pr-2 md:pr-3 flex h-10 md:h-12 items-center justify-between gap-2">
                ...
                <button
                  onClick={() => handleCopy(commands[activeOS].deploy, "deploy")}
                  className="shrink-0 p-1.5 text-white/60 hover:text-white transition-colors"
                  aria-label="Copy command"
                >
                  {copied === "deploy" ? (
              ...
              <div className="p-4 md:p-6 text-sm md:text-base">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white select-all">npx elizaos deploy</span>
                ...
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={status === "deploy" ? { opacity: 1 } : { opacity: 0 }}
                  transition={{
                    duration: 0.3,
