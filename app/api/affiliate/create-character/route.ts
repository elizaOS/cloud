...
import { z } from "zod";
import { apiKeysService, charactersService, usersService, anonymousSessionsService, organizationsService } from "@/lib/services";
...
function checkRateLimit(apiKeyId: string, limit = 100): { allowed: boolean; remaining: number } {
  const now = Date.now();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      logger.warn(`[Affiliate API] Rate limit exceeded for key ${apiKey.key_prefix}`);
      return NextResponse.json(
        { 
          success: false,
    ...
    try {
      ...
      if (!affiliateOrg) {
        ...
        logger.info("[Affiliate API] Created new affiliate organization", { id: affiliateOrg.id });
      }
    } catch (error) {
    ...
    logger.info(`[Affiliate API] Generated redirect URL for affiliate ${affiliateId}: ${redirectUrl.toString()}`);
