/**
 * /api/v1/app/agents/[id]/chats/[chatId]
 *
...
/**
 * OPTIONS /api/v1/app/agents/[id]/chats/[chatId]
 * CORS preflight handler for app chat management endpoint.
 *
...
/**
 * GET /api/v1/app/agents/[id]/chats/[chatId]
  ...
  try {
    ...
    const deduplicatedMessages = messages.filter((msg) => {
      ...
      if (seenMessages.has(key)) {
        logger.warn("[App API] Duplicate message detected", {
          duplicateId: msg.id,
    ...
    logger.info("[App API] After deduplication", {
    ...
    const transformedMessages = deduplicatedMessages.map((msg) => {
      ...
      if (process.env.NODE_ENV !== "production") {
        logger.debug("[App API] Message content structure", {
          msgId: msg.id,
    ...
    logger.error("[App API] Error getting chat", {
      error,
      chatId,
      agentId,
    });
...
/**
 * DELETE /api/v1/app/agents/[id]/chats/[chatId]
...
export async function DELETE(
  ...
  try {
    ...
    logger.info("[App API] Deleted chat", {
      chatId,
      agentId,
      userId: user.id,
    });
    ...
    logger.error("[App API] Error deleting chat", {
      error,
      chatId,
      agentId,
    });
