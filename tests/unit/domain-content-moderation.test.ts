...
mock.module("@/db/schemas/user-characters", () => ({
  userCharacters: { id: "id", is_public: "is_public", interaction_count: "interaction_count" },
}));

// Import after mocks
const { domainContentModerationService } = await import("@/lib/services/domain-content-moderation");
...
describe("Domain Content Moderation Service", () => {
  ...
  describe("runHeuristics", () => {
    it("flags CSAM patterns as critical", () => {
      const result = domainContentModerationService.runHeuristics("this contains child porn content");
    ...
    it("flags illegal patterns as high severity", () => {
      const result = domainContentModerationService.runHeuristics("buy drugs cocaine here cheap");
      expect(result.severity).toBe("high");
      expect(result.flags.some(f => f.type === "illegal")).toBe(true);
    });

    it("flags scam patterns as medium severity", () => {
      const result = domainContentModerationService.runHeuristics("double your bitcoin send 1 btc receive 2");
      expect(result.severity).toBe("medium");
      expect(result.needsAi).toBe(true);
    });

    it("returns clean for normal content", () => {
      const result = domainContentModerationService.runHeuristics("Welcome to our website about cooking recipes");
    ...
    it("detects pedo/paedo terms", () => {
      const result = domainContentModerationService.runHeuristics("pedophile content here");
      expect(result.severity).toBe("critical");
      expect(result.flags[0].type).toBe("csam");
    });

    it("detects hitman services", () => {
      const result = domainContentModerationService.runHeuristics("hitman for hire service available");
      expect(result.severity).toBe("high");
      expect(result.flags[0].type).toBe("illegal");
    });

    it("detects ransomware services", () => {
      const result = domainContentModerationService.runHeuristics("ransomware as a service kit");
      expect(result.severity).toBe("high");
      expect(result.flags[0].type).toBe("illegal");
    });

    it("detects crypto scams", () => {
      const result = domainContentModerationService.runHeuristics("free bitcoin no risk investment");
      expect(result.severity).toBe("medium");
    });
  });
  ...
  describe("shouldScan", () => {
    it("returns true when force option is set", async () => {
      const domain = {
        id: "test-id",
        contentScanCache: { contentHash: "abc123", scannedAt: new Date().toISOString(), result: "clean", confidence: 0.9, flags: [] },
        contentHash: "abc123",
        lastAiScanAt: new Date(),
      };
      
      const result = await domainContentModerationService.shouldScan(domain as any, "abc123", { force: true });
    ...
    it("returns true when content hash changed", async () => {
      ...
      const result = await domainContentModerationService.shouldScan(domain as any, "different-hash");
      expect(result).toBe(true);
    });
    ...
    it("returns false when content unchanged and recently scanned", async () => {
      ...
      const result = await domainContentModerationService.shouldScan(domain as any, "abc123");
  ...
  describe("getPublicAgentsForModeration", () => {
    it("returns formatted agent list", async () => {
      mockDb.query.userCharacters.findMany.mockResolvedValueOnce([
        { id: "agent-1", name: "Test Agent", organization_id: "org-1", is_public: true },
        { id: "agent-2", name: "Another Agent", organization_id: "org-2", is_public: true },
      ]);

      const result = await domainContentModerationService.getPublicAgentsForModeration(10);
  ...
  describe("sampleAgentResponses", () => {
    it("returns clean result when agent not found", async () => {
      ...
      const result = await domainContentModerationService.sampleAgentResponses("nonexistent-id");
    ...
    it("scans agent bio and system prompt", async () => {
      ...
      const result = await domainContentModerationService.sampleAgentResponses("agent-1");
  ...
  describe("runOpenAIModeration", () => {
      ...
      globalThis.fetch = mock(() => Promise.resolve({
        ...
        json: () => Promise.resolve({
          results: [{
            ...
            category_scores: { "sexual/minors": 0.95, hate: 0.01, violence: 0.01, "self-harm": 0.01 }
          }]
        })
      })) as typeof fetch;

      const result = await domainContentModerationService.runOpenAIModeration("some content");
      
      expect(result.aiUnavailable).toBe(false);
      expect(result.flags.some(f => f.type === "csam" && f.severity === "critical")).toBe(true);
      ...
      globalThis.fetch = mock(() => Promise.resolve({
        ...
        json: () => Promise.resolve({
          results: [{
            ...
            category_scores: { "sexual/minors": 0.01, hate: 0.01, violence: 0.01, "self-harm": 0.3, "self-harm/instructions": 0.8 }
          }]
        })
      })) as typeof fetch;

      const result = await domainContentModerationService.runOpenAIModeration("harmful content");
      
      expect(result.flags.some(f => f.type === "illegal" && f.severity === "high")).toBe(true);
    });
      ...
      globalThis.fetch = mock(() => Promise.resolve({
        ...
        json: () => Promise.resolve({
          results: [{
            flagged: false,
            categories: {},
            category_scores: { "sexual/minors": 0.001, hate: 0.002, violence: 0.003, "self-harm": 0.001 }
          }]
        })
      })) as typeof fetch;

      const result = await domainContentModerationService.runOpenAIModeration("normal cooking recipe");
    ...
    it("marks aiUnavailable when API fails", async () => {
      ...
      expect(result.flags.some(f => f.reason.includes("api_failed"))).toBe(true);
    });
    ...
    it("handles invalid response structure", async () => {
      ...
      expect(result.aiUnavailable).toBe(true);
      expect(result.flags.some(f => f.reason.includes("bad_response"))).toBe(true);
    });
  });
  ...
  describe("runDeepClassification", () => {
    ...
    it("flags violations from GPT classification", async () => {
      ...
      const result = await domainContentModerationService.runDeepClassification("buy drugs here", "website");
    ...
    it("flags CSAM-related categories as critical", async () => {
      ...
      const result = await domainContentModerationService.runDeepClassification("illegal content", "website");
    ...
    it("handles suspicious classification", async () => {
      ...
      const result = await domainContentModerationService.runDeepClassification("make money fast", "website");
    ...
    it("returns clean for clean classification", async () => {
      ...
      const result = await domainContentModerationService.runDeepClassification("cooking recipes", "website");
    ...
    it("handles malformed JSON response", async () => {
      ...
      const result = await domainContentModerationService.runDeepClassification("some content", "website");
