...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  const c = await platformCredentialsService.getCredential(
    id,
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  logger.info("[SocialConnections] Revoked", {
    credentialId: id,
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  const success = await platformCredentialsService.refreshToken(
    id,
    authResult.organization_id,
  );
  if (!success) {
    return NextResponse.json(
      {
        success: false,
        error:
          "Token refresh failed. The connection may need to be re-authorized.",
      },
      { status: 400 },
    );
  }
