...
```bash
curl -X POST "https://cloud.eliza.ai/api/v1/mcps" \
...
```bash
curl -X GET "https://cloud.eliza.ai/api/mcp/registry" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://cloud.eliza.ai/api/mcp" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
|--------|-----|-------------|
| Weather | `/api/mcp/demos/weather` | Weather data demo |
| Time | `/api/mcp/demos/time` | Time and timezone demo |
| Crypto | `/api/mcp/demos/crypto` | Cryptocurrency data demo |

### Test Demo Server

```bash
curl -X POST "https://cloud.eliza.ai/api/mcp/demos/weather" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/mcps?agentId=agent_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X DELETE "https://cloud.eliza.ai/api/v1/mcps/mcp_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://cloud.eliza.ai/api/v1/mcps/{mcpId}/publish" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "visibility": "public",
...
<Cards>
  <Cards.Card title="Minimal Tools">
    Only connect tools your agent needs
  </Cards.Card>
  <Cards.Card title="Test Thoroughly">
    Verify tools work before production
  </Cards.Card>
  <Cards.Card title="Handle Errors">
    MCP calls can fail - handle gracefully
  </Cards.Card>
  <Cards.Card title="Monitor Usage">
    Track tool invocations for optimization
  </Cards.Card>
</Cards>
