...
export interface CreditActivityProps {
    transactions: Array<
        Pick<CreditTransaction, "id" | "amount" | "type" | "description" | "created_at"> & {
            actor?: string | null;
        }
    >;
    className?: string;
    ...
    return (
        <Card className={cn("border-border/60 bg-background/85 shadow-sm", className)}>
            ...
            <CardContent className="flex flex-col gap-3">
                {items.length === 0 ? (
                    <p className="text-sm text-muted-foreground">No credit transactions recorded.</p>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-1 flex-col gap-1">
                                        ...
                                        {transaction.actor ? (
                                            <p className="text-xs text-muted-foreground/80">Actor: {transaction.actor}</p>
                                        ) : null}
                                    </div>
                                    <span
                                        className={cn(
