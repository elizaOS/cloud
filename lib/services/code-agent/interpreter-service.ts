...
async function spawnShell(cmd: string, timeout: number): Promise<ExecResult> {
  ...
  return new Promise((resolve) => {
    ...
    proc.on("error", (e) => {
      clearTimeout(timer);
      resolve({ output: "", error: e.message, exitCode: 1, durationMs: Date.now() - start });
    });
  });
}

function executePython(code: string, packages: string[], timeout: number): Promise<ExecResult> {
...
function executeShell(code: string, _packages: string[], timeout: number): Promise<ExecResult> {
  if (DANGEROUS.some((p) => p.test(code))) {
    return Promise.resolve({ output: "", error: "Dangerous command blocked", exitCode: 1, durationMs: 0 });
  }
...
async function executeJavaScript(code: string, packages: string[], timeout: number): Promise<ExecResult> {
  ...
  try {
    ...
    if (result instanceof Promise) {
      ...
      return { output: out.trim(), error: null, exitCode: 0, durationMs: Date.now() - start };
    }
    ...
    return { output: out.trim(), error: msg, exitCode: 1, durationMs: Date.now() - start };
  }
}
...
class InterpreterService {
  async execute(params: InterpreterParams): Promise<InterpreterResult> {
    ...
    const deduction = await creditsService.deductCredits({
      organizationId, amount: cost, description: `Interpreter: ${language}`, metadata: { user_id: userId, language },
    });
