...
export class ConnectionCache {
  ...
  async isEstablished(roomId: string, entityId: string): Promise<boolean> {
    ...
    if (this.memoryCache.has(cacheKey)) {
      logger.debug("[Connection Cache] ✓ Memory cache hit:", roomId.substring(0, 8), entityId.substring(0, 8));
      return true;
    }
    ...
    if (cached) {
      logger.debug("[Connection Cache] ✓ Redis cache hit:", roomId.substring(0, 8), entityId.substring(0, 8));
      // Populate memory cache for next time
      this.memoryCache.set(cacheKey, true);
      return true;
    }

    logger.debug("[Connection Cache] ✗ Cache miss:", roomId.substring(0, 8), entityId.substring(0, 8));
    return false;
  }
  ...
  async markEstablished(roomId: string, entityId: string): Promise<void> {
    ...
    logger.debug("[Connection Cache] ✓ Marked established:", roomId.substring(0, 8), entityId.substring(0, 8));
  }
  ...
  async invalidate(roomId: string, entityId: string): Promise<void> {
    ...
    logger.debug("[Connection Cache] ✗ Invalidated:", roomId.substring(0, 8), entityId.substring(0, 8));
  }
