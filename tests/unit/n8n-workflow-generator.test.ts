...
describe("Template Categories", () => {
  test("covers common use cases", () => {
    ...
    expect(templateNames.some((n) => n.includes("scheduled") || n.includes("report"))).toBe(true);
    
    // Should have data processing template
    expect(templateNames.some((n) => n.includes("data") || n.includes("pipeline"))).toBe(true);
    
    // Should have AI-related template
    expect(templateNames.some((n) => n.includes("ai") || n.includes("agent"))).toBe(true);
  });
...
describe("Prompt Quality", () => {
  test("webhook template mentions trigger and response", () => {
    const webhookTemplate = WORKFLOW_TEMPLATES.find((t) => t.id.includes("webhook"));
  ...
  test("scheduled template mentions timing", () => {
    const scheduledTemplate = WORKFLOW_TEMPLATES.find((t) => t.id.includes("scheduled"));
    expect(scheduledTemplate).toBeDefined();
    expect(scheduledTemplate!.prompt.toLowerCase()).toMatch(/schedule|day|hour|cron/);
  });

  test("data pipeline template mentions transformation", () => {
    const pipelineTemplate = WORKFLOW_TEMPLATES.find((t) => t.id.includes("pipeline"));
    expect(pipelineTemplate).toBeDefined();
    expect(pipelineTemplate!.prompt.toLowerCase()).toMatch(/transform|filter|map|process/);
  });

  test("agent template mentions multi-step or orchestration", () => {
    const agentTemplate = WORKFLOW_TEMPLATES.find((t) => t.id.includes("agent"));
    expect(agentTemplate).toBeDefined();
    expect(agentTemplate!.prompt.toLowerCase()).toMatch(/subtask|step|orchestrat/);
  });
});
...
describe("Template Selection Edge Cases", () => {
  test("selecting template updates prompt state", () => {
    let currentPrompt = "";
    const setPrompt = (value: string) => { currentPrompt = value; };
...
describe("Generated Workflow Validation", () => {
  const mockGeneratedWorkflow = {
    workflow: {
      nodes: [
        ...
        { id: "2", name: "HTTP Request", type: "n8n-nodes-base.httpRequest", position: [450, 300] },
      ],
      connections: {
  ...
  test("connections reference valid node IDs", () => {
    const nodeIds = new Set(mockGeneratedWorkflow.workflow.nodes.map(n => n.id));
    
    for (const [sourceId, outputs] of Object.entries(mockGeneratedWorkflow.workflow.connections)) {
