...
const STORAGE_SERVICE_URL =
  process.env.STORAGE_SERVICE_URL || "http://localhost:3100";
...
export const storageProviderDiscoveryService = {
  ...
  async discoverProviders(
    ...
    const indexerProviders = await this.fetchFromIndexer(options).catch(
      (err) => {
        logger.warn("[StorageDiscovery] Indexer unavailable", {
          error: err.message,
        });
        return [];
      },
    ...
    const localProviders = await this.fetchFromLocalStorage().catch((err) => {
      logger.debug("[StorageDiscovery] Local storage unavailable", {
        error: err.message,
      });
      return [];
    });
  ...
  async getMarketStats(): Promise<{
    ...
    const response = await fetch(INDEXER_URL, {
      ...
      logger.debug("[StorageProviderDiscovery] Indexer fetch failed", {
    ...
    const totalCapacity = providers.reduce(
      (sum, p) => sum + p.totalCapacityGB,
      0,
    );
    const avgPrice =
      providers.length > 0
        ? providers.reduce((sum, p) => sum + p.pricePerGBMonth, 0) /
          providers.length
        : 0.1;
  ...
  async fetchFromIndexer(
    ...
    if (options.minCapacityGB)
      where.availableCapacityGB_gte = options.minCapacityGB;
  ...
  filterProviders(
    ...
    if (options.minCapacityGB) {
      filtered = filtered.filter(
        (p) => p.availableCapacityGB >= options.minCapacityGB!,
      );
    }
    if (options.maxPricePerGBMonth) {
      filtered = filtered.filter(
        (p) => p.pricePerGBMonth <= options.maxPricePerGBMonth!,
      );
    }
    if (options.minHealthScore) {
      filtered = filtered.filter(
        (p) => p.healthScore >= options.minHealthScore!,
      );
    }
    ...
    if (options.tier) {
      filtered = filtered.filter((p) =>
        p.supportedTiers.includes(options.tier!),
      );
    }
    if (options.limit) {
  ...
  /**
   * Get a storage provider interface for uploads/deletes
   * Returns a wrapper around the underlying storage service
   */
  getProvider() {
    return {
      async upload(params: {
        ...
        const filename = params.key.split("/").pop() || "file";
        const result = await storageService.upload(params.data, {
          filename,
      ...
      async delete(key: string): Promise<void> {
        ...
        // storageService.delete expects a URL
