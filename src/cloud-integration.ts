...
export class CloudIntegration {
  ...
  async registerCloudAgent(
    ...
    const metadataEntries = [
      ...
      {
        key: "capabilities",
        value: ethers.toUtf8Bytes(JSON.stringify(metadata.capabilities)),
      },
    ];

    const tx = await this.cloudReputationProvider
      .connect(signer)
      .registerCloudAgent(tokenURI, metadataEntries);
  ...
  async setReputation(
    ...
    if (!this.config.cloudAgentSigner)
      throw new Error("cloudAgentSigner not configured");
    ...
    const tx = await this.cloudReputationProvider
      ...
      .setReputation(
        ...
        ethers.encodeBytes32String(subcategory),
        reason,
        signedAuth,
      );
    await tx.wait();
  ...
  async recordViolation(
    ...
    const tx = await this.cloudReputationProvider
      .connect(signer)
      .recordViolation(agentId, violationType, severityScore, evidence);
    await tx.wait();
  }
  ...
  async proposeBan(
    ...
    const tx = await this.cloudReputationProvider
      .connect(signer)
      .requestBanViaGovernance(agentId, violationType);
    const receipt = await tx.wait();
    const event = receipt.logs.find(
      (log: { topics: string[] }) =>
        log.topics[0] ===
        ethers.id("BanProposalRequested(uint256,bytes32,uint8)"),
    );
  ...
  async checkUserCredit(
    ...
    const required = await this.serviceRegistry.getServiceCost(
      serviceName,
      userAddress,
    );
    const [sufficient, available] =
      await this.creditManager.hasSufficientCredit(
        userAddress,
        tokenAddress,
        required,
      );
  ...
  async getAgentViolations(agentId: bigint, offset = 0, limit = 100) {
    return this.cloudReputationProvider.getAgentViolations(
      agentId,
      offset,
      limit,
    );
