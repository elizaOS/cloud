...
export function UsagePerformance({ stats, dateRangeLabel, className }: UsagePerformanceProps) {
    const { totalRequests, successfulRequests, failedRequests, totalCost, totalInputTokens, totalOutputTokens } = stats;

    const successRate = totalRequests > 0 ? successfulRequests / totalRequests : 0;
    const failureRate = totalRequests > 0 ? failedRequests / totalRequests : 0;
    const avgTokensPerRequest = totalRequests > 0 ? (totalInputTokens + totalOutputTokens) / totalRequests : 0;
        ...
        <Card className={cn("border-border/60 bg-background/85 shadow-sm", className)}>
            <CardHeader className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <CardTitle className="text-sm font-semibold tracking-tight">Request performance</CardTitle>
            ...
            <CardContent className="flex flex-col gap-4">
                <div className="grid gap-3 rounded-xl border border-border/60 bg-background/95 p-4 sm:grid-cols-3">
                    ...
                    <div className="space-y-1">
                        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Failure rate</p>
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl font-semibold text-foreground">
                    ...
                    <div className="space-y-1">
                        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Avg tokens / request</p>
                        <p className="text-2xl font-semibold text-foreground">
                            {numberFormatter.format(Math.round(avgTokensPerRequest))}
                        </p>
                        <p className="text-[11px] uppercase tracking-wide text-muted-foreground/80">
                            {numberFormatter.format(totalInputTokens)} in · {numberFormatter.format(totalOutputTokens)} out
                        </p>
                    </div>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
                        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Total cost (credits)</p>
                        <p className="text-lg font-semibold text-foreground">
