 ...
 * by Stripe event ID via webhook_events, then enqueues to STRIPE_QUEUE
 * and returns 200 immediately. The heavy fan-out (app credits, org
 * credits, revenue splits, redeemable earnings, cache invalidation,
 * Discord notifications, invoice rows) runs in the queue() consumer in
 * src/queue/stripe-event.ts so a slow split + Stripe retry burst can no
 * longer time out the webhook or double-credit on retry.
 *
 * Idempotency:
...
async function handleStripeWebhook(c: AppContext): Promise<Response> {
  ...
  await c.env.STRIPE_QUEUE.send(message, { contentType: "json" });
