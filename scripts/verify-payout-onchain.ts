...
async function verifyEvmNetwork(network: "ethereum" | "base" | "bnb") {
  ...
  try {
    ...
    fail(
      network,
      "rpc_connectivity",
      `RPC connection failed: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  ...
  try {
    ...
    if (code && code !== "0x") {
      pass(
        network,
        "contract_exists",
        `Token contract exists at ${tokenAddress.slice(0, 10)}...`,
      );
    } else {
    ...
    fail(
      network,
      "contract_exists",
      `Contract check failed: ${e instanceof Error ? e.message : "Unknown"}`,
    );
    return;
  }
  ...
  try {
    ...
    fail(
      network,
      "token_name",
      `Failed to read name: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
  ...
  try {
    ...
    fail(
      network,
      "token_symbol",
      `Failed to read symbol: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
  ...
  try {
    ...
    fail(
      network,
      "token_decimals",
      `Failed to read decimals: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
  ...
  try {
    ...
    pass(
      network,
      "total_supply",
      `Total supply: ${Number(formattedSupply).toLocaleString()} tokens`,
      {
    ...
    fail(
      network,
      "total_supply",
      `Failed to read totalSupply: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
    ...
    try {
      ...
      if (balanceNum > 0) {
        pass(
          network,
          "hot_wallet_balance",
          `Hot wallet has ${balanceNum.toLocaleString()} tokens`,
          {
        ...
        fail(
          network,
          "hot_wallet_balance",
          `Hot wallet is empty (${walletAddress.slice(0, 10)}...)`,
          {
            address: walletAddress,
          },
        );
      ...
      fail(
        network,
        "hot_wallet_balance",
        `Failed to check balance: ${e instanceof Error ? e.message : "Unknown"}`,
      );
    }
  } else {
    console.log(
      `  ⚠️ Skipping hot wallet check (EVM_PAYOUT_WALLET_ADDRESS not set)`,
    );
  }
}
...
async function verifySolana() {
  ...
  try {
    ...
    pass(
      "solana",
      "token_exists",
      `Token mint exists at ${tokenAddress.slice(0, 8)}...`,
    );
  } catch (e) {
    fail(
      "solana",
      "token_exists",
      `Token mint not found: ${e instanceof Error ? e.message : "Unknown"}`,
    );
    return;
  }
  ...
  if (mintInfo.decimals === 9) {
    ...
    fail(
      "solana",
      "token_decimals",
      `Expected 9 decimals, got ${mintInfo.decimals}`,
    );
  }
  ...
  if (walletAddress) {
    try {
      ...
      if (balance > 0) {
        pass(
          "solana",
          "hot_wallet_balance",
          `Hot wallet has ${balance.toLocaleString()} tokens`,
          {
            balance,
            address: walletAddress,
          },
    ...
    console.log(
      `  ⚠️ Skipping hot wallet check (SOLANA_PAYOUT_WALLET_ADDRESS not set)`,
    );
  }
}
...
async function verifyPriceOracle() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (price) {
        pass("oracle", "coingecko", `CoinGecko price: $${price.toFixed(8)}`, {
          price,
        });
      } else {
    ...
    fail(
      "oracle",
      "coingecko",
      `CoinGecko failed: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (price) {
        pass(
          "oracle",
          "dexscreener",
          `DexScreener price: $${parseFloat(price).toFixed(8)}`,
    ...
    fail(
      "oracle",
      "dexscreener",
      `DexScreener failed: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (price) {
        pass("oracle", "jupiter", `Jupiter price: $${price.toFixed(8)}`, {
          price,
        });
      } else {
    ...
    fail(
      "oracle",
      "jupiter",
      `Jupiter failed: ${e instanceof Error ? e.message : "Unknown"}`,
    );
  }
}
...
async function main() {
  ...
  console.log(
    "║       elizaOS Payout System On-Chain Verification            ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝",
  );
  ...
  if (process.env.SOLANA_PAYOUT_WALLET_ADDRESS) {
    console.log(
      `Solana Hot Wallet: ${process.env.SOLANA_PAYOUT_WALLET_ADDRESS}`,
    );
  }
  ...
  for (const network of ["ethereum", "base", "bnb", "solana", "oracle"]) {
    ...
    console.log(
      `    ${status} ${network.toUpperCase()}: ${networkPassed}/${networkTotal} passed`,
    );
  }
