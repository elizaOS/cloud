...
export const oauthConnectAction: ActionWithParams = {
  ...
  similes: [
    ...
    "CONNECT_JIRA", "LINK_JIRA",
    "ADD_INTEGRATION", "SETUP_CONNECTION", "LINK_GOOGLE", "AUTHENTICATE",
    "LINK_LINEAR", "LINK_SLACK", "LINK_GITHUB", "LINK_NOTION",
  ],
  description:
    "Connect an OAuth platform for the user. Returns an authorization URL. After user completes OAuth in browser, they should say 'done' to verify the connection. Available: google, linear, slack, github, notion, jira",

  parameters: {
    platform: {
      type: "string",
      description: "Platform to connect. Available: google, linear, slack, github, notion, jira",
      required: true,
    },
  },
  ...
  examples: [
    ...
    [
      { name: "{{name1}}", content: { text: "connect jira" } },
      { name: "{{name2}}", content: { text: "Connect Jira: https://auth.atlassian.com/...", actions: ["OAUTH_CONNECT"] } },
    ],
  ] as ActionExample[][],
};
