...
<Callout type="info">
  **API Key Support:** All voice endpoints support both session-based authentication and API key authentication. 
  This enables developers to build voice-enabled applications programmatically and allows AI agents to use voice capabilities autonomously.
</Callout>
...
|---------|-------------|----------|
| `/api/v1/voice/*` | **Recommended** - Generic, provider-agnostic endpoints | New integrations, programmatic access |
| `/api/elevenlabs/*` | Legacy endpoints (still supported) | Existing integrations, backwards compatibility |

Both patterns support API key authentication and provide identical functionality.
...
```bash
curl "https://elizacloud.ai/api/v1/voice/voice_xyz789" \
  -H "X-API-Key: YOUR_API_KEY"
```
...
```bash
curl -X PATCH "https://elizacloud.ai/api/v1/voice/voice_xyz789" \
  -H "X-API-Key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "https://elizacloud.ai/api/v1/voice/voice_xyz789" \
  -H "X-API-Key: YOUR_API_KEY"
```
