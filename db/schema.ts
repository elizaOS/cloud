...
export const organizations = pgTable(
  ...
  (table) => ({
    ...
    stripe_customer_idx: index('organizations_stripe_customer_idx').on(table.stripe_customer_id),
  })
);
...
export const usageRecords = pgTable(
  ...
  (table) => ({
    ...
    org_created_idx: index('usage_records_org_created_idx').on(table.organization_id, table.created_at),
  })
);
...
export const generations = pgTable(
  'generations',
  {
    ...
    usage_record_id: uuid('usage_record_id').references(() => usageRecords.id, { onDelete: 'set null' }),
    job_id: text('job_id'),
    created_at: timestamp('created_at').notNull().defaultNow(),
    updated_at: timestamp('updated_at').notNull().defaultNow(),
...
export const modelPricing = pgTable(
  'model_pricing',
  {
    ...
    provider: text('provider').notNull(),
    model: text('model').notNull(),
    input_cost_per_1k: decimal('input_cost_per_1k', { precision: 10, scale: 6 }).notNull(),
    output_cost_per_1k: decimal('output_cost_per_1k', { precision: 10, scale: 6 }).notNull(),
    input_cost_per_token: decimal('input_cost_per_token', { precision: 10, scale: 6 }),
    output_cost_per_token: decimal('output_cost_per_token', { precision: 10, scale: 6 }),
    is_active: boolean('is_active').notNull().default(true),
