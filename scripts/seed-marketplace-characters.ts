/**
 * Seed Script: Marketplace Template Characters
 *
 * This script creates a diverse set of template characters for the marketplace.
 * These characters serve as examples and starting points for users.
 *
 * Run with: npx tsx scripts/seed-marketplace-characters.ts
 */
...
async function seedMarketplaceCharacters() {
  console.log("🌱 Starting marketplace character seeding...\n");

  try {
    // Find the first user and organization to assign these template characters
    const firstUser = await db.query.users.findFirst();
    const firstOrg = await db.query.organizations.findFirst();

    if (!firstUser || !firstOrg) {
      console.error("❌ No user or organization found. Please create a user first.");
      console.log("💡 Sign up at /dashboard to create your first user account.");
      return;
    }

    console.log(`✓ Found user: ${firstUser.email}`);
    console.log(`✓ Found organization: ${firstOrg.id}\n`);
    ...
    if (existingTemplates.length > 0) {
      console.log(`⚠️  Found ${existingTemplates.length} existing template characters.`);
      console.log("   Delete them first if you want to reseed.\n");
      return;
    }

    console.log(`📝 Creating ${TEMPLATE_CHARACTERS.length} template characters...\n`);
    ...
    console.log(`\n🎉 Marketplace is now ready with template characters!`);
    console.log(`   Visit /dashboard/agent-marketplace to see them.\n`);

  } catch (error) {
    console.error("\n❌ Error seeding characters:", error);
    throw error;
  }
}

// Run the seeding function
seedMarketplaceCharacters()
  .then(() => process.exit(0))
  .catch((error) => {
