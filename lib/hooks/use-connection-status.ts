...
export interface ServiceStatus {
  ...
  details?: {
    ...
    teamName?: string;
    teamIcon?: string;
    // WhatsApp specific (uses phoneNumber)
    // Notion specific
    workspaceId?: string;
    workspaceName?: string;
    workspaceIcon?: string;
    // Airtable specific (uses email)
    // Webhooks specific
    webhookCount?: number;
  };
}
    ...
    switch (service) {
      ...
      case "webhooks":
        return {
          ...
          loading: false,
          details: {
            webhookCount: data.webhookCount,
          },
        };
      default:
        return {
  ...
  const [statuses, setStatuses] = useState<Record<ServiceType, ServiceStatus>>(
    () => {
      const initial: Record<ServiceType, ServiceStatus> = {
        ...
        notion: defaultStatus,
        airtable: defaultStatus,
        webhooks: defaultStatus,
      };
      return initial;
    },
  );
