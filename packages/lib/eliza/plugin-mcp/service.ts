...
const logMcpBootstrap = (level: "info" | "warn", message: string): void => {
...
export class McpService extends Service {
  ...
  async ensureConnected(serverName: string): Promise<void> {
    // Ensure the dynamic import of getRequestContext has completed before
    // we derive per-entity connection keys (prevents silent fallback to shared keys).
