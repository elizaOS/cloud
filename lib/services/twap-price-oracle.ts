...
export const TWAP_CONFIG = {
  ...
  MAX_VOLATILITY_PERCENT: 0.10, // 10% - if exceeded, pause redemptions
...
export class TWAPPriceOracle {
  ...
  async getRedemptionQuote(
    ...
    if (requiresDelay) {
      warnings.push(`Large redemption - ${SYSTEM_LIMITS.LARGE_REDEMPTION_DELAY_MS / 60000} minute processing delay`);
    }
