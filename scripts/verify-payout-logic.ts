...
async function main() {
  ...
  try {
    ...
    console.log(`  Estimated Cost: ${gasCostEth} ETH (~$${(parseFloat(gasCostEth) * 3500).toFixed(4)})`);
  ...
  try {
    ...
    console.log(`  ✅ PayoutProcessorService class exists: ${!!payoutModule.payoutProcessorService}`);
  } catch (e: unknown) {
    const error = e as Error;
