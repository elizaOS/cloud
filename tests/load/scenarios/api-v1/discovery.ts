...
export function discoverServices(sources = ["local"], limit = 20): Service[] {
  const body = httpPost<{ services: Service[] }>(
    "/api/v1/discovery",
    { sources, limit },
...
export function getServiceDetails(
  ...
  return httpGet<Record<string, unknown>>(`/api/v1/discovery/${agentId}`, {
    tags: { endpoint: "discovery" },
  });
}

export function findMcpTools(tools: string[]): unknown[] {
  const body = httpPost<{ results: unknown[] }>(
    "/api/v1/discovery/mcp-tools",
    { tools },
    { tags: { endpoint: "discovery" } },
  );
  return body?.results ?? [];
