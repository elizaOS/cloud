...
export async function routeCustomDomain(
  ...
  if (!domain) {
    return fail(
      404,
      `Domain ${hostname} not configured`,
      errorPage(
        "Domain Not Found",
        `The domain <strong>${hostname}</strong> is not connected to any resource.`,
      ),
    );
  }
  ...
  if (
    ...
    domain.moderationStatus === "suspended"
  ) {
    return fail(
      403,
      "Domain suspended",
      generateSuspensionPage(hostname, domain.suspensionReason || undefined),
    );
  }
  ...
  if (domain.status === "pending" || !domain.verified) {
    return fail(
      ...
      errorPage(
        ...
        `The domain <strong>${hostname}</strong> is pending DNS verification.`,
      ),
    );
  }
  ...
  switch (domain.resourceType) {
    ...
    default:
      return fail(
        ...
        errorPage(
          "Not Configured",
          `The domain <strong>${hostname}</strong> is not assigned to any service.`,
        ),
      );
  }
}
...
async function routeToContainer(
  ...
  if (!container)
    return fail(
      404,
      "Container not found",
      errorPage("Not Found", "The container no longer exists."),
    );
  if (container.status !== "running")
    return fail(
      503,
      "Container not running",
      errorPage("Unavailable", `Container is ${container.status}.`),
    );
  if (!container.public_url)
    return fail(
      503,
      "No public URL",
      errorPage("Unavailable", "Container has no endpoint."),
    );

  const targetUrl = new URL(pathname, container.public_url);
  targetUrl.search = request.nextUrl.search;

  logger.debug("[DomainRouter] Proxying to container", {
    hostname,
    containerId,
  });
  return {
...
async function routeToAgent(
  ...
  if (!agent)
    return fail(
      404,
      "Agent not found",
      errorPage("Not Found", "The agent no longer exists."),
    );
  ...
  if (
    ...
    pathname.startsWith("/.well-known/agent")
  ) {
  ...
  logger.debug("[DomainRouter] Routing to agent", {
    hostname,
    agentId,
    path: url.pathname,
  });
  return {
    ...
    response: NextResponse.rewrite(url, {
      headers: { "X-Agent-Domain": hostname, "X-Agent-Id": agentId },
    }),
  };
}
...
async function routeToMcp(
  ...
  if (!mcp)
    return fail(
      404,
      "MCP not found",
      errorPage("Not Found", "The MCP server no longer exists."),
    );
  ...
  if (!mcpEndpoint)
    return fail(
      ...
      errorPage("Unavailable", "MCP has no endpoint."),
    );

  const targetUrl = new URL(
    (mcp.endpoint_path || "/mcp") + pathname,
    mcpEndpoint,
  );
  targetUrl.search = request.nextUrl.search;
  ...
  return {
    success: true,
    response: NextResponse.rewrite(targetUrl, {
      headers: { "X-Forwarded-Host": hostname, "X-MCP-Id": mcpId },
    }),
  };
}
