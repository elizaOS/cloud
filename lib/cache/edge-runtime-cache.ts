...
export class EdgeRuntimeCache {
  ...
  async isRuntimeWarm(agentId: string): Promise<boolean> {
    ...
    try {
      const state = await this.redis.get<string>(`${EDGE_CACHE_PREFIX}warm:${agentId}`);
      if (!state) return false;
      
      const parsed = JSON.parse(state) as RuntimeWarmState;
      return parsed.isWarm && (Date.now() - parsed.warmedAt < this.WARM_STATE_TTL * 1000);
    } catch (error) {
   ...
   */
  async getCharacterData(characterId: string): Promise<CachedCharacterData | null> {
    if (!this.redis) return null;
    
    try {
      const data = await this.redis.get<string>(`${EDGE_CACHE_PREFIX}char:${characterId}`);
