/**
 * Canonical JSON error helper for Hono handlers.
 *
 * Shape matches `packages/lib/api/errors.ts` `ApiError.toJSON()`:
 * { success: false, error: <message>, code: <code>, details?: ... }
 *
 * Use `jsonError(c, status, message, code, details?)` for explicit failures
 * and `failureResponse(c, error)` to convert a thrown error.
 */
