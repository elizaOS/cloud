...
import { validateOrigin, addCorsHeaders, createPreflightResponse } from "@/lib/middleware/cors";

const EXCLUDED_PATHS = ["auth", "user", "storage"]; // These stay app-specific

export async function OPTIONS(request: NextRequest) {
  const cors = await validateOrigin(request);
  return createPreflightResponse(cors.origin, ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"]);
}
...
async function proxyRequest(request: NextRequest, path: string[], method: string) {
  ...
  ["content-type", "authorization", "x-api-key", "x-app-token", "x-app-id"].forEach(h => {
  ...
  return addCorsHeaders(NextResponse.json(data, { status: response.status }), cors.origin);
}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (EXCLUDED_PATHS.includes(path[0])) {
    ...
    return NextResponse.json({ error: "Use specific endpoint" }, { status: 404 });
  }
