...
describe("Webhook Signature Verification", () => {
  ...
  test("rejects expired signature", async () => {
    ...
    const signature = generateWebhookSignature({ payload, secret, timestamp: oldTimestamp });
...
describe("Cron Expression Parsing", () => {
  ...
  test("validates cron expression format", () => {
    ...
    invalidExpressions.forEach(expr => {
      ...
      expect(!hasCorrectParts || containsInvalidText || hasInvalidMinute).toBe(true);
    });
  });
});
...
describe("Response Formats", () => {
  ...
  test("trigger creation returns secret only once", () => {
    ...
    expect((getResponse.config as Record<string, unknown>).webhookSecret).toBeUndefined();
  });
});
