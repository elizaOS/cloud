...
<Tabs items={['cURL', 'JavaScript', 'Python']}>
  <Tabs.Tab>
```bash
curl -X POST "https://cloud.eliza.ai/api/v1/embeddings" \
...
```
  ...
  <Tabs.Tab>
```javascript
const response = await fetch('https://cloud.eliza.ai/api/v1/embeddings', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://cloud.eliza.ai/api/v1/embeddings',
    headers={
        'Authorization': 'Bearer YOUR_API_KEY',
        'Content-Type': 'application/json',
    },
...
```javascript
// Embed your query
const queryResponse = await fetch('https://cloud.eliza.ai/api/v1/embeddings', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer YOUR_API_KEY',
    'Content-Type': 'application/json',
