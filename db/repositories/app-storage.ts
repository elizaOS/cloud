...
export const appCollectionsRepository = {
  ...
  async updateStorageUsed(id: string, bytesUsed: number): Promise<void> {
    await db
      .update(appCollections)
      .set({ storage_used_bytes: bytesUsed, updated_at: new Date() })
      .where(eq(appCollections.id, id));
  },
};
...
export const appDocumentsRepository = {
  ...
  async listByCollection(
    ...
    const orderColumn =
      orderBy === "updated_at"
        ? appDocuments.updated_at
        : appDocuments.created_at;
    return db
      .select()
      .from(appDocuments)
      .where(
        and(
          eq(appDocuments.collection_id, collectionId),
  ...
  async queryByIndex(
    ...
    return db
      ...
      .where(
        and(
          eq(appDocuments.collection_id, collectionId),
          eq(appDocuments[indexSlot], value as never),
          isNull(appDocuments.deleted_at),
        ),
      )
  ...
  async deleteByCollection(collectionId: string): Promise<number> {
    const result = await db
      .delete(appDocuments)
      .where(eq(appDocuments.collection_id, collectionId));
...
export const appDocumentChangesRepository = {
  ...
  async getByDocument(
    documentId: string,
    limit: number = 50,
  ): Promise<Array<typeof appDocumentChanges.$inferSelect>> {
    return db
      .select()
      .from(appDocumentChanges)
      .where(eq(appDocumentChanges.document_id, documentId))
      .orderBy(desc(appDocumentChanges.changed_at))
      .limit(limit);
  },

  async getRecentByApp(
    appId: string,
    limit: number = 100,
  ): Promise<Array<typeof appDocumentChanges.$inferSelect>> {
    return db
      .select()
      .from(appDocumentChanges)
      .where(eq(appDocumentChanges.app_id, appId))
      .orderBy(desc(appDocumentChanges.changed_at))
      .limit(limit);
  },
};
