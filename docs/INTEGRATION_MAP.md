...
- **Primary Role:** User-facing platform, billing orchestrator, milaidy sandbox orchestration
...
### 3.1 Milaidy Sandbox Flow (ElizaCloud)

**Primary File:** `lib/services/milaidy-sandbox.ts`

#### Architecture
```
MilaidySandboxService (orchestrator)
  ↓ delegates to
...
#### Provision Flow
```typescript
1. CREATE: milaidySandboxService.createAgent({
     organizationId, userId, agentName, agentConfig, environmentVars
   }) → creates DB record (status: pending)

2. PROVISION: milaidySandboxService.provision(agentId, orgId)
...
3. BRIDGE: milaidySandboxService.bridge(agentId, orgId, rpc)
   POST {bridgeUrl}/bridge
   { jsonrpc: "2.0", method: "...", params: {...} }

4. SNAPSHOT: milaidySandboxService.snapshot(agentId, orgId, type)
   GET {bridgeUrl}/api/snapshot → returns { memories, config, workspaceFiles }
   Store in milaidy_sandbox_backups table
   Prune old backups (keep last 10)

5. SHUTDOWN: milaidySandboxService.shutdown(agentId, orgId)
...
```typescript
// Backup types
type MilaidyBackupSnapshotType = "auto" | "manual" | "pre-shutdown";

// Backup data structure
interface MilaidyBackupStateData {
  memories: Array<{ role, text, timestamp }>;
  config: Record<string, unknown>;
  workspaceFiles: Record<string, string>;
}

// Stored in milaidy_sandbox_backups table
...
MILAIDY_AGENT_TEMPLATE_URL: Git repo (default: elizaos/milaidy-cloud-agent-template)
...
```typescript
// Environment variables
MILAIDY_DOCKER_IMAGE: Docker image (default: milady/agent:cloud-full-ui)
MILAIDY_DOCKER_NODES: nodeId:hostname:capacity,... (e.g., "node1:10.0.0.5:8,node2:10.0.0.6:8")
...
#### Sandbox Creation
```typescript
1. Parse nodes from MILAIDY_DOCKER_NODES
...
7. docker pull {MILAIDY_DOCKER_IMAGE}
8. docker run -d \
     --name milady-{agentId} \
...
1. neon.createProject({
     name: `milaidy-{agentName}-{agentId.slice(0,8)}`,
     region: "aws-us-east-1"
   })
2. Returns: { projectId, branchId, connectionUri }
3. Stored in milaidy_sandboxes: neon_project_id, neon_branch_id, database_uri
...
**Recommendation:**
- Add `cost_per_hour` to milaidy_sandboxes table
...
| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/api/v1/milaidy/agents` | GET | Org | List all agents |
| `/api/v1/milaidy/agents` | POST | Org | Create new agent |
| `/api/v1/milaidy/agents/{id}` | GET | Org | Get agent details |
| `/api/v1/milaidy/agents/{id}` | DELETE | Org | Delete agent |
| `/api/v1/milaidy/agents/{id}/provision` | POST | Org | Provision agent |
| `/api/v1/milaidy/agents/{id}/bridge` | POST | Org | Send JSON-RPC to agent |
| `/api/v1/milaidy/agents/{id}/stream` | POST | Org | Streaming bridge call |
| `/api/v1/milaidy/agents/{id}/snapshot` | POST | Org | Create backup |
| `/api/v1/milaidy/agents/{id}/restore` | POST | Org | Restore from backup |
| `/api/v1/milaidy/agents/{id}/backups` | GET | Org | List backups |
...
**Example: Bridge Call**
```typescript
POST /api/v1/milaidy/agents/{id}/bridge
Headers:
  Authorization: Bearer {api_key}
Body:
{
...
#### **ElizaCloud: milaidy_sandbox_backups**
```sql
id                              UUID PRIMARY KEY
sandbox_record_id               UUID NOT NULL REFERENCES milaidy_sandboxes
...
```
1. User calls POST /api/v1/milaidy/agents
2. ElizaCloud creates milaidy_sandboxes record (status: pending)
3. On provision:
   a. Neon API → create project
   b. Update milaidy_sandboxes (neon_project_id, database_uri)
   c. SandboxProvider → create container/sandbox
   d. Update milaidy_sandboxes (sandbox_id, bridge_url, status: running)
```
...
async syncAllocatedCounts(): Promise<Map<string, {before, after}>> {
  // Count containers in milaidy_sandboxes WHERE node_id=X AND status NOT IN (stopped, error)
...
5. IP stored in milaidy_sandboxes.headscale_ip
```
...
HEADSCALE_USER=milaidy  # VPN user namespace
```
...
- Manual node provisioning via `MILAIDY_DOCKER_NODES` env var
...
**Recommendation:**
1. Add `cost_per_hour` to milaidy_sandboxes table
...
3. Add endpoint: `POST /api/v1/milaidy/agents/{id}/restore?backupId={id}`
...
# Docker Nodes
MILAIDY_DOCKER_NODES=node1:10.0.0.5:8,node2:10.0.0.6:8
MILAIDY_DOCKER_IMAGE=milady/agent:cloud-full-ui

# Vercel Sandbox
VERCEL_OIDC_TOKEN=... (or VERCEL_TEAM_ID, VERCEL_PROJECT_ID, VERCEL_TOKEN)
MILAIDY_AGENT_TEMPLATE_URL=https://github.com/elizaos/milaidy-cloud-agent-template.git
```
...
- Sandboxes: `lib/services/milaidy-sandbox.ts`, `lib/services/vercel-sandbox-provider.ts`, `lib/services/docker-sandbox-provider.ts`
- Infrastructure: `lib/services/docker-node-manager.ts`, `lib/services/docker-ssh.ts`, `lib/services/headscale-client.ts`
- API: `app/api/v1/milaidy/`, `app/api/v1/credits/`, `app/api/v1/models/`
