...
async function testEndpointDiscovery() {
  ...
  console.log(
    `   - A2A: ${discoveredEndpoints.filter((e) => e.type === "a2a").length}`,
  );
  console.log(
    `   - MCP: ${discoveredEndpoints.filter((e) => e.type === "mcp").length}`,
  );
...
async function testNodeGeneration() {
  ...
  const a2aEndpoints = discoveredEndpoints
    .filter((e) => e.type === "a2a")
    .slice(0, 5);
  const mcpEndpoints = discoveredEndpoints
    .filter((e) => e.type === "mcp")
    .slice(0, 5);
  const restEndpoints = discoveredEndpoints
    .filter((e) => e.type === "rest")
    .slice(0, 10);

  console.log(
    `Testing node generation for ${a2aEndpoints.length} A2A, ${mcpEndpoints.length} MCP, ${restEndpoints.length} REST endpoints...\n`,
  );
    ...
    try {
      ...
      if (endpoint.type === "rest") {
        ...
        if (!node.parameters.jsonBody || !node.parameters.sendBody) {
          throw new Error(
            `${endpoint.type.toUpperCase()} node missing jsonBody or sendBody`,
          );
        }
        if (
          ...
          throw new Error(
            `${endpoint.type.toUpperCase()} node missing correct contentType/specifyBody`,
          );
        }
      }
...
async function testA2AAndMCPCoverage() {
  ...
  const missingA2a = Array.from(a2aSkillIds).filter(
    (id) => !discoveredA2aIds.has(id),
  );
  ...
  const missingMcp = mainMcpTools.filter(
    (name) => !discoveredMcpToolNames.has(name),
  );
...
async function generateComprehensiveReport() {
  ...
  for (const endpoint of discoveredEndpoints) {
    byCategory.set(
      endpoint.category,
      (byCategory.get(endpoint.category) || 0) + 1,
    );
  ...
  console.log("\nBy Source:");
  for (const [source, count] of Array.from(bySource.entries()).sort(
    (a, b) => b[1] - a[1],
  )) {
...
async function runAllValidations() {
  ...
  if (results.endpointDiscovery) {
    ...
    const coveragePercent = (
      (coverage.covered / coverage.totalRoutes) *
      100
    ).toFixed(1);
  ...
  if (results.nodeGeneration) {
    ...
    const successRate = (
      (gen.successCount / (gen.successCount + gen.errorCount)) *
      100
    ).toFixed(1);
  ...
  if (results.a2aMcpCoverage) {
    ...
    console.log(
      `A2A Coverage: ${a2a.a2aDiscovered}/${a2a.a2aTotal} skills discovered`,
    );
  ...
  if (allPassed) {
    console.log(
      "🎉 ALL VALIDATIONS PASSED! All APIs are exposed as n8n nodes.",
    );
    process.exit(0);
  } else {
