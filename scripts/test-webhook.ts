...
import { WebhookTestClient, testWebhook, sendWebhook } from "../lib/utils/webhook-test-client";

// Parse arguments
const args = process.argv.slice(2);

if (args.length < 2 && !args.includes("--help") && !args.includes("-h")) {
  console.error("❌ Missing required arguments: webhook-key and webhook-secret");
  console.error("   Usage: bun run scripts/test-webhook.ts <webhook-key> <webhook-secret> [options]");
...
const baseUrl = getOption("--base-url") || process.env.WEBHOOK_BASE_URL || "http://localhost:3000";
...
async function main() {
  ...
  // Rate limit info
  const rateLimit = result.headers["x-ratelimit-limit"] || result.headers["X-RateLimit-Limit"];
  const remaining = result.headers["x-ratelimit-remaining"] || result.headers["X-RateLimit-Remaining"];
  if (rateLimit) {
