...
<Callout type="info">
Most public REST endpoints live under <code>/api/v1</code>. Some internal endpoints (admin/cron) are hidden by default — toggle “Show all” in the explorer if you need them.
</Callout>

## Base URL

```text
https://cloud.eliza.ai/api/v1
```
...
```bash
curl -X GET "https://cloud.eliza.ai/api/v1/models" \
...
```text
https://cloud.eliza.ai/api/openapi.json
```
