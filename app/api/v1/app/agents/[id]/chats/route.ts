...
/**
 * OPTIONS /api/v1/app/agents/[id]/chats
 * CORS preflight handler for app agent chats endpoint.
 *
...
export async function GET(
  ...
  try {
    ...
    logger.error("[App API] Error listing chats", { error, agentId });
...
/**
 * POST /api/v1/app/agents/[id]/chats
...
export async function POST(
  ...
  try {
    ...
    logger.info("[App API] Created chat", {
      roomId: room.id,
      agentId,
      userId: user.id,
    });
    ...
    logger.error("[App API] Error creating chat", { error, agentId });
