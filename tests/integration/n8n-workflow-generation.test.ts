...
describe("N8N Workflow Generation", () => {
  test("Endpoint Discovery API works", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }

    const response = await fetchWithAuth("/api/v1/n8n/nodes/discover?limit=10");
  ...
  test("Node Generation API works", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }
    ...
    const discoverResponse = await fetchWithAuth("/api/v1/n8n/nodes/discover?limit=5");
    ...
    // Generate a node
    const response = await fetchWithAuth("/api/v1/n8n/nodes/generate", "POST", {
  ...
  test("Workflow Generation API works (simple workflow)", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }

    const response = await fetchWithAuth("/api/v1/n8n/generate-workflow", "POST", {
      prompt: "Create a simple workflow with a start node and an HTTP request node that calls https://api.example.com/data",
      autoSave: false,
    });
  ...
  test("Workflow Generation with Auto-Save works", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }
    ...
    const getResponse = await fetchWithAuth(`/api/v1/n8n/workflows/${data.savedWorkflow.id}`);
    ...
    console.log(`✅ Generated and saved workflow: ${workflowName} (${data.savedWorkflow.id})`);
    
    // Cleanup - delete the test workflow
    await fetchWithAuth(`/api/v1/n8n/workflows/${data.savedWorkflow.id}`, "DELETE");
  }, TIMEOUT);

  test("Workflow Generation includes discovered endpoints", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }
    ...
    const response = await fetchWithAuth("/api/v1/n8n/generate-workflow", "POST", {
      prompt: "Create a workflow that uses the check_credits MCP tool and then calls a REST API",
      autoSave: false,
    });
    ...
    const hasHttpNodes = data.workflow.nodes.some((node: { type: string }) => 
  ...
  test("Workflow Generation validation catches errors", async () => {
    if (!serverAvailable || !apiKeyValid) {
      console.log("⏭️  Skipping test - prerequisites not met");
      return;
    }
