...
```bash
# List your voices
curl -X GET "https://cloud.eliza.ai/api/elevenlabs/voices/user" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
  </Tabs.Tab>
</Tabs>
...
```javascript
const response = await fetch('https://cloud.eliza.ai/api/elevenlabs/tts', {
  method: 'POST',
  headers: {
...
```bash
curl -X GET "https://cloud.eliza.ai/api/elevenlabs/voices" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://cloud.eliza.ai/api/elevenlabs/voices/voice_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X GET "https://cloud.eliza.ai/api/elevenlabs/voices/jobs" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
| TTS (per character) | 0.0001 credits |
| STT (per minute) | 0.10 credits |
...
<Cards>
  <Cards.Card title="Quality Samples">
    Use high-quality, noise-free audio
  </Cards.Card>
  <Cards.Card title="Natural Speech">
    Include varied intonation and pacing
  </Cards.Card>
  <Cards.Card title="Sufficient Length">
    1-3 minutes of audio for best results
  </Cards.Card>
  <Cards.Card title="Test Thoroughly">
    Verify clone quality before production
  </Cards.Card>
</Cards>
