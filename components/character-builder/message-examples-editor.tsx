...
import { Plus, Trash2, MessageSquare, User, Bot, X } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {(!examples || examples.length === 0) && (
          <div className="text-center p-8 border border-dashed border-white/10 bg-white/5 rounded-xl">
        ...
        {examples?.map((conversation, convIndex) => (
          <div
            key={convIndex}
            className="group relative border border-white/10 bg-black/40 p-4 space-y-4 rounded-xl"
          >
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="h-6 w-32 text-xs bg-transparent border-transparent hover:border-white/10 focus:border-white/20 px-1 py-0 text-white/70 rounded-lg"
                        placeholder="Speaker"
                      />
                    ...
                    <Textarea
                      ...
                      className="min-h-[60px] rounded-xl border-white/10 bg-white/5 text-sm text-white placeholder:text-white/20 focus:ring-1 focus:ring-[#FF5800] focus:border-[#FF5800]"
            ...
            <button
              ...
              className="w-full py-2 text-xs text-white/40 hover:text-white hover:bg-white/5 border border-dashed border-white/10 transition-colors rounded-lg"
            >
              + Add Message
            </button>
          </div>
