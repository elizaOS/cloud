        ...
        <Card className="h-full border-border/60 bg-background/70">
            ...
            >
                <CardHeader className="pb-0">
                    <CardTitle className="text-xl font-semibold">Generate a video</CardTitle>
                    <CardDescription>
                        Describe the scene you have in mind, choose the model preset, and submit
                        to send a generation job to the Fal runtime.
                    </CardDescription>
                    {errorMessage ? (
                    ...
                    <div className="grid gap-3 rounded-xl border border-dashed border-border/60 bg-muted/20 p-4 text-sm text-muted-foreground">
                        ...
                        <div className="grid gap-2 rounded-lg bg-background/60 p-3">
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                                ...
                                <span className="font-medium text-foreground">{activeModel.dimensions}</span>
                            </div>
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        <p className="text-xs leading-relaxed">
                            Advanced controls for seed, duration, and motion strength will surface
                            here as soon as the generation API exposes them.
                        </p>
                    </div>
                ...
                <CardFooter className="flex flex-col gap-2 border-t border-border/60 bg-background/80 py-4">
                    ...
                    <div className="space-y-1 text-center text-xs text-muted-foreground" aria-live="polite">
                        {statusMessage ? (
                            <p className="text-foreground/80">{statusMessage}</p>
                        ) : null}
                        <p>
                            Generation credits refresh as renders finish—wire this panel to your
