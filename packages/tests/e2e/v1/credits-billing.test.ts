...
describe.skipIf(!api.hasApiKey())("Organization Credits", () => {
  ...
  test("POST /api/v1/credits/checkout creates Stripe session", async () => {
    ...
    // May return 200 (session created) or 500 (Stripe not configured in test env)
    if (response.status === 200) {
      ...
      // Stripe may not be configured — that's OK for test validation
...
// ── App-Level Credits ───────────────────────────────────────────────
describe.skipIf(!api.hasApiKey())("App Credits", () => {
  let appId: string;
  ...
  test("POST /api/v1/app-credits/checkout creates app checkout session", async () => {
    ...
    if (response.status === 200) {
      ...
      // Stripe may not be configured — validate error is reasonable
      expect([400, 500]).toContain(response.status);
    }
  });
...
// ── Credit Verification ─────────────────────────────────────────────
describe.skipIf(!api.hasApiKey())("Credit Verification", () => {
