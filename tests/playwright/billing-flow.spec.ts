...
test.describe("Billing Page UI", () => {
  ...
  test("transaction history section exists", async ({ page }) => {
    ...
    console.log(
      `✅ Transaction history - Heading: ${hasHeading}, Table: ${hasTable}`,
    );
  });
});
...
test.describe("Billing API", () => {
  ...
  test("GET /api/credits/balance returns credit balance", async ({
    request,
  }) => {
  ...
  test("GET /api/credits/transactions returns transaction history", async ({
    request,
  }) => {
  ...
  test("GET /api/stripe/credit-packs returns available packs", async ({
    request,
  }) => {
  ...
  test("POST /api/stripe/create-checkout-session creates checkout session", async ({
    request,
  }) => {
    // First get available packs
    const packsResponse = await request.get(
      `${BASE_URL}/api/stripe/credit-packs`,
      {
        headers: authHeaders(),
      },
    );
    ...
    if (response.status() === 200) {
      ...
      console.log(
        "ℹ️ Stripe checkout not configured (expected in test environment)",
      );
    }
  });
});
...
test.describe("Auto Top-Up API", () => {
  ...
  test("GET /api/auto-top-up/settings returns settings", async ({
    request,
  }) => {
    ...
    console.log(
      `✅ Auto top-up - Enabled: ${data.enabled}, Amount: $${data.amount}, Threshold: $${data.threshold}`,
    );
  });

  test("POST /api/auto-top-up/settings updates settings", async ({
    request,
  }) => {
    // First get current settings
    const getResponse = await request.get(
      `${BASE_URL}/api/auto-top-up/settings`,
      {
        headers: authHeaders(),
      },
    );
  ...
  test("POST /api/auto-top-up/settings validates threshold < amount", async ({
    request,
  }) => {
    const response = await request.post(
      `${BASE_URL}/api/auto-top-up/settings`,
...
test.describe("Billing Success Page", () => {
  ...
  test("dashboard billing success page handles missing session_id", async ({
    page,
  }) => {
