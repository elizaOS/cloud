...
export function createX402RequirementResponse(
  ...
  const usdcAddress =
    (x402Config.networks as Record<string, { usdc: string }>)[network]?.usdc ||
    "0x0000000000000000000000000000000000000000";
  const payToAddress =
    (x402Config.elizaToken.evm as Record<string, string>)[network] ||
    "0x0000000000000000000000000000000000000000";
  ...
  const requirementHeader = Buffer.from(JSON.stringify(requirement)).toString(
    "base64",
  );
