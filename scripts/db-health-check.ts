...
async function checkRecentSeqScans(): Promise<
  Array<{
    relname: string;
    seq_scan: string;
    idx_scan: string;
...
async function main() {
  ...
  console.log(
    `  Total: ${connStats.total} | Active: ${connStats.active} | Idle: ${connStats.idle}`,
  );
  console.log();
  ...
  if (!slowQueryData.exists) {
    ...
    if (stats.oldest_query) {
      console.log(
        `  First logged: ${new Date(stats.oldest_query).toLocaleString()}`,
      );
      console.log(
        `  Last logged: ${new Date(stats.newest_query).toLocaleString()}`,
      );
    }
  }
  console.log();
  ...
  if (slowQueryData.topQueries.length > 0 && detailed) {
    ...
    for (let i = 0; i < slowQueryData.topQueries.length; i++) {
      ...
      console.log();
      console.log(
        `  ${i + 1}. Avg: ${q.avg_duration_ms}ms | Max: ${q.max_duration_ms}ms | Calls: ${q.call_count}`,
      );
      console.log(
        `     Last seen: ${new Date(q.last_seen_at).toLocaleString()}`,
      );
  ...
  console.log(
    `  Environment: ${env || "not set"} (VERCEL_ENV: ${vercelEnv || "not set"})`,
  );
  console.log(
    `  Instrumentation: ${instrumentationEnabled ? "✅ ENABLED" : "❌ DISABLED"}`,
  );
  ...
  console.log(
    `  Alerts: ${hasDiscord || hasSlack ? "✅ Configured" : "❌ Not configured"}`,
  );
