...
export async function GET(request: NextRequest) {
  ...
  const collections = await appStorageService.listCollections(app.id);
...
export async function POST(request: NextRequest) {
  ...
  const existing = await appStorageService.getCollection(app.id, name);
  if (existing) {
    return NextResponse.json(
  ...
  const collection = await appStorageService.createCollection({
    appId: app.id,
    name,
    description,
  ...
  logger.info(`[App Storage API] Created collection: ${name}`, {
    appId: app.id,
    userId: user.id,
